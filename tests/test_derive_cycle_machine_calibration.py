...
def test_the_verdict_is_a_conjunction_not_an_average(fresh):
    ...
    assert cal["band"] == {
        "low": CALIBRATION_BAND[0], "high": CALIBRATION_BAND[1],
...
def test_the_boundary_is_read_off_the_design_document(loaded):
    ...
    assert rule["design_cycles_per_boundary"] == 120
    assert rule["design_band"] == [107, 137]
    assert "fill 36" in rule["design_decomposition"]
    assert "120-cycle boundary" in rule["rung_rule"]
...
def test_the_boundary_is_unmeasured_and_folded_into_the_verdict(fresh):
    ...
    assert b["ratio_model_chain_over_design"] == pytest.approx(
        b["model_exposed_chain_cycles"] / b["design_cycles_per_boundary"]
    )
    lo, hi = CALIBRATION_BAND
    outside = not (lo <= b["ratio_model_chain_over_design"] <= hi)
    assert outside, "the model's chain reached the design band; revisit rung_item_met"
    assert b["technology_latency_rederived"] is False
    assert b["rung_item_met"] is False
    assert "FOLDED" in b["rung_item_decision"]
...
# ---------------------------------------------------------------------------
# 6.  The control plane carries no cycle count, and the artifact says so
# ---------------------------------------------------------------------------
def test_the_control_plane_record_carries_no_cycles_and_the_artifact_says_so(
    fresh, loaded, paths,
):
    cp = fresh["calibration"]["control_plane"]
    assert cp["measured"] is False and cp["status"] == "unmeasured"
    assert cp["cases"], "the L1-CP record has cases"
    for case in cp["cases"]:
        assert case["rtl_cycles"] is None and case["ratio_model_over_rtl"] is None
        assert case["within_band"] is None
        assert case["model_front_end_cycles_lower_bound"] == case["rtl_fetched"] * 3
    assert "control_plane" in fresh["calibration"]["blocks_without_measured_cycles"]
    text = paths["control_plane"].read_text()
    assert "cycle" not in text.lower(), (
        "results/rtl/abi3_deployment_campaign.json now carries a cycle field; "
        "extend calibrate_control_plane to divide the model's sequencer cost by it"
    )
