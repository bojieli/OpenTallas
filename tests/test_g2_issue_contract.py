...
def test_g2_issue_contract(tmp_path, resolve, inputs, stream):
    image = tmp_path / "sim"
    subprocess.run(
        [
            "iverilog",
            ...
            f"-Ptb_a3_g2_issue_contract.STREAM={stream}",
            "-s",
    ...
    assert f"PASS G2 issue contract checks={30+4*resolve+14*inputs+2*stream}" in result.stdout
