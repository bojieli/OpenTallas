...
def test_contract_is_bound_to_pinned_source_profiles_and_nonclaims() -> None:
    ...
    assert COMPRESS_STATE_PROFILE == ("opentallas.deepseek_v4_compress_state_update.v1")
...
def test_overlap_short_prefill_only_populates_current_remainder() -> None:
    ...
    result = compress_state_update_f32(state, kv, scores, ape, start_pos=0)
    ...
    assert result.state.kv_f32_codes[0][:4] == before.kv_f32_codes[0][:4]
    assert result.state.kv_f32_codes[0][4:7] == kv[0]
    assert result.state.kv_f32_codes[0][7] == before.kv_f32_codes[0][7]
...
def test_overlap_exact_one_group_pads_pool_and_retains_full_group() -> None:
    ...
    old_current = state.kv_f32_codes[0][4:]
    ...
    assert result.state.kv_f32_codes[0][4:] == old_current
...
def test_overlap_long_prefill_uses_previous_and_current_feature_halves() -> None:
    ...
    assert counters == CompressionStateCounters(
        ...
        logical_kv_state_f32_values_written=24,
        logical_score_state_f32_values_written=24,
        logical_kv_state_write_bytes=96,
        logical_score_state_write_bytes=96,
        kv_state_f32_values_preserved=40,
        score_state_f32_values_preserved=40,
...
def test_overlap_exact_multiple_leaves_current_half_untouched() -> None:
    ...
    assert result.state.kv_f32_codes[0][:4] == kv[0][4:8]
    assert result.state.kv_f32_codes[0][4:] == old_current_kv
    assert result.state.score_f32_codes[0][4:] == old_current_score
...
def test_nonoverlap_short_prefill_stores_only_remainder_without_pool() -> None:
    ...
    assert result.state.kv_f32_codes[0][3:] == before.kv_f32_codes[0][3:]
...
def test_nonoverlap_prefill_emits_complete_prefix_and_stores_remainder() -> None:
    ...
    assert result.state.kv_f32_codes[0][2:] == before.kv_f32_codes[0][2:]
    ...
    assert counters.logical_kv_state_f32_values_written == 2
    assert counters.logical_kv_state_f32_values_read == 0
    assert counters.kv_state_f32_values_preserved == 254
    assert counters.pool_kv_f32_values == 128
    assert counters.overlap_pool_zero_f32_values == 0


def test_nonoverlap_exact_multiple_does_not_rewrite_decode_state() -> None:
    state = _tagged_state(ratio=128, batches=1, head_dim=1)
    ...
    assert result.should_compress
    assert result.counters.logical_kv_state_f32_values_written == 0
    assert result.counters.kv_state_f32_values_preserved == 128
...
def test_overlap_decode_nonboundary_writes_current_slot_only() -> None:
    ...
    ape = _ape(4, 4)
    result = compress_state_update_f32(state, kv, scores, ape, start_pos=5)
...
def test_overlap_decode_boundary_emits_then_rolls_full_current_state() -> None:
    ...
    ape = _ape(4, 4, base=2)
    result = compress_state_update_f32(state, kv, scores, ape, start_pos=7)
...
def test_nonoverlap_decode_boundary_pools_state_without_roll() -> None:
    ...
    ape = _ape(128, 1)
    result = compress_state_update_f32(state, kv, scores, ape, start_pos=127)
...
def test_randomized_transactions_match_independent_structural_oracle() -> None:
    ...
    for ratio in (4, 128):
        for case in range(40):
            ...
            if rng.randrange(2):
                ...
                phase = rng.randrange(ratio)
                start_pos = ratio * rng.randint(1, 5) + phase
                sequence = 1
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (CompressionState(4, (((),) * 8,), (((),) * 8,)), "at least one value"),
        (
            CompressionState(
                4,
                (((_f32(1),),) * 7,),
...
def test_malformed_source_and_ape_axes_fail_closed(
    ...
    state = zero_compression_state_f32(ratio=4, batch_capacity=1, head_dim=1)
    with pytest.raises(CompressionStateReferenceError, match=match):
        compress_state_update_f32(state, kv, scores, ape, start_pos=0)
...
def test_invalid_or_nonfinite_transaction_inputs_fail_closed(
    ...
    with pytest.raises(CompressionStateReferenceError, match=match):
        compress_state_update_f32(state, kv, scores, ape, start_pos=1)
...
def test_only_negative_infinity_score_state_sentinel_is_admitted() -> None:
    ...
    state = compression_state_f32(kv, valid_scores, ratio=4)
