...
def test_operand_admission(tmp_path):
    ...
    cases += [(65535, 8, 65535, 1, 0, 0, 0, 0, 0), (65535, 16, 65535, 1, 0, 0, 0, 0, 0)]
    vectors = []
    for i, (rows, cols, k, g, sa, sb, ba, bb, rpb) in enumerate(cases):
        ...
        extent = rows * (cols // 8) * ((k + effective_g - 1) // effective_g)
        bad = bad or extent == 0 or extent > 0xFFFFFFFF or extent + 400 > 0x100000000
        vectors.append(
            f"""check_case(16'd{rows},16'd{cols},16'd{k},8'd{g},1'b{sa},1'b{sb},16'd{ba},16'd{bb},16'd{rpb},1'b{int(bad)},16'd{(k + effective_g - 1) // effective_g},16'd{k // ba if sa and ba else 0},16'd{k // bb if sb and bb else 0},32'd400);"""
        )
    for base, bad in [(0xFFFFFFFE, 0), (0xFFFFFFFF, 1)]:
        vectors.append(f"check_case(1,8,2,1,0,0,0,0,0,1'b{bad},2,0,0,32'd{base});")
...
integer cycles;
reg [63:0] expected_extent;
begin
 expected_extent=64'(nr)*64'(nc/8)*64'(words);
 cfg_generation=32'h123;cfg_a_base=100;cfg_s_base=200;cfg_ws_base=300;cfg_w_base=wb;
 ...
 repeat(4)begin
  ...
  if(!bad && stream_words!=expected_extent[31:0])$fatal(1,"stream extent mismatch");
