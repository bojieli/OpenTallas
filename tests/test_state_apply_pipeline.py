...
def test_apply_pipeline(tmp_path,simulator):
...
integer t,i,waits,transactions=0,cancellations=0;
...
'''.replace('PORTS',ports).replace('COMPARE',compare))
    sources=[str(ROOT/'rtl/abi3/ot_a3_pkg.sv'),str(ROOT/'results/rtl/state_apply_pipeline/candidate.sv'),str(old),str(bench)]
    ...
    assert 'PASS apply pipeline transactions=90 cancellations=2' in p.stdout
