...
cost class, and implementation owner. Eight operator references—HC expansion,
token embedding, hash-route lookup, ordinary window indices, compressed-dense
indices, DSpark window indices, biased-router top-k, and learned-index top-k—are
implemented and unit-qualified; the other 35 reference kinds and all 43
service-engine/RTL kinds remain pending. The DSpark noise-token constructor is
also implemented, but its combined embed/expand graph operator remains pending
integration.
