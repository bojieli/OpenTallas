...
cost class, and implementation owner. Thirteen operator references—dense FP8
linear, target-hidden HC capture, HC expansion, token embedding, hash-route
lookup, ordinary window indices, compressed-dense indices, DSpark window
indices, DSpark noise embedding/HC expansion, biased-router top-k, learned-index
top-k, routed-weight normalization, and expert dispatch—are implemented and
unit-qualified; the other 30 reference kinds and all 43 service-engine/RTL kinds
remain pending.
