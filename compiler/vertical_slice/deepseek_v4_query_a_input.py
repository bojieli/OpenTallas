"""Create-once composer for causally verified DeepSeek V4 Query-A input.

The independent checker derives the exact byte-identical payload, canonical
request manifest, and external composition report.  This module materializes
those objects privately, invokes the checker against the private objects, and
publishes without replacement.  The report is published first; if request
directory publication loses a race, only the report inode owned by this
transaction is rolled back.

Publication is atomic within each caller-trusted parent.  This layer does not
claim crash durability without a filesystem transaction spanning both parents.
"""
...
class DeepSeekV4QueryAInputBuildError(RuntimeError):
    """Raised when a causally verified Query-A request cannot be published."""
...
def _require_secure_file_operations() -> None:
    ...
    if (
        ...
        raise DeepSeekV4QueryAInputBuildError(
            "platform lacks race-resistant Query-A input publication operations"
        )
...
def _specific_path(value: Path, label: str) -> Path:
    ...
    if not path.name or path.name in {".", ".."} or path == Path(path.anchor):
        raise DeepSeekV4QueryAInputBuildError(f"{label} must name a specific path")
    return path


def _safe_relative(value: str, label: str) -> str:
    if type(value) is not str or not value or "\\" in value or "\x00" in value:
        raise DeepSeekV4QueryAInputBuildError(f"{label} is not a safe relative path")
    relative = Path(value)
    ...
    if relative.as_posix() != value:
        raise DeepSeekV4QueryAInputBuildError(f"{label} is not canonical POSIX")
    return value
...
def _open_root(stack: ExitStack, root: Path, label: str) -> int:
    ...
    if not stat.S_ISDIR(os.fstat(descriptor).st_mode):
        raise DeepSeekV4QueryAInputBuildError(f"{label} is not a directory")
    return descriptor
...
def _verify_root_binding(root: Path, descriptor: int, label: str) -> None:
    ...
    try:
        ...
        if not stat.S_ISDIR(current.st_mode) or (current.st_dev, current.st_ino) != (
            ...
            raise DeepSeekV4QueryAInputBuildError(f"{label} was replaced")
    finally:
...
def _create_private_directory(
    ...
    *,
    ...
    flags = os.O_RDONLY | os.O_CLOEXEC | os.O_DIRECTORY | os.O_NOFOLLOW
    for _ in range(128):
        name = f".query-a-input.tmp-{secrets.token_hex(16)}"
        try:
            ...
            raise DeepSeekV4QueryAInputBuildError(
                f"cannot create private Query-A request directory: {exc}"
            ) from exc
        try:
            ...
            try:
                ...
                pass
            raise DeepSeekV4QueryAInputBuildError(
                f"cannot open private Query-A request directory: {exc}"
            ) from exc
        stack.callback(os.close, descriptor)
        if not stat.S_ISDIR(os.fstat(descriptor).st_mode):
            raise DeepSeekV4QueryAInputBuildError(
                "private Query-A request path is not a directory"
            )
        return parent / name, name, descriptor
    raise DeepSeekV4QueryAInputBuildError(
        "cannot reserve a collision-free Query-A request directory"
    )
...
def _create_private_report(
    ...
    *,
    ...
    for _ in range(128):
        name = f".query-a-input-report.tmp-{secrets.token_hex(16)}"
        try:
            descriptor = os.open(
                name,
            ...
            raise DeepSeekV4QueryAInputBuildError(
                f"cannot create private Query-A composition report: {exc}"
            ) from exc
        stack.callback(os.close, descriptor)
        try:
            offset = 0
            while offset < len(payload):
                ...
                if written <= 0:
                    raise DeepSeekV4QueryAInputBuildError(
                        "private Query-A composition report ended while being written"
                    )
                offset += written
            os.fsync(descriptor)
            metadata = os.fstat(descriptor)
            if not stat.S_ISREG(metadata.st_mode) or metadata.st_size != len(payload):
                raise DeepSeekV4QueryAInputBuildError(
                    "private Query-A composition report size differs"
                )
            ...
            try:
                ...
                raise DeepSeekV4QueryAInputBuildError(
                    "cannot clean a failed private Query-A composition report: "
                    f"{cleanup_exc}"
                ) from exc
            if isinstance(exc, DeepSeekV4QueryAInputBuildError):
                raise
            raise DeepSeekV4QueryAInputBuildError(
                f"cannot write private Query-A composition report: {exc}"
            ) from exc
        return parent / name, name, descriptor, _fingerprint(metadata)
    raise DeepSeekV4QueryAInputBuildError(
        "cannot reserve a collision-free Query-A composition report"
    )
...
def _cleanup_private_directory(
    *,
    ...
    try:
        ...
        raise DeepSeekV4QueryAInputBuildError(
            f"cannot reopen temporary Query-A request for cleanup: {exc}"
        ) from exc
    try:
        current = os.fstat(current_descriptor)
        if not stat.S_ISDIR(current.st_mode) or (current.st_dev, current.st_ino) != (
            ...
            raise DeepSeekV4QueryAInputBuildError(
                "refusing to clean a replaced temporary Query-A request"
            )
        visited = 0
        ...
        def remove_contents(directory_descriptor: int, depth: int) -> None:
            ...
            if depth > _MAX_TREE_DEPTH:
                raise DeepSeekV4QueryAInputBuildError(
                    "temporary Query-A request cleanup exceeds its depth bound"
                )
            for child_name in os.listdir(directory_descriptor):
                visited += 1
                if visited > _MAX_CLEANUP_ENTRIES:
                    raise DeepSeekV4QueryAInputBuildError(
                        "temporary Query-A request cleanup exceeds its entry bound"
                    )
                metadata = os.stat(
                    child_name,
                    dir_fd=directory_descriptor,
        ...
        raise DeepSeekV4QueryAInputBuildError(
            f"cannot clean temporary Query-A request: {exc}"
        ) from exc
    finally:
        os.close(current_descriptor)
    os.rmdir(name, dir_fd=parent_descriptor)
...
def _unlink_if_held(
    *,
    ...
    if (
        ...
        raise DeepSeekV4QueryAInputBuildError(
            "refusing to remove a replaced Query-A composition report"
        )
    os.unlink(name, dir_fd=parent_descriptor)
...
def _publish_create_once(
    *,
    ...
    if renameat2 is None:  # pragma: no cover - checked before private creation
        raise DeepSeekV4QueryAInputBuildError(
            "platform lacks atomic rename-without-replacement support"
        )
    renameat2.argtypes = [
    ...
    if error in {errno.EEXIST, errno.ENOTEMPTY}:
        raise DeepSeekV4QueryAInputBuildError(f"output already exists: {destination}")
    raise DeepSeekV4QueryAInputBuildError(
        f"cannot publish Query-A input atomically: {os.strerror(error)}"
    )


def build_deepseek_v4_query_a_input_request(
    *,
    hc_result_root: Path,
    ...
    """Derive, independently recheck, and publish one Query-A request once."""

    _require_secure_file_operations()
    output = _specific_path(output, "Query-A request output")
    report_output = _specific_path(report_output, "Query-A composition report output")
    try:
    ...
    if os.path.lexists(output):
        raise DeepSeekV4QueryAInputBuildError(f"output already exists: {output}")
    ...
    if shutil.disk_usage(output.parent).free < _MINIMUM_FREE_BYTES:
        raise DeepSeekV4QueryAInputBuildError(
            "Query-A request output lacks its bounded free-space reserve"
        )
    if shutil.disk_usage(report_output.parent).free < _MINIMUM_FREE_BYTES:
        raise DeepSeekV4QueryAInputBuildError(
            "Query-A report output lacks its bounded free-space reserve"
        )
    try:
        ...
        raise DeepSeekV4QueryAInputBuildError(
            f"cannot derive a verified Query-A input request: {exc}"
        ) from exc

    with ExitStack() as stack:
        output_parent_descriptor = _open_root(
            stack, output.parent, "Query-A request output parent"
        )
        report_parent_descriptor = _open_root(
            stack, report_output.parent, "Query-A report output parent"
        )
        _verify_root_binding(
            output.parent,
            output_parent_descriptor,
            "Query-A request output parent",
        )
        _verify_root_binding(
            report_output.parent,
            report_parent_descriptor,
            "Query-A report output parent",
        )
        ...
        try:
            ...
            try:
                integrity = verify_deepseek_v4_query_a_input_composition(
                    hc_result_root=hc_result_root,
                    executable_deployment_root=executable_deployment_root,
                    executable_application_root=executable_application_root,
                    snapshot=snapshot,
                ...
                raise DeepSeekV4QueryAInputBuildError(
                    f"independent Query-A input verification failed: {exc}"
                ) from exc
            ...
            _verify_root_binding(
                temporary, temporary_descriptor, "temporary Query-A request"
            )
            _verify_root_binding(
                output.parent,
                output_parent_descriptor,
            ...
            _verify_root_binding(
                output, temporary_descriptor, "published Query-A request"
            )
            ...
            if not stat.S_ISREG(current_report.st_mode) or (
                ...
                raise DeepSeekV4QueryAInputBuildError(
                    "published Query-A composition report was replaced"
                )
