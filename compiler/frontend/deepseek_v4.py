...
class _TensorBuilder:
    """Accumulates one release's tensor specs under its own quantization blocks.

    ``fp8_block`` is the released ``quantization_config.weight_block_size``, so
    the FP8 scale geometry is a read of the config rather than a constant here:
    DeepSeek-V4 publishes ``[128, 128]`` and DeepSeek-V4.1 publishes ``[32, 32]``,
    and both are generated by the same code.  The MXFP4 block below is not a
    config field -- it is the 32-element micro-scaling block the format itself
    defines, unchanged across both releases' differing FP8 blocks.
    """

    #: Elements per E8M0 scale in MXFP4, fixed by the format, not by a model.
    MXFP4_BLOCK_ELEMENTS = 32

    def __init__(self, fp8_block: tuple[int, int]) -> None:
        rows, columns = fp8_block
        if rows <= 0 or columns <= 0:
            raise DeepSeekV4AdapterError(
                f"released FP8 weight_block_size must be positive: {fp8_block!r}"
            )
        self.fp8_block_rows = rows
        self.fp8_block_columns = columns
    ...
    def fp8_linear(
        ...
        *,
        ...
        self.add(
            ...
            "F8_E8M0",
            (
                math.ceil(out_features / self.fp8_block_rows),
                math.ceil(in_features / self.fp8_block_columns),
            ),
            f"{role}.scale",
            scope=scope,
            layer=layer,
    ...
    def fp4_linear(
        ...
        *,
        ...
        block = self.MXFP4_BLOCK_ELEMENTS
...
def build_official_tensor_specs(
    ...
    validate_official_config(config, release)
    builder = _TensorBuilder(
        tuple(release.quantization_config["weight_block_size"])
    )
    dim = config["hidden_size"]
