...
_OPERATORS = (
    ...
    _op(
        ...
        "Clamp, round-scale, quantize, and dequantize BF16 activation blocks through FP8 E4M3FN.",
        "CONVERT",
        "numeric.fp8_qdq",
    ),
    _op(
        ...
        "Power-of-two scale, clamp, and quantize/dequantize activation blocks through FP4 E2M1.",
        "CONVERT",
        "numeric.fp4_qdq",
    ),
    _op(
        ...
        "Apply normalized Hadamard rotation before indexer FP4 quantization.",
        "CONVERT",
        "vector.hadamard",
    ),
    _op(
    ...
    _op(
        ...
        "inference/model.py:Gate.forward",
        "Gather unbiased scores, normalize selected weights, and apply routed scaling factor.",
        "EXPERT_WEIGHT",
...
def _add_block_graph(
    ...
    *,
    ...
    (kv,) = graph.add(
        ...
        (kv,),
        phases=normal_phases,
        attributes={"block_size": 64, "dimensions": "non_rope"},
    )
    if dspark:
        (main_kv,) = graph.add(
...
def build_official_graph_contract() -> dict[str, Any]:
    ...
    for layer, ratio in enumerate(config["compress_ratios"][:43]):
        ...
        if layer in config["dspark_target_layer_ids"]:
            (captured,) = graph.add(
                ...
                "TARGET_HIDDEN_CAPTURE",
                (hidden,),
                attributes={"hc_reduce": "mean", "layer": layer},
            )
    ...
    body: dict[str, Any] = {
        ...
        "open_semantic_issues": [
            ...
            {
                "id": "DSV4-SEM-003",
                "issue": "Gumbel-max sampling consumes ambient PyTorch generator state and the release recipe also recommends top_p, which the local generate.py path does not implement.",
                "required_resolution": "Version RNG/counter, temperature, top-p, greedy, and reproducibility semantics at the service boundary.",
                "severity": "blocking",
                "source_anchor": "inference/model.py:sample;inference/generate.py:main;README.md:How to Run Locally",
            },
            {
                "id": "DSV4-SEM-004",
                "issue": "This graph begins at token IDs; official message encoding, tokenizer behavior, output decoding, and message parsing are not graph nodes.",
                "required_resolution": "Implement the pinned encoding/tokenizer host front end and pass all official encoding fixtures.",
                "severity": "blocking",
                "source_anchor": "encoding/encoding_dsv4.py;inference/generate.py:main",
            },
            {
                "id": "DSV4-SEM-005",
                "issue": "TileLang and PyTorch define the current FP4/FP8/BF16 behavior operationally, but the target rounding, subnormal, NaN, infinity, signed-zero, and reduction-order contract is not independently executable.",
                "required_resolution": "Implement and qualify scalar target-precision references before marking any operator executable.",
                "severity": "blocking",
                "source_anchor": "inference/kernel.py:act_quant_kernel;inference/kernel.py:fp4_quant_kernel;inference/kernel.py:fp8_gemm_kernel;inference/kernel.py:fp4_gemm_kernel",
            },
            {
                "id": "DSV4-SEM-006",
                "issue": "The source mutates KV and compressor buffers in place and has no accelerator abort/prepare/commit transaction semantics.",
                "required_resolution": "Specify atomic prepare/commit, rollback, poison, bounds, and session isolation for all mutable state.",
                "severity": "blocking",
                "source_anchor": "inference/model.py:Attention.forward;inference/model.py:Compressor.forward",
            },
            {
                "id": "DSV4-SEM-007",
                "issue": "The official conversion path views packed expert I8 as FP4, dequantizes wo_a FP8 blocks to BF16, removes their scales, shards tensors, and omits duplicate MTP embedding/head records; those transforms are not yet a canonical compiler pass.",
                "required_resolution": "Specify, implement, hash, and inversely verify every raw-checkpoint-to-runtime transformation before image generation.",
                "severity": "blocking",
                "source_anchor": "inference/convert.py:main;inference/convert.py:cast_e2m1fn_to_e4m3fn",
            },
        ],
        "operator_catalog": [
        ...
        "system_scope": {
            ...
            "unresolved": [
                "message encoding and tokenizer",
                "multi-step generation and EOS control",
                "DSpark target verification and speculative acceptance",
                "canonical checkpoint-to-runtime tensor transforms",
                "target-precision reference implementations",
                "service-engine and microcode implementations",
