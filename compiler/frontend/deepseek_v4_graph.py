...
def build_official_graph_contract() -> dict[str, Any]:
    ...
    body: dict[str, Any] = {
        ...
        "open_semantic_issues": [
            ...
            {
                "id": "DSV4-SEM-005",
                "issue": "Independent references now define E2M1, E8M0, E4M3FN, BF16, activation microscaling, ordered binary32 accumulation, official 32-value routed and 128-value dense block dots, complete routed-MXFP4 and shared-FP8 SwiGLU, complete dense FP8 and BF16 linear, binary32 router-score, sqrt-softplus router activation, compressor, and DSpark confidence projections, causal raw compressor-state updates, deterministic compressor pooling, explicit pooled-binary32 to BF16 conversion, session-bound compressed-KV prefix commit and valid-prefix view, learned sparse-index scoring, block-64 sparse attention with learned sink and explicit mutable-KV traffic, base/YaRN RoPE application and inverse, weighted RMS normalization, unweighted BF16 head RMS normalization, final HC-head reduction, KV FP8 and indexer FP4 QDQ, indexer Hadamard semantics, and fail-closed greedy/target-adapted sampling, comprising thirty-eight complete matrix/vector/normalization/structural/index/lookup/selection/routing/attention/conversion/state/control operator kinds. Eight matrix, MHC, KV-view/window-state, output, and speculative-control operator kinds remain pending even though every kind has a source, lowering, and cost-class ledger entry.",
