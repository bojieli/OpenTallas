...
_OPERATORS = (
    ...
    _op(
        "KV_WINDOW_WRITE",
        "inference/model.py:Attention.forward",
        "Atomically write finite BF16 KV into session-bound circular-window state, retire removed lanes, and expose only the chronological committed causal view under exact version authority.",
        "KV_COMMIT",
...
def build_official_graph_contract() -> dict[str, Any]:
    ...
    body: dict[str, Any] = {
        ...
        "open_semantic_issues": [
            ...
            {
                "id": "DSV4-SEM-006",
                "issue": "Independent references now define immutable causal raw compressor-state updates, session-bound circular-window KV mutation/retirement/chronological views, and session-bound compressed-KV writes/valid-prefix views, including fresh-session reset, exact decode cursor, lane retirement, tombstones, monotonic versions, and stale-capacity exclusion. The generation controller still has no operator-complete executor that invokes those transactions together and binds the resulting committed-state hash to each processed span.",
