"""Canonical logical schedule for the exact DeepSeek V4 HC_PRE fragment.

The schedule records instruction order, register causality, and bounded resource
identity for the already frozen ``HC_PRE; COMPLETE`` microcode fragment.  A slot
is only an ordinal in the semantic program.  It is not a cycle, pipeline stage,
physical placement, bandwidth allocation, or execution observation.
"""
...
class DeepSeekV4HCPreScheduleError(ValueError):
    """Raised when the exact HC_PRE fragment cannot form its logical schedule."""
...
def _register_table(
    ...
    if len(minimum_specs) != len(maximum_specs):  # pragma: no cover - frozen contract
        raise DeepSeekV4HCPreScheduleError("HC_PRE tensor bounds have different ranks")
    ...
    for slot, instruction in enumerate(instructions):
        ...
        for register_id in _destination_ids(instruction):
            if register_id in producers:  # pragma: no cover - frozen contract
                raise DeepSeekV4HCPreScheduleError(
                    f"register {register_id} has more than one producer"
                )
            producers[register_id] = slot

    records: list[dict[str, Any]] = []
    for lower, upper in zip(minimum_specs, maximum_specs):
        if (
...
def _slots(
    ...
    for index, instruction in enumerate(instructions):
        ...
        raw_identity = _instruction_identity(instruction)
        slots.append(
            {
                ...
                "instruction_sha256": _sha256(raw_identity),
...
def build_deepseek_v4_hc_pre_logical_schedule(
    instructions: tuple[Instruction, ...] | None = None,
) -> dict[str, Any]:
    """Build the sole canonical logical schedule for the exact HC_PRE fragment."""
    ...
    try:
        ...
        raise DeepSeekV4HCPreScheduleError(
            f"HC_PRE logical schedule requires the exact microcode fragment: {exc}"
        ) from exc
    ...
    summary = {
        "checkpoint_parameter_bytes": sum(record["size_bytes"] for record in resources),
        ...
        "hc_pre_count": sum(slot["opcode"] == "HC_PRE" for slot in slots),
