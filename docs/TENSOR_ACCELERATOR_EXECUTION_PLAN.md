...
non-generation boundary. Commit `720cb2a` closes the separately versioned
one-token-prompt, 32-generated-decision target-precision diagnostic session;
...
**Execution-claim decision record `TA-EXEC-2026-08-29`:** “Artifact-only” is a
causality and implementation-path requirement, not a claim about workload
length. It requires the simulator to derive every activation, logit, and state
transition from authenticated compiler artifacts, input tokens, and prior
architectural state, with no framework-model call, compiler-internal execution,
precomputed activation or logit, model-specific service, or whole-model bypass.
Every report must separately state its execution scope. A complete one-request
forward transaction may therefore be artifact-only without being a complete
end-to-end acceptance workload. Unqualified “full execution” is not an admitted
claim; Sections 3.1 and 3.1.1 define the required scope-qualified terms.

**Greedy-selection decision record `TA-DECODE-2026-08-29`:** target-precision
greedy selection returns the lowest token ID whose materialized logit equals the
maximum, matching first-occurrence argmax over vocabulary order. Reports retain
the exact maximum multiplicity. A tie is therefore deterministic evidence to be
matched by the independent reference, not an execution failure or permission to
...
work subsequently closes at its diagnostic target-precision boundary in
Section 1.25 without broadening this one-step claim.

### 1.25 Closed Qwen 32-decision target-precision diagnostic session

Commit `ta-integration@720cb2a` closes the short, stateful target-precision
diagnostic that follows QW-FM4. It does not close `TA-QWEN-4`. A versioned
session manifest binds the immutable deployment, checkpoint lock, tokenizer,
one-token prompt `!` (token ID `0`), deterministic
`greedy_lowest_token_id_argmax`, two EOS IDs, and exactly 32 generated-token
decisions. A distinct dynamic request binds every changing token ID, position,
expected generation and length, predecessor report, phase, and transaction ID;
fixed request v1 remains unchanged. The simulator executes the complete
924,386-command, 617-operation model transaction for every step and carries all
36 KV resources causally through 31 true decode transitions.

The first candidate campaign is classified and documented as failure history,
not admitted as passing evidence. Independent replay matched transactions 0 through 9 and
then stopped at step 10, layer-30 attention operation `node.0519`. Scaled-score
bytes were exact, but probability and attention-output bytes differed. The
target implementation used the host NumPy single-precision exponential, which
returns binary32 `0x3ebc5ab1` for `exp(-1.0)` while the frozen correctly rounded
contract requires `0x3ebc5ab2`. The target kernel was repaired to evaluate the
architectural binary32 input with guard precision before the one architectural
binary32 rounding. Adversarial scalar tests cover this and other one-ulp cases;
the original retained attention qualification remains byte-identical with
report ID `82bd8f8b...a5bd`, so no IR, ABI, capability, or deployment identity
changed.

The clean corrected campaign has these retained identities:

- session ID:
  `aaf9015953e6c27690c3dd097cea0e9285e79743d9ceadbd97fa0080597dcf6e`;
- session-execution ID:
  `9e53f5d79dff7f530652116df3a9196e3a58dd07375a07c2b6f3ae227b83f0f1`;
- session-execution file SHA-256:
  `3bc3efcaca71016decc4eddb4e5c7fa8ea792735e3472cab45f95771cec8cdf2`;
- independent session-reference ID:
  `9f17054b8a78fb52cbe23f5fb11fd16371045fb3f1814e27297a1fdfc7118151`;
- independent-reference file SHA-256:
  `6073550e57c4c577b5527a9dff3cb23057f8e42ea3138aef9d331c86c6dd8480`;
- step-reference-chain SHA-256:
  `8d11c040a5cdbd87e346f949b1dd6cfbb4335e79810126b09e5136e46779c11a`;
  and
- aggregate-counter SHA-256:
  `8c65270b82f3bf86571af7a946e9c4a6c2f2dd0f852fbd844412b2f1690a4dd1`.

The exact generated token IDs are
`[50994, 67, 21, 19, 19, 19, 19, 20, 19, 19, 20, 19, 66, 19, 19, 24, 20, 23, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15]`.
They decode with the pinned tokenizer to
`[](d6444454454c4495800000000000000`. This text is reported exactly; its poor
surface form is not relabeled as proof of official release quality. The
independent target-precision reference authenticates 399 row-major checkpoint
tensors per transaction, uses explicit segmented-K matrices and separately
implemented scalar kernels, carries its own KV state, and matches all 19,744
operation events, 1,152 layer boundaries, 1,152 state transitions, complete
logits and maximum multiplicities, all 32 token decisions, zero saturation,
and 2,240 counter comparisons. All 36 final resources have generation and
length 32.

Two fresh corrected simulator campaigns produce all 66 session, request,
transaction, and aggregate files byte for byte identically: 7,591,673 bytes
with tree-manifest SHA-256
`f0d0bb4726202d15c87f17c4b63a1b93ecf58f56ab4140f3dfcfb8a901aed8f9`.
The retained reference is separate and does not mutate the simulator aggregate,
whose target-reference flag remains false. All 78 strict schemas admit their
authentic retained instances; identity, ordering, omission, overwrite,
predecessor, state-metadata, counter, decode, overclaim, and corruption tests
fail closed. The complete 591-test compiler/runtime campaign passes 582 tests
and skips the same nine explicitly unavailable-source cases. Scoped lint,
bytecode compilation, 169 JSON parses, and diff hygiene pass.

This closure proves complete artifact-only target-precision execution for this
...
### 1.15 Memory-bound control for the authentic Qwen ADD command

QW-RTL-ADD-SRAM-001 wraps the admitted streaming ADD executor with explicit
SRAM request, response, and writeback control. It permits one outstanding read,
fetches the left and right operand in command order, holds each request until
accepted, and retires a finite result only when its two-byte write is accepted.
The module reports exact read and write transaction counts alongside the
existing command, element, and saturation completion fields. A nonfinite or
overflowing arithmetic result retires as a command error without asserting the
write interface.
...
`f872ce6f57ca36a30edf6abccfa2877bbb7234a905fe9e1417d99ea89ee79ec3`.
Icarus and Verilator each check all 8,192 ordered 16-bit reads and 4,096
ordered 16-bit writes, including exact byte addresses, byte enables, payloads,
counter totals, and completion stability. The memory models independently
insert read-request and writeback stalls plus variable response latency. Two
additional error executions each perform the two required operand reads,
retire one faulting element, and perform zero writes.

This closes memory request/writeback control only for the one ADD command. The
campaign SRAM is a behavioral test model, not a characterized or qualified
memory macro. It does not exercise ECC, bank mapping or conflicts, arbitration,
DMA, HBM, multiple outstanding tags, a command sequence, or a complete layer.
It supplies no qualified cycle, timing, area, power, or performance value and
does not close `TA-RTL-6`. The next horizon is a bounded command sequencer with
...
QW-RTL-DMA-ADD-001 introduces `ot_ta_dma_add_sequencer` as the first bounded
multi-command RTL control point. It admits only one command at a time, requires
strictly increasing submitted command indices, gives the active DMA or ADD
engine exclusive ownership of the shared SRAM write port, accumulates
architectural transaction/byte/element counters, and holds one stable program
completion until acknowledged. CRC/decoder errors, executor errors, and
non-monotonic submissions end the bounded program before another command can be
accepted. An explicit `cmd_last` marks the final submitted record; full command-
program header/body CRC and terminal `COMPLETE` semantics remain separate work.
...
The first unchanged record is Qwen command 1, which transfers the authentic
8,192-byte layer-0 input-normalization weight from HBM to SRAM address
2,097,152. The second unchanged record is Qwen command 5,131, the existing
4,096-element residual ADD. For this bounded composition, the DMA destination
is exactly the ADD right-operand address; the expected ADD result is recomputed
by independent scalar and optimized BF16 implementations and has payload hash
`8901268db88a2d0e207c56b01aab6bcd0cdae0c2de2538e7d5a0120c595202ac`.

Icarus and Verilator reconcile two accepted/completed commands, 128 HBM
requests and responses, 8,192 HBM bytes, 512 16-byte DMA writes, 8,192
two-byte ADD reads, 4,096 two-byte ADD writes, 4,096 retired elements, zero
saturations, and 16,384 total SRAM bytes written. The test models independently
insert HBM-request, SRAM-read, and shared-write backpressure. They also prove
three fail-stop paths: a corrupted DMA record causes CRC error 1 with no HBM or
SRAM activity; a failing first HBM response causes error 11 with no admitted
payload or successor reads; and a second non-monotonic submitted index causes
program-order error 12 after the successful DMA but before ADD dispatch.

This is a real ordered shared-memory composition, but not a semantic shortcut.
Both records are authentic members of the frozen program, yet commands 2
through 5,130 are deliberately absent; using the normalization weight as this
ADD operand is not the graph-valid Qwen layer sequence. The schema fixes
`graph_valid_qwen_operation_sequence`, `complete_layer_execution`, and
`ta_rtl_6_closed` to false. The next horizon is the authentic adjacent
command-1 DMA plus command-2 RMSNorm path, followed by additional DMA/tensor/
...
QW-RTL-DMA-MATMUL-001 now implements authentic production commands 3 through
34 without claiming the complete projection. Sixteen alternating
`DMA_HBM_TO_SRAM` and `MATMUL_BF16_TILE` pairs cover all 4,096 K elements for
the first 64-output q-projection block. Each DMA stages 32,768 bytes at SRAM
address 4,194,304. Command 4 carries `MATMUL_INIT`; commands 6 through 32 carry
neither accumulation flag; command 34 carries `MATMUL_FINAL`. Every MATMUL has
shape `1 x 64 x 256`, advances its input by 512 bytes, uses FP32 accumulator
address 5,242,880, and names BF16 auxiliary address 6,291,456.

The contiguous 524,288-byte weight slice is extracted from immutable HBM shard
SHA-256
...
all sixteen constituent tile hashes are retained separately.
...
`fd56b85bf301661c8655ed517928304d25df7d6b3ea3d9be85c021159d61bad8`.
The 4,096-element input is the complete retained command-2 RMSNorm output row
and has hash
`976d6de1a3ed91a066c7efed4354e578edf366a3b51a7e6077d68282981ffa58`.
...
For each non-init command, the engine reloads all 64 FP32 lanes as ordered low-
then-high 16-bit SRAM reads before reading the next input and weight segments.
The optimized NumPy implementation and the independent exact-Fraction scalar
oracle agree after every K tile. The final little-endian accumulator payload
SHA-256 is
`9ca6beb437222939a843d9a12d2fc93440de926cf7e0a5b3af40ce2c63a28f49`.
Command 34 validates and buffers all final conversions before writing the FP32
tile followed by 64 BF16 values. The BF16 payload SHA-256 is
`c485049ad6aa3e7c6e641defacfabee7de0c159cd8d45a65e74409493d17eb5e`;
its saturation count is zero.
...
Both simulators reconcile 8,192
HBM requests/responses, 32,768 16-byte DMA writes, 1,920 accumulator halfword
reads, 4,096 input reads, 262,144 weight reads, 262,144 products, 262,144
ordered additions, 1,024 four-byte accumulator writes, and 64 two-byte BF16
writes under independent backpressure. CRC, HBM-response, command-order,
input-nonfinite, weight-nonfinite, multiplication-overflow,
accumulation-overflow, and reloaded-accumulator-nonfinite cases all fail stop.
A failing command performs zero destination writes; the reload fault also
proves that the already retired prior accumulator tile remains intact.
...
### 1.22 Complete authentic K- and V-projection execution in RTL

QW-RTL-KV-PROJ-001 extends the bounded sequencer across exact adjacent kernel
ranges without weakening their command or kernel-index profiles. Authentic
commands 2,051 through 2,562 execute `node.0003` K projection, and commands
2,563 through 3,074 execute `node.0004` V projection. Each operation contains
256 alternating DMA/MATMUL pairs, 16 output blocks, and all 16 K tiles per
block. Only command 3,074 may carry the terminal marker.
...
The deployed K- and V-weight identities are respectively
...
their combined 16,777,216-byte slice has SHA-256
`a7d864dfda7e9659bcf297728eed39ce59aa85b09d40c1f63e638dc45fc8a10c`.
The builder authenticates the containing immutable HBM shard and streams the
slice only to a temporary simulator input. The retained vector artifact is
1.1 MiB and contains commands, exact accumulators, outputs, and hashes rather
than the raw weight payload.

Both Icarus and Verilator execute all 1,024 commands under independent
backpressure and reconcile 262,144 HBM requests/responses, 1,048,576 DMA
writes, 61,440 accumulator halfword reload reads, 131,072 input reads,
8,388,608 weight reads, products, and ordered additions, 32,768 FP32
accumulator writes, and 2,048 BF16 writes. Every intermediate accumulator tile
matches the oracle; their aggregate SHA-256 is
`a1c2f04da9b9059abf17fb048a6a7ffda68b243b9ec7fe0ac8bebe516ac81f69`.
The complete K and V BF16 outputs match architectural and independent-reference
hashes
...
All conversions are finite with zero saturation.

An early terminal assertion on command 2,051 reports program-order error 12
before any HBM request or SRAM write. Icarus observes 48,506,095 cycles and
Verilator 48,505,442 cycles under their deterministic harness schedules. These
are correlation observations, not characterized latency or performance. The
earlier Q campaign remains immutable and binds its original RTL source hashes;
this K/V campaign binds the current generalized sources while the default
single-block profile retains compatibility evidence.

This closes complete RTL execution and architectural correlation for the K and
V projection graph operations. It does not close complete QKV preparation,
because per-head Q/K RMSNorm and RoPE remain. It also does not execute KV
prepare, attention, state, vector kernels, a complete layer, program
...
correctness result. Its frozen tie rule selects the lowest token ID among all
exact target-precision maxima and records how many maxima were present. Seeded
stochastic sampling is a later qualification profile with its random-number
stream and selection algorithm included in the request identity.

#### 3.1.1 Artifact-only execution versus full end-to-end execution

Artifact-only execution is real, data-bearing execution. The functional or
timing simulator reads the versioned deployment, command stream, HBM image,
request, and prior architectural state; executes the commanded arithmetic and
...
- **artifact-only operator, slice, or layer execution:** only the named bounded
  portion of the model executed;
- **artifact-only full-model transaction:** every operation in one complete
  model-forward request executed, complete logits were produced, and all
  required state effects were either atomically committed or fail-closed;
- **artifact-only short end-to-end generation:** the complete pinned short
  prompt and at least 32 ordinary greedy generated-token decisions executed as
  one causally chained session unless the frozen expected EOS occurred earlier;
  and
- **artifact-only full end-to-end acceptance execution:** the pinned tokenizer
  and input fixture, every token of the exact resident prompt, the complete
  accelerator prefill, all state commits, the frozen ordinary greedy-decode
  campaign, stop/EOS behavior, generated token IDs, and decoded text all
...
followed by at least 32 ordinary greedy generated tokens under the same EOS
rule. Qwen 8,192 and DeepSeek 1,048,576 are separate capacity/qualification
profiles and cannot substitute for those acceptance workloads.

The closed QW-FM4 evidence in Section 1.12 is an authentic artifact-only
...
decision. Commit `720cb2a` separately closes the target-precision short
diagnostic while leaving the exactly 8,000-token fixture, official/golden
acceptance, timing, RTL, physical characterization, DeepSeek, and the governed
comparison open. The release route preserves the following ordered decisions:
