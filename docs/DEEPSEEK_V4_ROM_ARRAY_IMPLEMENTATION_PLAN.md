...
| DRA-S4 | **in flight, first attempt lost** | `tools/run_accelerator_tokens.py --backend rom_deepseek_v4_array` on `TA-DS-CTX-129-1` lowers, publishes and is admitted (1,329 instructions, all 4,159,376 work units verified). The first 32-node functional execution of the 129-token prefill plus 8 generated tokens ran for 2 h 45 min of wall time and 5 h of CPU at 64 GB resident before the session's background job was stopped with it still executing; no record was written. A replacement began at 22:00:44 UTC, but its live command writes to the owning session's temporary `scratchpad/array_ctx129_rerun.json`, not directly to `results/abi3`; completion and preservation of the result must be confirmed before it can be cited. No token claim is made here |
| DRA-Y5 | **attempted, no artifact** | `tools/run_abi3_cycle.py` on the published array deployment, the HBM cluster's own 32-token prefill request, `abi3_cost_rom_array_v2`, `--check-functional-agreement`: admitted and started, then killed by the kernel's OOM killer after 155 min of wall time and 29 h of CPU at 84 GB resident on a 188 GB host (`Out of memory: Killed process ... total-vm:33893752844kB`). The 32 node-private arenas of a deployment whose live state is declared at the IR horizon (1.2 TB per node) are the cause; this is the same replicated-live-state boundary as WP-D2, seen from the simulator's side. No cycle artifact exists for the array |
...
2. **The cycle model cannot host the replicated live state either.** The kill above is not a modelling result; it is the same boundary as (1): 32 nodes times a 1.2 TB declared arena is 33.9 TB of virtual zero objects, and the model's resident working set outgrew the host. Until WP-D2 shards the live state, a cycle run of the array needs either a host with several hundred gigabytes free or a per-node HBM declaration the program actually fits, which the IR's 1,048,576-position horizon forbids.
