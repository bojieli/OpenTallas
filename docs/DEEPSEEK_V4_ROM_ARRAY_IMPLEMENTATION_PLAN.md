...
| DRA-S4 | **closed at the 32-token rung** | `tools/run_accelerator_tokens.py --backend rom_deepseek_v4_array` on `TA-DS-CHAT-1-P32` generates **4** tokens on 32 nodes and every one matches the reference oracle: `[13806, 345, 7472, 55560]` against the oracle's own prefix, `agreement: true`, `first_divergence_index: null`, terminal acceptance `oracle_prefix` accepted with no failed check. The record is `results/abi3/accelerator_tokens/deepseek_v4_flash_rom_array_p32.json`, deployment `a3cc19ba2f7d`, 172 `LINK.COLLECTIVE` reductions and 674,240 expert reductions over the run. That is the first end-to-end numerical agreement for the array: consecutive expert ownership, the node-sharded ROM images and the data-bearing all-reduce are all exercised and all correct. `results/abi3/deepseek_v4_rom_array_p32_context_gate.json` additionally revalidates every relevant source identity and matches all four sparse-attention counters independently on each of 32 nodes and at cluster total. It cost three defects to reach, recorded in the boundaries below. Wall time is 8,745.6 s of simulator throughput and is not a TPOT claim. Still open for the full gate: the `TA-DS-CTX-*` ladder through 200K, `TA-DS-CHAT-1` and `TA-DS-AGENT-1` |
