...
| DRA-S4 | **in flight, first attempt lost** | `tools/run_accelerator_tokens.py --backend rom_deepseek_v4_array` on `TA-DS-CTX-129-1` lowers, publishes and is admitted (1,329 instructions, all 4,159,376 work units verified). The first 32-node functional execution of the 129-token prefill plus 8 generated tokens ran for 2 h 45 min of wall time and 5 h of CPU at 64 GB resident before the session's background job was stopped with it still executing; no record was written. The run has been relaunched detached from the session, writing to `results/abi3/deepseek_v4_rom_array_ta-ds-ctx-129_execution.json` on completion. No token claim is made here |
