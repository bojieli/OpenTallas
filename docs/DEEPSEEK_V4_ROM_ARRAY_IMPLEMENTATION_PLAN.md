...
| DRA-P3 | **closed for current Flash compiler/capacity boundary** | The sparse-corrected 3,956-kernel graph builds to 1,346 instructions and 3,429 descriptors, admits with zero errors and zero `STATE` resources, and is byte-identical on a second clean build. Backend-neutral liveness reduces 1,073,359,364,104 logical activation bytes to an 83-slot, 172,292,907,016-byte physical arena without shortening any IR extent. The array uses 178,281,603,216 of the physical 180,000,000,000 HBM bytes per node. The source-current identities and full capacity proof are retained in `results/abi3/deepseek_v4_activation_liveness_capacity.json`; the older inverse proof still reconstructs all **156,015,698,140** checkpoint payload bytes bit-identically, but must be regenerated before final release because the deployment identity changed |
| DRA-S4 | **in flight on superseded source; first attempt lost** | `tools/run_accelerator_tokens.py --backend rom_deepseek_v4_array` on `TA-DS-CTX-129-1` lowers, publishes and is admitted. The first 32-node functional execution of the 129-token prefill plus 8 generated tokens ran for 2 h 45 min of wall time and 5 h of CPU at 64 GB resident before the session's background job was stopped; no record was written. A replacement began at 22:00:44 UTC and writes to the owning session's temporary `scratchpad/array_ctx129_rerun.json`. It predates the sparse-phase and liveness commits, so even a completed artifact is a diagnostic only and cannot close Gate 1. No token claim is made here |
| DRA-Y5 | **historical attempt failed; source-current retry open** | The old `tools/run_abi3_cycle.py` attempt was killed by the kernel OOM killer after 155 min of wall time and 29 h of CPU at 84 GB resident and wrote no cycle artifact. That deployment gave every logical activation a separate object and declared 1.2 TB per node. The source-current liveness build instead fits the physical 180 GB per-node boundary, so the historical OOM is not evidence about its host-resident behavior. A fresh run is required, and any resulting cycles remain ineligible for TPOT until the same execution passes exact token correctness |
