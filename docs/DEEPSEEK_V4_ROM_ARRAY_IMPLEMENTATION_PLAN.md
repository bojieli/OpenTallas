...
| DRA-P3 | **closed for current Flash compiler/capacity boundary** | The sparse-corrected 3,956-kernel graph builds to 1,344 instructions and 3,165 descriptors, admits with zero errors and zero `STATE` resources, and is byte-identical on a second clean build. Backend-neutral liveness reduces 1,073,359,364,104 logical activation bytes to an 83-slot, 172,292,907,016-byte physical arena without shortening any IR extent. The array uses 178,281,603,216 of the physical 180,000,000,000 HBM bytes per node. The source-current identities and full capacity proof are retained in `results/abi3/deepseek_v4_activation_liveness_capacity.json`. Against this deployment identity the independent schedule checker passes **140** checks <!-- figure: 140 src="results/abi3/rom_schedule_checks.json#cases[case=deepseek-v4-flash-rom-array-32].passed_check_count" name="array independent ROM schedule checks passed" --> over **1,344** instructions <!-- figure: 1,344 src="results/abi3/rom_schedule_checks.json#cases[case=deepseek-v4-flash-rom-array-32].actual.instructions" name="array ROM instructions" --> and **3,159** descriptors <!-- figure: 3,165 src="results/abi3/rom_schedule_checks.json#cases[case=deepseek-v4-flash-rom-array-32].actual.descriptors" name="array ROM descriptors" -->, and the regenerated inverse proof reconstructs all **156,015,698,140** checkpoint payload bytes <!-- figure: 156,015,698,140 src="results/abi3/deepseek_v4_rom_array_inverse.json#report.payload_bytes" name="array inverse payload bytes" --> bit-identically from the node-sharded images |
| DRA-S4 | **closed at 32-token/four-output diagnostic; ladder open** | The fixed `rom_deepseek_v4_array` lowering completes `TA-DS-CHAT-1-P32` and produces four oracle-identical legal tokens, with all 32 per-node sparse-attention counter maps and the cluster totals independently reconciled in `results/abi3/deepseek_v4_rom_array_p32_context_gate.json`. Earlier superseded 129-token attempts remain no-result/refusal history and are not promoted. The 129/160/256/2,052/200,000 context ladder, natural full chat, deterministic agentic case, and EOS-or-256 production horizon remain open |
...
| DRA-X6 | **array-versus-HBM built; the two wafer pairs wait on one rerun** | All three targets execute `TA-DS-CHAT-1-P32` on one source and generate the same four oracle-matching tokens. `results/abi3/comparison_deepseek_rom_array_vs_hbm.json` is the first governed comparison the array target was built to make: tokens identical, so the gate admits it as a performance comparison rather than a divergence analysis, with 30 differing counters. Both contracts are written and all three validate against the amended schema; the report builder admits a ROM-versus-ROM pair and carries the declared-silicon block. The array backend's governed source boundary is registered (its four lowering sources), without which its record was not promotable. The wafer's record was produced minutes before the last lowering edit, so the source-currency gate refuses it and the wafer-versus-HBM and wafer-versus-array pairs wait on the rerun in flight |
...
### What the executed comparison shows, and the trade it prices

`results/abi3/comparison_deepseek_rom_array_vs_hbm.json` is the array against
the 32-node HBM cluster on the same workload, the same graph and the same four
tokens. The counters price the storage-class choice in both directions:

| Counter | ROM array | HBM cluster |
|---|---:|---:|
| `rom.bytes_read` | 1,167,163,159,424 | 0 |
| `hbm.bytes_read` | 679,051,886,048 | 1,641,140,792,528 |
| `link.bytes_sent` | 537,303,449,600 | 787,342,241,792 |
| `tensor.multiplications` | 7,753,931,157,504 | 3,495,509,618,688 |

The array moves **2.42x less through HBM** and **1.47x less over the fabric**,
which is the point of putting weights in ROM. It also performs **2.22x the
multiplications**, and that number is not noise: the deployment declares
`dense_replication_factor: 32`, so every node recomputes the whole dense path
instead of taking a shard of it. Solving `(32d + r)/(d + r) = 2.2183` gives
`d/r = 0.0409`, so dense is 3.9% of the cluster's total multiplication work and
replicating it 32 times accounts for the ratio exactly. Nothing else needs to
be invoked.

That is the price of expert-parallel ownership without column-sharded dense
compute, and it is the same boundary WP-D2 names for ROM bytes, now with an
executed figure for compute as well: 2.76x the unique checkpoint in physical
ROM, and 2.22x the arithmetic. A reader comparing the two targets should hold
both facts at once, because the array's HBM saving is real and so is what it
costs.
