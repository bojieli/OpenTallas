...
| GEN-DSV4-COMPRESSOR | `results/model-execution/deepseek-v4-compressor-audit.json` and `docs/DEEPSEEK_V4_COMPRESSOR_EVIDENCE.md` | exact target references, checkpoint payload audit, and bounded official-method execution | All 62 released FP32 APE tensors contain 1,418,240 finite values and 5,672,960 bytes with aggregate SHA-256 `ad6333d91c83b72c3fc426ea712b91446ef4020eac1dc39a299d9e56ab288ea4`. The unmodified pinned `Compressor.forward` and `overlap_transform` matched all 1,024 BF16 outputs on an APE-cancelled corpus on CPU and SM120; both differed from the deterministic target at one of 1,024 outputs by one BF16 code on a broad exact-eighth corpus. Atomic raw-state, FP32-to-BF16, and compressed-cache references expose logical traffic only; they do not establish service/RTL execution, physical bytes, cycles, PPA, or GPU advantage. |
