...
| per-MAC period | — | 4.4 ns predicted vs baseline 104.1 ns (`results/physical_abi3/asap7/matmul_bf16_sram_engine/pnr.json`) | 28 ns predicted vs 60 ns class |
| per-MAC area | — | ~4,100 cells x 0.12 µm² ≈ 490 µm² std-cell, ≈ 1,370 µm² routed at 0.36 utilisation vs baseline 48,274 µm² per MAC/cycle | cell count only |
...
| --- | --- | ---: | --- | --- |
| LQ8 | 8 format-scaled lanes (§4.2) + stream control | ~41k | T1 | — |
| LQ16 | 16 lanes | ~75k | T2 (gated) | — |
| T64 | 8 x LQ8 (or 4 x LQ16) + tile stream sequencer, unpacker, staging control (~10k) | ~340k | BLOCKS assembly of hardened LQs | staging 2 x 16 KB (vehicle 2 x 2 KB) |
...
| CP-FE | fetch/decode (~2k, seed `ot_a3_instruction_decoder`), loop stack (~6k), symbol file (~2k), predicate unit + cache (~5k), scoreboard 2,048 x 3 with four read ports (~31k), control (~5k) | ~51k | T1 | program store (vehicle 4 KiB) |
...
flowchart LR
  ...
  L1 --> L1CP["L1-CP control plane<br/>595,020 checks, engines active, randomised run-ahead"]
