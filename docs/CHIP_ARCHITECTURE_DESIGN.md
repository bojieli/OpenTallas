...
| T64 | 8 × LQ8 + sequencer/unpacker/staging | not measured; lane hierarchy exceeds flat-flow budget | hierarchical only, pilot required | 2 × 16 KiB staging |
...
**What the gate therefore accepts.** The deployment ships a *decode* entrypoint as its own case, so running decode from a supplied KV cache is the shipped program's own path and not a synthetic construct: the three generated tokens execute in the integrated RTL against the real checkpoint, and the 16-token prefill KV cache and position-16 state are supplied as golden-model images bound by SHA-256 in the record. The evaluator was tightened to match before any artifact existed (`e91ed38`).
