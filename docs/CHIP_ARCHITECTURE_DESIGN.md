...
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Qwen3-8B (ROM = HBM program) | walk (executed artifact: 2,105 retired, `results/abi3/accelerator_tokens/qwen3_8b_rom_chat1.json`; the walk's 2,104 omits the COMPLETE retirement) | 2,105 | 691 | 1,413 | 1 | 0 | 2,143 | **5.6 µs** | 0.04 µs | 74 µs -> **170,031 cycles** <!-- figure: 170031 src="results/rtl/abi3_deployment_campaign.json#control_plane_cycles.per_simulator[simulator=iverilog].cases[index=1].rtl_transaction_cycles" tol="exact" name="Qwen decode control-plane cycles, Icarus" --> = **170.0 µs** (Verilator 170,099) |
| Qwen3-8B on CLUSTER_N x4 (re-lowered, +72 LINK.COLLECTIVE SUM, +72 wait sets) | derived | ~2,250 | 763 | 1,485 | 1 | 0 | ~2,290 | ~6.0 µs | — | — |
| Flash ROM wafer | 200K walk / campaign context | 12,184 / 12,929 | 3,807 / 4,490 | 8,377 / 8,439 | 3 | 1,853 | 13,221 | **33.9 / 36.8 µs** | 3 x ≥ 40 cycles | 491 µs -> **1,202,389 cycles** <!-- figure: 1202389 src="results/rtl/abi3_deployment_campaign.json#control_plane_cycles.per_simulator[simulator=iverilog].cases[index=5].rtl_transaction_cycles" tol="exact" name="Flash ROM wafer decode control-plane cycles, Icarus" --> = **1,202.4 µs** (Verilator 1,202,802) |
...
| Flash HBM cluster-32 | walk | 11,662 | 4,659 | 7,003 | 2,042 | 1,983 | 14,511 | **35.0 µs occupancy + ≥ 82 µs of drains = ≥ 117 µs** | 2,042 x ≥ 40 cycles (up to the longest outstanding MATMUL, 512–1,024 cycles) | 478 µs -> **1,020,870 cycles** <!-- figure: 1020870 src="results/rtl/abi3_deployment_campaign.json#control_plane_cycles.per_simulator[simulator=iverilog].cases[index=7].rtl_transaction_cycles" tol="exact" name="Flash HBM cluster decode control-plane cycles, Icarus" --> = **1,020.9 µs** (Verilator 1,022,500) |
