...
**(a) The boundary is not the binding constraint.** `blocked_only_by_the_boundary` is **empty on both stores**: every one of the 11 missing layer operators is *independently* blocked by a placement-role shortfall, so clearing PC 32 would unblock none of them. The bridge's placement surface is prefix-shaped — `mapped_family_operand` needs 10 distinct objects against 8 `cfg_map_object_N` pins and `matmul_weight` needs 7 against 3; `matmul_input`, `rms_input` and `rms_weight` have a single base **not keyed by object at all**, so they need an addressing-model change rather than more pins; and the sixth role is not a width problem in any form — the layer writes 19 intermediates into 10 buffers and **reuses 4**, while the vehicle writes every result at `cfg_output_base + cursor`, so a reused object is handed two addresses and widening pins leaves that untouched.
