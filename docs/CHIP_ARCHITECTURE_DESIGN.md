...
24. **The re-freeze split the chain, and the split is mine (opened 2026-09-06).** Gate G3's Qwen budget was re-frozen on `ROM-N5-native-SRAMKV-array-pipeline-x5-romfill` because the superseded point could not fail on the thesis — setting mask-ROM read time to zero left its step unchanged to six figures, the budget being 68.2% NVLink, 29.2% KV read, 2.7% fixed latency and **0.0% mask ROM**. The re-freeze is right and it is **half done**: `tools/derive_cycle_machine.py` still carries `DEFAULT_ROM_DESIGN = "Qwen3-8B/ROM-N5-native-HBMKV-array-tensor-x4"`, and `results/derived/qwen3_n5_design_target_reconciliation.json` — the artifact gate C3 passes on — is bound to that same superseded point. So **G3's budget and G4's calibration now describe different machines**, which is precisely the regime mismatch rule R10 exists to prevent, committed by the hand that wrote the rule. Nothing may be composed across G3 and G4 until it is closed. Acceptance: the derivation moves to the re-frozen point; the fifteen machine pairs, the reconciliation and the calibration are re-run from it; C1, C2, C3 and C4 are re-read on the new evidence and whatever they then say is reported, including if one of them stops passing.
