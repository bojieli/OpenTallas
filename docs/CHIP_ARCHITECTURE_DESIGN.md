...
17. **The certificate chain is closed, and the shipped deployments are the pre-AM-E9 ones (opened 2026-09-05 by the G1 scout, reproduced here).** `tools/build_abi3_deployment_rtl_vectors.py::certified_deployment_identity` refuses all four RTL targets, so no vector set can be rebuilt: both ROM targets because `compiler/backends/rom/common/check.py` moved under AM-E9 v2, both HBM targets because `runtime/abi3/constants.py` grew from 14,042 to 14,732 bytes. This is the governance working, not failing — but it means the repository is mid-migration, with `build/abi3/qwen3-8b-{rom,hbm-tokens}` (pre-AM-E9) still shipped while `-e9` carries the fixed lowering that gate C2 accepts. The resolution is a **promotion**, and it must be done once, as one act: make the `-e9` pair the shipped pair, regenerate the four certificates against it, rebuild the RTL vector sets, re-run every campaign whose record binds them, and reconcile every pinned count and prose figure. Section 7.3 anticipated this — "scheduled for the moment the RTL needs a re-lowered deployment, so the re-run of every source-bound record is paid once" — and that moment is now. Note what the promotion may reveal rather than hide: the new checker adds a `staging_bank_mask` rule, so a pre-AM-E9 deployment may legitimately fail it, and if it does, that failure is the evidence that the shipped pair had to be replaced, not a defect in the check. Sequencing: after gate C2's re-derivation and after the asynchronous front end's campaign regeneration, both of which read the same trees.
