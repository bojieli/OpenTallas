...
tile scheduling and standalone certifying nonpositive-exp/direct-sigmoid RTL,
...
projection/RMS and stable-softmax arithmetic, scheduler integration through a
layer, token selection, and TPOT remain open. Standalone CR32 nonpositive exp
and direct sigmoid now pass 169,962 exact checks on both simulators, but the
checkpoint-reachable T=512 input corpus and descriptor integration remain open.
