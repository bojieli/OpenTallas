...
**Last reconciled:** 2026-09-03 — froze the simple ABI 3.0 live-buffer profile,
...
control campaigns, made the exact-200K Gate-B launcher fail closed, and extended
the source-bound shipped-program engine-prefix campaign through embedding
lookup.
...
123/123 checks <!-- figure: 123 src="results/abi3/rom_schedule_checks.json#cases[case=deepseek-v4-flash-rom-wafer].passed_check_count" name="current DeepSeek ROM checks, checkpoint" --> at 1,312 instructions <!-- figure: 1312 src="results/abi3/rom_schedule_checks.json#cases[case=deepseek-v4-flash-rom-wafer].actual.instructions" name="current DeepSeek ROM instructions, checkpoint" --> and 3,841 descriptors <!-- figure: 3841 src="results/abi3/rom_schedule_checks.json#cases[case=deepseek-v4-flash-rom-wafer].actual.descriptors" name="current DeepSeek ROM descriptors, checkpoint" -->. The shipped RTL
...
its exact BF16 `TENSOR.EMBED_LOOKUP` launch count is **4**. It checks 17,408
result words through 40 resolved views with 50,427 checks per simulator. The
legal token ID 0 embedding is a
bounded synthetic probe, not a decoded model token. Qwen next traps at
`VECTOR.RMS_NORM` PC 8; DeepSeek next traps at `DMA.TRANSFER` PC 10. This is
not a full token-producing RTL system.
...
**Remaining-work order:** (1) monitor and strictly validate the already-running
Qwen W10.1 HBM natural capture A; (2) launch independent capture B only after A
passes and proves resource headroom, then complete the Qwen ROM/HBM workload
matrix; (3) execute DeepSeek `--gate-b-production` through EOS or 256 when
resources permit, including both full-byte identity boundaries; (4) extend RTL
next through Qwen RMS norm and DeepSeek transfer, then across every required
operator, memory/link service, selection, append, and EOS path; (5) pass short
integrated-token diagnostics, then run the wafer-ROM and exact-32-node HBM
accelerators at exact 200K; (6) complete W8.3/W8.4/W8.5/W9.4 and the four-target
SKY130/ASAP7 characterized rerun in W9.5; and (7) close W11.2 only when its
...
- [x] W4.6 Qwen single-node deployment — `make abi3-hbm-qwen-deployment`
  ...
  **215** descriptors <!-- figure: 215 src="results/abi3/hbm_qwen_deployment_certificate.json#cases[case=qwen3-hbm-single-chip].actual.descriptors" name="Qwen HBM deployment descriptors, W4.6" -->,
...
- [x] W5.3 DeepSeek wafer-scale reticle/tile ROM placement, on-wafer fabric,
  ...
  certificate admits **1,312** instructions <!-- figure: 1312 src="results/abi3/rom_schedule_checks.json#cases[case=deepseek-v4-flash-rom-wafer].actual.instructions" name="current DeepSeek ROM instructions, W5.3" -->,
  **3,841** descriptors <!-- figure: 3841 src="results/abi3/rom_schedule_checks.json#cases[case=deepseek-v4-flash-rom-wafer].actual.descriptors" name="current DeepSeek ROM descriptors, W5.3" -->,
  **439** schedules <!-- figure: 439 src="results/abi3/rom_schedule_checks.json#cases[case=deepseek-v4-flash-rom-wafer].actual.schedules" name="current DeepSeek ROM schedules, W5.3" --> and **18** on-wafer communication records <!-- figure: 18 src="results/abi3/rom_schedule_checks.json#cases[case=deepseek-v4-flash-rom-wafer].actual.communications" name="current DeepSeek ROM communications, W5.3" -->,
...
- [x] W5.5 Independent schedule checker —
  ...
  while DeepSeek passes **123/123** checks <!-- figure: 123 src="results/abi3/rom_schedule_checks.json#cases[case=deepseek-v4-flash-rom-wafer].passed_check_count" name="DeepSeek independent ROM schedule checks passed" --> over 1,312 instructions, 439 schedules and
...
Verilator each pass 50,427 checks over four cases, six real dense FP32
`DMA.GATHER` launches plus four exact BF16 `TENSOR.EMBED_LOOKUP` launches, 40
resolved views, and 17,408 checked words: 1,024 authenticated generated-RoPE
FP32 words plus 16,384 BF16 embedding codes. The embedding reads exactly four
selected 8 KiB checkpoint rows and binds each range, segment declaration,
checkpoint revision, shard, and deployment identity. It does not rehash the
...
trap at `VECTOR.RMS_NORM` PC 8 (descriptor 50 ROM, 64 HBM); DeepSeek next traps
at `DMA.TRANSFER` PC 10 (descriptor 363 ROM, 532 HBM). Neither unsupported
operation retires or publishes its signal, and the campaign records zero
post-fault writes and zero compatibility-state activity under `STATE_COMPAT=0`.
...
- [x] W8.9 Express every sequencer bound where a deployment can be refused for
  exceeding it — the event scoreboard is **1,024** entries <!-- figure: 1024 src="results/rtl/abi3_deployment_campaign.json#rtl_implementation_bounds.values.A3_EVENT_COUNT" name="RTL event-ID storage bound, W8.9" --> and all active
...
- [~] W11.3 Evidence ledger: every number traced to executed counters or labeled
  ...
  **2,718** candidates remain explicitly `untriaged`. <!-- figure: 2718 src="results/abi3/prose_figure_coverage.json#totals.unbound_triage.untriaged" name="W11.3 candidates still awaiting triage" -->
