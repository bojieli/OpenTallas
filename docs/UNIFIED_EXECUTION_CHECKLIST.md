...
**Last reconciled:** 2026-09-02 — closed W4.7 with node-local expert ownership
and causal distributed dataflow, rebuilt all four canonical deployments,
re-executed all four source-current token lanes, refreshed the HBM/ROM,
context/comparison, and four-deployment RTL certificates, retained a governed
DeepSeek HBM prefill cycle result, and completed the release-status refresh.
...
- [~] W6.6 Checkpoint/restart exactness on all four — the retained governed v2 captures are **historical evidence, not source-current evidence**. Each recorded Qwen HBM, Qwen ROM, DeepSeek HBM P32, and DeepSeek ROM P32 capture passed its own source-tree, process, deployment, implementation, node-count, non-vacuity, and real-resume guards when produced, but all four record the same historical Python source identity. The working tree has since changed, so no dirty-tree digest is presented here as a durable current source identity. The DeepSeek HBM capture also predates the node-local deployment (`45e2…`, 1,249 instructions / 2,915 descriptors versus the current `294319…`, 1,146 / 2,953). `make abi3-restart` retains one reproducible rerun target per lane. The historical method runs an uninterrupted comparator, stops a second process after **two** generated tokens <!-- figure: 2 src="results/abi3/restart_exactness_qwen3_hbm.json#record.notes.stop_after_tokens" name="restart interruption token count" -->, serialises complete mutable device state, and finishes the final token in a third process that loads only that checkpoint. Each retained capture reaches **three** generated tokens <!-- figure: 3 src="results/abi3/restart_exactness_deepseek_hbm_p32.json#record.generated_token_count" name="restart generated-token horizon" --> with token sequence, retired work, aggregate architectural counters, and every per-node counter value and sticky-overflow set identical to its uninterrupted run. The historical DeepSeek HBM checkpoint covers all **32** node arenas <!-- figure: 32 src="results/abi3/restart_exactness_deepseek_hbm_p32.json#record.target.node_count" name="restart DeepSeek HBM node count" --> and **4,032** mutable node/object images <!-- figure: 4,032 src="results/abi3/restart_exactness_deepseek_hbm_p32.json#record.notes.checkpoint.object_count" name="restart DeepSeek HBM mutable object images" -->; sparse v2 encoding retains **1,272,545,408** payload bytes <!-- figure: 1,272,545,408 src="results/abi3/restart_exactness_deepseek_hbm_p32.json#record.notes.checkpoint.stored_bytes" name="restart DeepSeek HBM sparse payload bytes" --> from **5,661,837,527,296** logical mutable bytes <!-- figure: 5,661,837,527,296 src="results/abi3/restart_exactness_deepseek_hbm_p32.json#record.notes.checkpoint.mutable_bytes" name="restart DeepSeek HBM logical mutable bytes" --> without scanning untouched sparse holes. In every retained lane, erasing all STATE images changes the first resumed token while the complementary STATE-only control reproduces the baseline after non-STATE scratch is erased. Evidence: `results/abi3/restart_exactness_{qwen3_hbm,qwen3_rom,deepseek_hbm_p32,deepseek_rom_p32}.json`. **Remaining closure:** rerun all four recipes from the current source tree and current deployments. **Claim boundary:** this is functional-simulator restart over a 93-token Qwen request <!-- figure: 93 src="results/abi3/restart_exactness_qwen3_hbm.json#record.workload.prompt_token_count" name="restart Qwen prompt length" --> and a 32-token DeepSeek prefix <!-- figure: 32 src="results/abi3/restart_exactness_deepseek_hbm_p32.json#record.workload.prompt_token_count" name="restart DeepSeek prompt length" -->, split 2+1 generated tokens. It is not a long-context, timing, performance, RTL, or silicon claim, and the HBM lane checkpoints the prepared state images described in [OI-23], not a repaired committed-state durability layout
...
- [~] W8.5 ROM service RTL (Qwen chip, DeepSeek wafer tile) — **the read path is implemented and correlated on two simulators, and it contains no ROM array.** `rtl/rom/ot_rom_read_service.sv` turns `(object_id, byte_offset, byte_length)` into a physical access against the compiled region plan the deployment publishes as `notes.rom_plan`: object lookup, shard walk to a placement resource and an address inside it, row-redundancy translation, region masking, fail-closed quarantine and column-repair refusal, row-activation accounting against a persistent row buffer, and the column mux onto the operand bus. Three vector sets, all read back out of real ROM deployments — never hand written — and replayed under Icarus and Verilator through independently written checkers (`results/rtl/rom_service_campaign.json`):
  ...
  - **Evidence freshness:** the campaign is now source-current and reports `status: pass` <!-- figure: "pass" src="results/rtl/rom_service_campaign.json#status" name="ROM service campaign status" -->. Both Qwen streams were re-executed from the current admitted ROM deployment (`925351…`) and current verified degraded rebuild (`811aa1…`) against the authenticated Qwen snapshot. Each records and rechecks a complete admission/decode/numeric/all-engine source map, the exact capability and workload file hashes, a **399**-range checkpoint-map identity verified by `DeviceMemory` <!-- figure: 399 src="results/rtl/rom_service_campaign.json#correlation.vector_sets.qwen_chip.executed_source.checkpoint_binding.authenticated_range_count" name="Qwen ROM authenticated checkpoint ranges" -->, zero unexpressible reads, and successful admission. The retained campaign directly pins every source and loaded input it consumes, and `python3 tools/rtl_rom_service_campaign.py --verify` re-hashes them; `executed_stream_source_drift`, `executed_stream_source_missing`, and `executed_stream_integrity_problems` are all empty. The DeepSeek set is refreshed to deployment `fa9077…` but remains deliberately plan-derived with a zero-byte checkpoint window: this campaign did not activate that 156 GiB device, so it does not call those bytes authenticated. The ignored deployment bundles are not retained dependencies; each vector's content-addressed ABI deployment digest is the durable boundary, not a claim that any arbitrary `build/` path still contains it.
...
W13.4 is the remaining open workload-coverage lane. The source-current summary
...
- [x] W11.4 Final status report and README update — `README.md`,
  ...
  clean-tree `make abi3-status` snapshot republishes these **82/94** milestone
  counts in `docs/PROGRAM_STATUS.md` and
  `results/abi3/program_status.json`
