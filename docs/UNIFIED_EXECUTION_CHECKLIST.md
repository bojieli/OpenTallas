...
- [x] W2.6 Cross-model operator union report; both exporters pass one schema + verifier — `tests/compiler/test_neutral_ir_cross_model.py` (22 gates); union published as `spec/abi3/numeric_contract_union.json` (68 contracts) <!-- figure: 68 src="spec/abi3/numeric_contract_union.json#contract_count" name="numeric contract union size" -->
...
- [ ] W13.4 DeepSeek-V4-Flash **ROM** (wafer) — **zero tokens.** Nine blockers
  ...
  128, and [OI-41] names `ATTENTION.SPARSE`'s scalar reference as the standing
...
|---|---:|---:|---|
| Pro @1M, 554,700 mm², batch 1 | 54.2× → 35.4× | **8.6×** | GPU denied a topology; then per-user latency conflated with throughput | <!-- figure: 8.6 src="results/roofline/n6_vs_a100/REPORT.md#Ratio after" table="latency separation" where="Model=DeepSeek-V4-Pro-0813;mm2=554700" name="Pro batch-1 iso-area ratio at 554,700 mm2, N6" -->
...
| on-wafer tensor-parallel | 116,278 tok/s | **5,000–9,000** | an all-reduce charged one flat hop however far it reached | <!-- figure: 9,000 src="results/roofline/n5_vs_b200/REPORT.md#Hard ceiling" table="Array or wafer" where="Design=Qwen3-8B/ROM-N5-native-SRAMKV-wafer-tensor-x1" tol="1%" name="on-wafer tensor hard ceiling, Qwen3-8B" why="the row states a deliberately rounded band; the artifact's Qwen on-wafer hard ceiling is 9,018.8 tok/s and Flash's is 7,550.6. The tolerance is the rounding the band declares, not slack." -->
| N5 vs B200, Pro @1M | 27.3× | **4.62×** | both of the above | <!-- figure: 4.62 src="results/roofline/n5_vs_b200/REPORT.md#Ratio after" table="latency separation" where="Model=DeepSeek-V4-Pro-0813;mm2=554700" name="Pro batch-1 iso-area ratio at 554,700 mm2, N5" -->
| per-region over broadcast @b64 | 27× | **3.57×** | mean engaged-region load where the physics is the busiest | <!-- figure: 3.57 src="results/roofline/n6_vs_a100/REPORT.md#Per-region over broadcast" table="batch-amortisation" where="Model=DeepSeek-V4-Flash-0731;B=64;Spare silicon=sram" name="per-region over broadcast, Flash b64 sram" -->
...
- *"The ROM advantage erodes with batch"* holds only for the **dense** model.
  Qwen goes 8.91× at batch 1 to **0.92× at 256** — the GPU wins outright. Both <!-- figure: 8.91 src="results/roofline/n6_vs_a100/REPORT.md#Per-user ratio" table="Iso-area comparison" where="Model=Qwen3-8B;B=1;Pick=fastest" name="Qwen per-user iso-area ratio at B=1" --> <!-- figure: 0.92 src="results/roofline/n6_vs_a100/REPORT.md#Per-user ratio" table="Iso-area comparison" where="Model=Qwen3-8B;B=256;Pick=fastest" name="Qwen per-user iso-area ratio at B=256" -->
  sparse models now **rise**: Flash 9.19× → **36.52×**, because a GPU's per-user <!-- figure: 9.19 src="results/roofline/n6_vs_a100/REPORT.md#Per-user ratio" table="Iso-area comparison" where="Model=DeepSeek-V4-Flash-0731;B=1;Pick=fastest" name="Flash per-user iso-area ratio at B=1" --> <!-- figure: 36.52 src="results/roofline/n6_vs_a100/REPORT.md#Per-user ratio" table="Iso-area comparison" where="Model=DeepSeek-V4-Flash-0731;B=256;Pick=fastest" name="Flash per-user iso-area ratio at B=256" -->
...
- **OI-41 — `ATTENTION.SPARSE` has no data-bearing implementation, and that is
