...
**Last reconciled:** 2026-09-01 — closed the governed Qwen HBM deployment gate
and made the DeepSeek cluster commit path causally atomic. The retained W4.7
findings now pass 20/26 checks. The repository-wide prose-figure audit passes.
...
- [~] W4.7 DeepSeek 32-node sharded deployment (experts, sparse gather, collectives, coordinated commit) — the deterministic retained-failure artifact `results/abi3/hbm_deepseek_deployment_findings.json` passes **20/26** checks <!-- figure: 20 src="results/abi3/hbm_deepseek_deployment_findings.json#cases[case=deepseek-v4-flash-hbm-cluster].passed_check_count" name="DeepSeek HBM deployment checks passed, W4.7" --> and makes the remaining six failures executable rather than implicit. The positive boundary is real: two clean builds reproduce the shipped exact-32-node deployment, the independent checker passes all 36 of its generic checks over 1,271 instructions, 2,938 descriptors, 224 zero-copy weight objects / 68,214 authenticated ranges, and the program contains 34 communications. The coordinated-commit path is now causal: predicated terminal work joins onto a four-event frontier, the route-class-4 cluster barrier waits for that frontier, and all 11 `STATE.COMMIT`s are issued later and wait for the barrier event. It is **not yet the named W4.7 deployment**: maximum-context placement requires **181,687,268,622** HBM bytes per node against **103,079,215,104** available <!-- figure: 181687268622 src="results/abi3/hbm_deepseek_deployment_findings.json#cases[case=deepseek-v4-flash-hbm-cluster].actual.hbm_bytes_per_node" name="DeepSeek HBM required bytes per node, W4.7" --> <!-- figure: 103079215104 src="results/abi3/hbm_deepseek_deployment_findings.json#cases[case=deepseek-v4-flash-hbm-cluster].actual.hbm_available_per_node" name="DeepSeek HBM available bytes per node, W4.7" -->; route classes 0/1/3 still have no expert-dispatch, sparse-gather or reduction transfer, and 4/20/12 such sites are explicitly recorded as replicated. The later four-token cluster execution therefore remains functional evidence, not a waiver for distributed placement.
