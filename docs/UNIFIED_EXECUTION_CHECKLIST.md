...
every remaining exit against its retained evidence, completed all four W6.6
restart lanes, completed the W10.1/W10.2 implementation preflight, and rebuilt
the standard Qwen bundles and their deployment, schedule, numeric and ROM-read
evidence. The final serialized DeepSeek HBM restart campaign passed in 2:56:39
against its governed 32-node deployment; the later W10 tranche deliberately
does not reuse that source identity as evidence for changed code.
**Current execution checkpoint (2026-09-02):** P10.7 is complete, but no new
multi-hour W10 capture has been launched and no top-level marker moves in this
checkpoint. Qwen's source-current graph is `84bb97dd…`; its rebuilt HBM and ROM
deployments are `679a41be…` and `274e4bc…`. Both admit the shared 8,256-position
session contract, 512-row token blocks and verifier-safe backing for the padded
8,704-row final block. The HBM deployment certificate passes 21/21 checks, the
ROM schedule certificate passes 62/62, and the refreshed six-case ROM-service
campaign passes on Icarus and Verilator with all 45 recorded sources and inputs
re-hashed. Numeric qualification now binds every blocked-library measurement
to a validated implementation-and-executed-shape manifest: 15 isolated phases,
including 2,035 blocked calls in the measured 8,000-token prefill contraction
set, with zero vocabulary-head argmax changes. The token runner enforces exact
EOS-or-cap termination and the independent W10 validator fails closed on stale
inputs, prefix-only agreement, malformed transactions, or unequal association
manifests. Fresh temporary one-token HBM and ROM diagnostics both produced
token 1654 and the same five-shape/253-call association digest; those diagnostics
remain preflight observations, not retained W10 closure evidence.
...
**Certification freshness:** the four published W6.6 restart artifacts remain
passing closure evidence for the exact 288-file source map each artifact binds.
This W10 tranche changes governed compiler and runtime files, so those records
must not be called source-current relative to the new head. W6.6 remains a
completed evidenced milestone; all four restart lanes must be refreshed before
the changed head is promoted as a new source-current restart baseline.
...
**Remaining-work order:** (1) run two clean W10.1 HBM natural captures and the
W10.2 HBM/ROM stress captures against the frozen external oracle, then pass the
independent exact-sequence/association gates; (2) refresh all four W6.6 restart
lanes against the changed governed source and
use accepted W10.1 evidence to close W11.1 and fill the six missing W13.4
...
- [x] P10.7 Rebuild standard bundles and refresh retained deployment, schedule and numeric evidence — graph `84bb97dd…`; HBM deployment `679a41be…`, 21/21 checks and 19,743,846,404 B/node; ROM deployment `274e4bc…`, 62/62 schedule checks; numeric report with 15 validated isolated association phases and zero vocabulary argmax changes; six-case ROM-service campaign current across 45 recorded sources. The shared planner source change is also reflected in the still-passing 34/34 DeepSeek HBM certificate.
...
- [ ] P10.10 Refresh the four restart-exactness lanes against the changed source map.
...
evidence; it is not a count of source-current evidence horizons. The longer
Qwen EOS, 24/192-token, reasoning, closed-loop agentic and historical 8K records
remain historical evidence. The previously current short token and restart
records bind the pre-W10 source map and become retained prior-head evidence when
this tranche lands. None may be presented as a rerun of the changed source.
...
- [x] W5.2 Qwen conventional single-chip ROM partition + images + schedules + deployment — the retained deployed Qwen ROM execution record admits at 75 instructions and 239 descriptors (`results/abi3/qwen3_rom_ta-qw-chat-1_execution.json`). Separately, the current-source v2 storage-and-placement proof lowers the current graph twice through the ROM backend and emits 75 instructions and 239 descriptors on both its ROM and comparison-HBM sides (`results/abi3/storage_class_equivalence_qwen3.json`). Its matching counts do not turn that comparison-HBM build into the shipped 75-instruction/218-descriptor HBM product deployment; [OI-19] remains the cross-backend gap. The ROM deployment retains 14 role-striped banks and 16,381,470,720 B <!-- figure: 75 src="results/abi3/qwen3_rom_ta-qw-chat-1_execution.json#record.notes.verification.instruction_count" name="Qwen ROM deployed instructions" --> <!-- figure: 239 src="results/abi3/qwen3_rom_ta-qw-chat-1_execution.json#record.notes.verification.descriptor_count" name="Qwen ROM deployed descriptors" --> <!-- figure: 75 src="results/abi3/storage_class_equivalence_qwen3.json#instruction_count.rom" name="Qwen ROM v2 equivalence-build instructions" --> <!-- figure: 239 src="results/abi3/storage_class_equivalence_qwen3.json#descriptor_count.rom" name="Qwen ROM v2 equivalence-build descriptors" -->
...
> **Retained evidence freshness.** At the pre-W10 head, Qwen ROM (`925351…`)
> and HBM (`8e1185…`) each reproduced the same four-token oracle prefix. The
> later 8,256-position and association-manifest tranche changes their governed
> source map, so they are now prior-head milestone evidence. The longer EOS and
> 24-token records are older again and have not been rerun on either identity.
...
- [x] W6.6 Checkpoint/restart exactness on all four — **all four lanes passed
  against one retained governed source map.** Qwen HBM (`8e1185…`), Qwen ROM (`925351…`),
...
- [x] W8.8 Co-simulation against the **four shipped deployment images**, not programs built for the campaign — `tools/rtl_abi3_deployment_campaign.py` loads each real deployment bundle into the microsequencer and compares every retirement, engine issue, and resolved operand view with `runtime.sim.device.Device` (`results/rtl/abi3_deployment_campaign.json`). The vector producer no longer contains copied deployment hashes or a digest-drift override: it resolves each image through its passing retained ROM-schedule or HBM-deployment certificate, re-hashes the certificate source map, Kernel IR, capability, and all three ignored bundle files, and records the certificate artifact identity beside the deployment. The refreshed artifact status is `pass`, `divergences[]` is empty, and both prefill and decode correlate on Icarus and Verilator for Qwen ROM single-chip (`274e4bc…`), Qwen HBM single-chip (`679a41be…`), DeepSeek-V4-Flash ROM wafer (`fa907792…`), and DeepSeek-V4-Flash HBM 32-node cluster (`2943197b…`).
...
W13.4 is the remaining partial workload-coverage lane. At the retained pre-W10
head, **all four designs ran the model end to end with a four-token
oracle-identical capture**. Those captures are now prior-head evidence because
this tranche changes governed compiler/runtime sources. Retained historical evidence extends both Qwen lanes
