...
|---|---:|---:|---|
| Pro @1M, 554,700 mm², batch 1 | 54.2× → 35.4× | **9.32×** | GPU denied a topology; then per-user latency conflated with throughput; the corrected physical/energy assumptions lowered it again | <!-- figure: 9.32 src="results/roofline/n6_vs_a100/REPORT.md#Ratio after" table="latency separation" where="Model=DeepSeek-V4-Pro-0813;mm2=554700" name="Pro batch-1 iso-area ratio at 554,700 mm2, N6" -->
...
| N5 vs B200, Pro @1M | 27.3× | **3.66×** | both of the above, plus the corrected physical/energy assumptions | <!-- figure: 3.66 src="results/roofline/n5_vs_b200/REPORT.md#Ratio after" table="latency separation" where="Model=DeepSeek-V4-Pro-0813;mm2=554700" name="Pro batch-1 iso-area ratio at 554,700 mm2, N5" -->
| per-region over broadcast @b64 | 27× | **2.27×** | busiest-region load plus the corrected model inputs | <!-- figure: 2.27 src="results/roofline/n6_vs_a100/REPORT.md#Per-region over broadcast" table="batch-amortisation" where="Model=DeepSeek-V4-Flash-0731;B=64;Spare silicon=sram" name="per-region over broadcast, Flash b64 sram" -->
...
- *"The ROM advantage erodes with batch"* holds only for the **dense** model.
  Qwen goes 22.87× at batch 1 to **4.51× at 256** — more than half the lead is gone, though it no longer inverts now that the array class is sampled on its own device-count ladder. Both <!-- figure: 22.87 src="results/roofline/n6_vs_a100/REPORT.md#Per-user ratio" table="Iso-area comparison" where="Model=Qwen3-8B;B=1;Pick=fastest" name="Qwen per-user iso-area ratio at B=1" --> <!-- figure: 4.51 src="results/roofline/n6_vs_a100/REPORT.md#Per-user ratio" table="Iso-area comparison" where="Model=Qwen3-8B;B=256;Pick=fastest" name="Qwen per-user iso-area ratio at B=256" -->
  sparse models now **rise**: Flash 9.82× → **11.27×**, because a GPU's per-user <!-- figure: 9.82 src="results/roofline/n6_vs_a100/REPORT.md#Per-user ratio" table="Iso-area comparison" where="Model=DeepSeek-V4-Flash-0731;B=1;Pick=fastest" name="Flash per-user iso-area ratio at B=1" --> <!-- figure: 11.27 src="results/roofline/n6_vs_a100/REPORT.md#Per-user ratio" table="Iso-area comparison" where="Model=DeepSeek-V4-Flash-0731;B=256;Pick=fastest" name="Flash per-user iso-area ratio at B=256" -->
...
through every correction. The Taalas HC1 gate **passes** at ratio 0.64× <!-- figure: 0.64 src="results/roofline/n6_vs_a100/REPORT.md#Ratio" table="Validation gates" where="Gate=Taalas HC1, Llama-3.1-8B on 815 mm2 at N6, per user" name="Taalas HC1 validation gate ratio" -->
(10,803 modelled against 16,960 published tokens/s; 24,675, or 1.45×, under the flat
...
on capacity; the legacy per-bit rule still fails it and is kept as a test.
...
the declared 2× gate. HC1 is 77.8 W against its published 200–250 W band
...
- [~] W11.3 Evidence ledger: every number traced to executed counters or labeled
  ...
  **2,226** candidates remain explicitly `untriaged`. <!-- figure: 2226 src="results/abi3/prose_figure_coverage.json#totals.unbound_triage.untriaged" name="W11.3 candidates still awaiting triage" -->
