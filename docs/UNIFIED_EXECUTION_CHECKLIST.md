...
and published the DeepSeek HBM retained findings. The repository-wide
prose-figure audit passes.
...
- [~] W4.7 DeepSeek 32-node sharded deployment (experts, sparse gather, collectives, coordinated commit) — the deterministic retained-failure artifact `results/abi3/hbm_deepseek_deployment_findings.json` passes **18/26** checks <!-- figure: 18 src="results/abi3/hbm_deepseek_deployment_findings.json#cases[case=deepseek-v4-flash-hbm-cluster].passed_check_count" name="DeepSeek HBM deployment checks passed, W4.7" --> and makes the remaining eight failures executable rather than implicit. The positive boundary is real: two clean builds reproduce the shipped exact-32-node deployment, the repaired independent checker passes all 36 of its generic checks over 1,249 instructions, 2,915 descriptors, 224 zero-copy weight objects / 68,214 authenticated ranges, and the program contains 34 communications. It is **not yet the named W4.7 deployment**: maximum-context placement requires **181,687,268,622** HBM bytes per node against **103,079,215,104** available <!-- figure: 181687268622 src="results/abi3/hbm_deepseek_deployment_findings.json#cases[case=deepseek-v4-flash-hbm-cluster].actual.hbm_bytes_per_node" name="DeepSeek HBM required bytes per node, W4.7" --> <!-- figure: 103079215104 src="results/abi3/hbm_deepseek_deployment_findings.json#cases[case=deepseek-v4-flash-hbm-cluster].actual.hbm_available_per_node" name="DeepSeek HBM available bytes per node, W4.7" -->; route classes 0/1/3 have no expert-dispatch, sparse-gather or reduction transfer; 4/20/12 such sites are explicitly recorded as replicated; and although route class 4 contains one commit barrier, that barrier waits for no prior work and none of the 11 `STATE.COMMIT`s waits for its completion event. The later four-token cluster execution therefore remains functional evidence, not a waiver for distributed placement or atomic commit.
