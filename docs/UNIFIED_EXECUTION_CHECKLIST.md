...
**Last reconciled:** 2026-09-01 — bounded the DeepSeek expert and index
pipelines with rolling activation arenas, closing its maximum-context HBM
capacity check. The retained W4.7 findings now pass 21/26 checks. The
repository-wide prose-figure audit passes.
...
- [x] W4.6 Qwen single-node deployment — `make abi3-hbm-qwen-deployment` rebuilds the current graph twice, checks both clean builds against the shipped bundle byte-for-byte, and publishes the source-locked `results/abi3/hbm_qwen_deployment_certificate.json`. All **18/18** deployment checks pass <!-- figure: 18 src="results/abi3/hbm_qwen_deployment_certificate.json#cases[case=qwen3-hbm-single-chip].passed_check_count" name="Qwen HBM deployment checks passed, W4.6" -->: the independent checker and frozen verifier admit **75** instructions and **218** descriptors <!-- figure: 75 src="results/abi3/hbm_qwen_deployment_certificate.json#cases[case=qwen3-hbm-single-chip].actual.instructions" name="Qwen HBM deployment instructions, W4.6" --> <!-- figure: 218 src="results/abi3/hbm_qwen_deployment_certificate.json#cases[case=qwen3-hbm-single-chip].actual.descriptors" name="Qwen HBM deployment descriptors, W4.6" -->; all **399** authenticated checkpoint ranges form 14 zero-copy weight objects <!-- figure: 399 src="results/abi3/hbm_qwen_deployment_certificate.json#cases[case=qwen3-hbm-single-chip].actual.weight_segments" name="Qwen HBM checkpoint ranges, W4.6" -->; and the emitted HBM-plus-state map spans **19,670,900,740** of 103,079,215,104 HBM bytes while the plan uses 29,360,128 of 134,217,728 SRAM bytes per node <!-- figure: 19670900740 src="results/abi3/hbm_qwen_deployment_certificate.json#cases[case=qwen3-hbm-single-chip].actual.hbm_bytes_per_node" name="Qwen HBM bytes per node, W4.6" -->. The topology is exactly one chip with no communication descriptors, while an independent profile diff proves its engines, feature bits, memory and numerics are the same as the 32-node profile and the otherwise-idle endpoint/credit/retry hardware remains advertised. This closes the deterministic deployment, not W10's long-run numeric/EOS acceptance or later cycle/RTL/physical gates.
- [~] W4.7 DeepSeek 32-node sharded deployment (experts, sparse gather, collectives, coordinated commit) — the deterministic retained-failure artifact `results/abi3/hbm_deepseek_deployment_findings.json` passes **21/26** checks <!-- figure: 21 src="results/abi3/hbm_deepseek_deployment_findings.json#cases[case=deepseek-v4-flash-hbm-cluster].passed_check_count" name="DeepSeek HBM deployment checks passed, W4.7" --> and makes the remaining five failures executable rather than implicit. Two clean builds reproduce the shipped exact-32-node deployment, and the independent checker passes all 36 of its generic checks over **1,225** instructions and **2,923** descriptors <!-- figure: 1225 src="results/abi3/hbm_deepseek_deployment_findings.json#cases[case=deepseek-v4-flash-hbm-cluster].actual.instructions" name="DeepSeek HBM instructions, W4.7" --> <!-- figure: 2923 src="results/abi3/hbm_deepseek_deployment_findings.json#cases[case=deepseek-v4-flash-hbm-cluster].actual.descriptors" name="DeepSeek HBM descriptors, W4.7" -->, with 224 zero-copy weight objects / 68,214 authenticated ranges and 34 communications. Six structurally closed expert/index pipelines now execute 64 kernels inside shared token-block loops; their **78** fixed-address arena slots occupy **533,676,032** bytes <!-- figure: 78 src="results/abi3/hbm_deepseek_deployment_findings.json#cases[case=deepseek-v4-flash-hbm-cluster].actual.rolling_activation_slots" name="DeepSeek rolling activation slots, W4.7" --> <!-- figure: 533676032 src="results/abi3/hbm_deepseek_deployment_findings.json#cases[case=deepseek-v4-flash-hbm-cluster].actual.rolling_activation_bytes" name="DeepSeek rolling activation bytes, W4.7" -->, while amendment A26 clamps each final partial block without advancing its object address. The resulting maximum-context plan needs **102,141,195,534** of **103,079,215,104** HBM bytes per node, leaving **938,019,570** bytes of checked headroom <!-- figure: 102141195534 src="results/abi3/hbm_deepseek_deployment_findings.json#cases[case=deepseek-v4-flash-hbm-cluster].actual.hbm_bytes_per_node" name="DeepSeek HBM required bytes per node, W4.7" --> <!-- figure: 103079215104 src="results/abi3/hbm_deepseek_deployment_findings.json#cases[case=deepseek-v4-flash-hbm-cluster].actual.hbm_available_per_node" name="DeepSeek HBM available bytes per node, W4.7" --> <!-- figure: 938019570 src="results/abi3/hbm_deepseek_deployment_findings.json#cases[case=deepseek-v4-flash-hbm-cluster].actual.hbm_headroom_per_node" name="DeepSeek HBM headroom per node, W4.7" -->. The coordinated-commit path remains causal: predicated terminal work joins onto a four-event frontier, the route-class-4 cluster barrier waits for it, and all 11 `STATE.COMMIT`s wait for the barrier. It is **not yet the named W4.7 deployment**: route classes 0/1/3 still have no expert-dispatch, sparse-gather or reduction transfer, and 4/20/12 such sites are explicitly recorded as replicated. The later four-token cluster execution remains functional evidence, not a waiver for distributed placement.
...
- [x] W6.6 Checkpoint/restart exactness on all four — **fresh governed v2 captures pass for Qwen HBM, Qwen ROM, DeepSeek HBM P32, and DeepSeek ROM P32.** `make abi3-restart` exposes one reproducible target per lane. Each target runs an uninterrupted comparator, stops a second process after **two** generated tokens <!-- figure: 2 src="results/abi3/restart_exactness_qwen3_hbm.json#record.notes.stop_after_tokens" name="restart interruption token count" -->, serialises the complete mutable device state, and finishes the final token in a third process that loads only that checkpoint. Every capture reaches **three** generated tokens <!-- figure: 3 src="results/abi3/restart_exactness_deepseek_hbm_p32.json#record.generated_token_count" name="restart generated-token horizon" --> with token sequence, retired work, aggregate architectural counters, and every per-node counter value and sticky-overflow set identical to its uninterrupted run; every non-vacuity, process, deployment, implementation, source-tree, node-count, and real-resume guard passes. The DeepSeek HBM checkpoint covers all **32** node arenas <!-- figure: 32 src="results/abi3/restart_exactness_deepseek_hbm_p32.json#record.target.node_count" name="restart DeepSeek HBM node count" --> and **4,032** mutable node/object images <!-- figure: 4,032 src="results/abi3/restart_exactness_deepseek_hbm_p32.json#record.notes.checkpoint.object_count" name="restart DeepSeek HBM mutable object images" -->; sparse v2 encoding retains **1,272,545,408** payload bytes <!-- figure: 1,272,545,408 src="results/abi3/restart_exactness_deepseek_hbm_p32.json#record.notes.checkpoint.stored_bytes" name="restart DeepSeek HBM sparse payload bytes" --> from **5,661,837,527,296** logical mutable bytes <!-- figure: 5,661,837,527,296 src="results/abi3/restart_exactness_deepseek_hbm_p32.json#record.notes.checkpoint.mutable_bytes" name="restart DeepSeek HBM logical mutable bytes" --> without scanning untouched sparse holes. In every lane, erasing all STATE images changes the first resumed token while the complementary STATE-only control reproduces the baseline after non-STATE scratch is erased. Evidence: `results/abi3/restart_exactness_{qwen3_hbm,qwen3_rom,deepseek_hbm_p32,deepseek_rom_p32}.json`. **Claim boundary:** this is functional-simulator restart over a 93-token Qwen request <!-- figure: 93 src="results/abi3/restart_exactness_qwen3_hbm.json#record.workload.prompt_token_count" name="restart Qwen prompt length" --> and a 32-token DeepSeek prefix <!-- figure: 32 src="results/abi3/restart_exactness_deepseek_hbm_p32.json#record.workload.prompt_token_count" name="restart DeepSeek prompt length" -->, split 2+1 generated tokens. It is not a long-context, timing, performance, RTL, or silicon claim, and the HBM lane checkpoints the prepared state images described in [OI-23], not a repaired committed-state durability layout
