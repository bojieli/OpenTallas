...
| fabric router `ot_rom_fabric_router` | 10,072.2 → 11,116.4 | +10.4 <!-- figure: 10.4 src="results/dft/summary.json#blocks.fabric_router.route.overhead.standard_cell_area" scale="100" name="fabric_router scan area overhead %" -->% | 1,204.9 <!-- figure: 1204.9 src="results/dft/summary.json#blocks.fabric_router.route.noscan.fmax_mhz" name="fabric_router Fmax no scan" --> → 1,155.6 <!-- figure: 1155.6 src="results/dft/summary.json#blocks.fabric_router.route.scan.fmax_mhz" name="fabric_router Fmax scan" --> | -4.1 <!-- figure: -4.1 src="results/dft/summary.json#blocks.fabric_router.route.overhead.fmax" scale="100" name="fabric_router scan Fmax change %" -->% | +7.3 <!-- figure: 7.3 src="results/dft/summary.json#blocks.fabric_router.route.overhead.routed_wirelength" scale="100" name="fabric_router scan wirelength overhead %" -->% | not_met / not_met |
...
- **Routes marked `not_met`.** `not_met` in the status column means a route
  failed an acceptance check. It is not always timing:
  - the package link's and fabric router's routes all meet timing and fail
    only on max-slew violations at the shared slew margin
    (`--slew-margin-percent 20`): 12 without scan and 19 with it for the
    package link, 52 and 3 for the fabric router;
  - the KV streamer's unscanned route missed setup by 3.5 ps.

  Both halves of each pair use the same settings, so the comparison is still
  like for like.
