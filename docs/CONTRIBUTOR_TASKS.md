...
| 7 | **Retire an `assumed` input.** §9 defines exactly what turns `assumed` into `measured`: raw artifact, command, tool version, config, unit conversion, uncertainty, reproduction check. Anyone with the relevant hardware can close one. | M | `ASSUMPTIONS.md` §9, priorities in `COMPARISON_FAIRNESS_AUDIT.md` Part D |
