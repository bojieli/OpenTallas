...
| M1 | The pinned official graph has a complete 2,136-node/46-kind ledger; 39 kinds now have qualified target references, including all routed-MXFP4/shared-FP8 SwiGLU, sqrt-softplus router activation, base/YaRN RoPE application and inverse, complete HC pre-mixing/final-head reduction, compressor transaction/view, sparse-attention, and fail-closed greedy/target-adapted sampling sites | Partial: seven operator references and operator-complete semantics remain open |
