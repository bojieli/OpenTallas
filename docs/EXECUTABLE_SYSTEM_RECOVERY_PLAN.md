...
| M1 | The pinned official graph has a complete 2,136-node/46-kind ledger; 45 kinds now have qualified target references, including all routed-MXFP4/shared-FP8 SwiGLU, sqrt-softplus router activation, session-bound circular-window KV transition/view, exact main/DSpark attention KV row-space composition, complete grouped attention-output projection, DSpark main-conditioning projection and stage-local prefill-KV transaction, the shared BF16-storage/binary32-runtime vocabulary head, base/YaRN RoPE application and inverse, complete HC pre-mixing/final-head reduction, compressor transaction/view, sparse-attention, and fail-closed greedy/target-adapted sampling sites | Partial: only the `MARKOV_AUTOREGRESSIVE_LOOP` reference remains open; graph-wide service-engine and RTL semantics are not implied by unit-reference coverage |
