...
| See every target at a glance | [Architecture atlas](ARCHITECTURE_ATLAS.html) (architecture specification OT-ARCH-001; open in a browser): decode-core microarchitecture, ISA, memory maps, numerics, link protocol, array/wafer/reticle systems, DeepSeek-V4.1 package, physical data, verification, risks, and the ROM-vs-HBM comparisons | [token pipeline plan](TOKEN_PIPELINE_OPTIMIZATION_PLAN.md) and the [analytical report](ANALYTICAL_REPORT.md) |
...
| Decide ROM wafer versus ROM array for DeepSeek-V4.1-Flash | [Wafer versus array at equal silicon](WAFER_VS_ARRAY_ISO_AREA.md) — per-operator critical path at 2/3/4/12 wafers against arrays of the same silicon, with fabric and Sinkhorn axes and a verdict | [analytical report](ANALYTICAL_REPORT.md) and `tools/decode_critical_path.py` |
