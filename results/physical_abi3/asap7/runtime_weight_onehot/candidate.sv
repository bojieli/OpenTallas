...
module ot_a3_weight_tile_prefetch #(
    ...
    reg outstanding;
    reg [PW-1:0] tail;
    reg [FIFO_DEPTH-1:0] head_select;
    reg [CW-1:0] count;
    ...
    // Decode the head in its ownership register before the wide selection and
    // downstream tag comparison. Data and identity use the same one-hot token.
    reg [138+TAG_BITS:0] selected_word;
    integer h;
    always @* begin
      selected_word=0;
      for(h=0;h<FIFO_DEPTH;h=h+1)
        selected_word=selected_word |
          ({data_mem[h],tag_mem[h],index_mem[h],last_mem[h]} &
           {(139+TAG_BITS){head_select[h]}});
    end
    assign {word_data,word_tag,word_index,word_last}=selected_word;
    ...
    always @(posedge clk or negedge rst_n) begin
      if(!rst_n) begin
        ...
        issued<=0;received<=0;outstanding<=0;head_select<=FIFO_DEPTH'(1);tail<=0;count<=0;
        ...
        case({read_fire,push})
          ...
          default:begin end
        endcase
        if(pop)head_select<=(head_select << 1) | FIFO_DEPTH'(head_select[FIFO_DEPTH-1]);
        if(push)begin
