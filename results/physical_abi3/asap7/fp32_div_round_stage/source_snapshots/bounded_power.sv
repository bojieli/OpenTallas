...
module ot_a3_fp32_div_rne_pipe (
    ...
    // Finite binary32 significand powers are -149..104.
    reg signed [8:0] numerator_power;
    reg signed [8:0] denominator_power;

    // Stage-1 registers: what the multiply cycle hands the compare cycle.
    reg [48:0] product_q;
    // Midpoint power reaches -150; product powers are -299..208.
    reg signed [9:0] product_lsb_power_q;
    reg        degenerate_q;      // a zero significand on either side
    reg [30:0] candidate_code_q;
    reg [24:0] rounding_significand_q;
    reg signed [8:0] rounding_power_q;
    ...
    function automatic integer compare_registered_product;
        ...
        begin
            ...
            if (degenerate) begin
                ...
                for (bit_index = 0; bit_index < 49; bit_index = bit_index + 1)
            ...
            if (in_valid && in_ready) begin
                ...
                if (numerator_code[30:23] == 8'hff ||
                    ...
                    if (numerator_code[30:23] == 0) begin
                        ...
                        numerator_power <= 9'({24'b0, numerator_code[30:23]} - 150);
                    end
                    if (denominator_code[30:23] == 0) begin
                        ...
                        denominator_power <=
                            9'({24'b0, denominator_code[30:23]} - 150);
                    end
                end
                ...
                case (phase)
                    STEP_MUL: begin
                        if (low_code <= high_code) begin
                            ...
                            product_lsb_power_q <=
                                10'(candidate_power + denominator_power);
                            phase <= STEP_CMP;
                        end else begin
                            // Capture the RNE midpoint before multiplication.
                            // This stage runs once per division, outside the
                            // iterative search, so it costs only one cycle.
                            rounding_significand_q <= midpoint_significand;
                            rounding_power_q <= 9'(midpoint_power);
                            phase <= ROUND_MUL;
                        end
                    end
                    ROUND_MUL: begin
                        degenerate_q <= (rounding_significand_q == 0) ||
                                        (denominator_significand == 0);
                        product_q <= rounding_significand_q *
                                     {25'b0, denominator_significand};
                        product_lsb_power_q <= 10'(rounding_power_q) + 10'(denominator_power);
                        phase <= ROUND_CMP;
                    end
                    STEP_CMP: begin
                        if (comparison <= 0) begin
