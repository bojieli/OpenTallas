...
module ot_a3_lq8_auxiliary_prefetch #(
    ...
    // Caller owns one generation until clear; do not enable for mixed streams.
    ...
    reg [31:0] operation_generation;
    wire [31:0] response_expected_generation,head_generation,dispatch_generation;
    generate if(SINGLE_GENERATION)begin: shared_generation
        assign response_expected_generation=operation_generation;
        assign head_generation=operation_generation;
        assign dispatch_generation=operation_generation;
    ...
    assign {service_generation,service_a,service_s,service_ws,service_w}=
        REGISTER_REQUESTS ? {dispatch_generation,identities[dispatch_head][127:0]} :
        {request_generation,request_a,request_s,request_ws,request_w};
    wire identity_matches=response_generation==response_expected_generation &&
    ...
    always @(posedge clk or negedge rst_n)begin
        if(!rst_n)begin head_select<=DEPTH'(1);tail<=0;response_tail<=0;pending<=0;complete<=0;occupied<=0;dispatch_head<=0;unsent<=0;end
            ...
            endcase
            if(take)begin
                identities[tail]<=ID_BITS'({request_generation,request_a,request_s,request_ws,request_w});
                tail<=next_ptr(tail);
            end
            if(push)begin
