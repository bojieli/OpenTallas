...
module ot_a3_auxiliary_window_scheduler #(
 ...
 always @(posedge clk)begin
  ...
  offset_q<=address_q-base_q;
  // A rolling activation window retains a revisited row from its missed
  // address. Scale planes keep base-relative fixed pages. Same capacity/tail.
  page_q<=(ACTIVATION_MISS_ALIGNED && plane==0)?offset_q:{offset_q[31:8],8'b0};
  remaining_q<=words_q-((ACTIVATION_MISS_ALIGNED && plane==0)?offset_q:{offset_q[31:8],8'b0});
  if(state==SIZE)begin
