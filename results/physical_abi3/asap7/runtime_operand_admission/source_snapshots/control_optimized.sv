...
module ot_a3_lq8_operand_admission #(
    ...
    // Only publication/ownership state needs reset. Payload is captured before
    // use and may change while invalid, including on a clear edge. Products
    // form a feed-forward pipeline from held geometry; no per-stage enable.
    always @(posedge clk)begin
        output_elements<=32'(rows)*32'(local_cols);
        stream_extent<=48'(output_elements)*48'(depth_words);
