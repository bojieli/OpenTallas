...
2. **Per-user latency and aggregate throughput are now separate quantities, and separating them is the largest correction in this report.** A token under pipeline parallelism is served by one stage's silicon at a time and must visit every stage, so its latency is the aggregate service time multiplied by `token_slots`, not divided by anything. The two factors cancel exactly: **adding devices under pipeline parallelism buys aggregate throughput and buys one user nothing.** On the ROM side the correction reaches 467x (ROM-N6-native-SRAMKV-wafer-pipeline-x12, 681 slots), and the batch-1 design that now wins -- the smallest silicon within 5% of the best per-user rate, which is the rule this report has since REPLACED and keeps only to compute the before/after -- runs `tensor` on 1 device. On the GPU side the correction reaches 508x (a100_sxm_80gb-x672-pipeline, 672 slots), and the batch-1 design that now wins -- the smallest silicon within 5% of the best per-user rate, which is the rule this report has since REPLACED and keeps only to compute the before/after -- runs `tensor` on 87 devices. Both validation gates are single-slot machines and are unchanged to the digit.
3. **Each model is recommended one design, by a rule stated in this report, and the answer is not the same class for all three.** The rule keeps every design nothing else beats on BOTH per-user tokens/s and tokens/s per mm2, then walks that frontier from the smallest feasible machine and stops when the next slab of silicon returns less than the silicon already bought. DeepSeek-V4-Flash-0731 takes 1 x 46,225 mm2 (46,225 mm2, wafer, KV in SRAM) at 4,708 tok/s per user and 102 tok/s per 1,000 mm2, holding 1 session, against 56 copies of one unified HBM die at the same silicon: 6.6x per user. Every model lands in the same class under this rule, which is a result rather than an assumption. Ranking on per-user rate alone -- which is what this report used to do -- hands Qwen3-8B a whole wafer for a checkpoint that holds in three reticle dies.
4. **The largest ratio anywhere in this study is not the study's result, and it is reported here so nobody has to go looking for it.** The maximum batch-1 per-user ratio is DeepSeek-V4-Flash-0731 on 46,225 mm2 of ROM silicon at 4,708 tok/s per user against 46,256 mm2 of a100_sxm_80gb-x56-tensor at 715 tok/s: **6.6x**, ROM binding on `link_latency` and the GPU on `link_latency`. It holds 1 resident session against the GPU cluster's 561. A maximum over a sampling grid is a fact about the grid; the recommended-design ratios above are the ones this report stands behind.
...
6. **Letting the GPU choose its own parallelism is worth up to 153.25x to it.** At 554,700 mm2 on DeepSeek-V4-Flash-0731 the pipeline-only GPU delivers 4.09 tok/s and the same silicon running tensor delivers 627 tok/s.
7. **The advantage erodes with batch, and the erosion is a KV effect.** At batch 256 the aggregate ratio at equal area spans 0.74x (DeepSeek-V4-Flash-0731, ROM binding on `link_latency`) to 19.45x (DeepSeek-V4-Flash-0731, ROM binding on `link_latency`). Weight traffic is what ROM removes; KV traffic it does not, and KV traffic is what grows with batch.
8. **Sparse MoE buys aggregate throughput on a ROM machine, not latency.** DeepSeek-V4-Flash-0731 engages 6.7% of its ROM array at batch 1 and 16.0% at batch 256, while the weight-read time is identical at both. What the machine delivers rises from 422 to 19,364 tok/s, and its rate with every slot occupied from 18,585 to 19,364. The second rises far less than the first because the batch-1 figure is already a full-machine number -- this design has 44 slots -- so most of what batching adds there is coverage rather than occupancy. An unselected expert's read ports cannot be borrowed, so its idle bandwidth is only recovered by giving the sweep more users.
...
11. **A wafer has less die edge per unit area than the same area of separate dies, and that is an argument against it.** Perimeter grows as the square root of area, so HBM beachfront -- and therefore KV bandwidth -- does not scale with wafer area the way compute and ROM capacity do. This model charges both sides the same edge utilisation a shipping GPU achieves, and the consequence shows up wherever a design binds on `kv_read`: 144 of 1846 feasible points.
12. **The largest open question is not in this model's inputs but in the architecture, and a batch-1 anchor cannot settle its scaling law.** If a ROM cell both stores and multiplies, each concurrent stream needs its own pass and aggregate per-die throughput never exceeds the per-user rate. At batch 256 that costs up to 6.2x of aggregate throughput (DeepSeek-V4-Flash-0731). Their sweep counts coincide at batch 1, but their cell and pre-compute costs make their floorplans different; the current compute-in-ROM anchor reconstruction fails capacity. A batch-1 validation therefore cannot establish either high-batch law.
13. **A third machine sits between them, and for a sparse model it recovers part of what compute-in-ROM gives up -- less than the mean-region arithmetic used to say.** Give each expert region its own activation port and two tokens selecting disjoint experts drive disjoint regions at the same time; only the tokens landing on one region serialise, and the sweep waits for the BUSIEST region rather than the average engaged one. The largest gain over a global broadcast is 2.64x, on DeepSeek-V4-Flash-0731 at batch 256, where the busiest region carries 3.04x the load of the mean engaged one. It is not free ground: per-region still loses to the amortising ROM-plus-MAC machine at 15 of 16 operating points. A dense model has one region, so it gains nothing -- the disjointness is what sparsity buys.
14. **Every number here is conditional on the assumed inputs listed in the evidence ledger below.** The ROM cell-area ratio and the ROM read bandwidth density are the two that move the answer most, and neither has been measured at N6.
15. **No point in this study is power-limited.** Static power is charged per mm2 per second, so this is a statement about the designs rather than an artifact of a traffic-proportional energy model: the worst point here reaches 66% of its cooling budget. The companion study at the other node does have power-limited points.
...
- 4,708 tok/s aggregate with every slot full, over 1 resident session (fill limited by `batch`)
- 3,601 W at 0.078 W/mm2, 764.9 mJ/token, thermal scale 1.000
...
| user tok/s | 4,707.9 | 715.0 | 6.58x |
| aggregate tok/s | 4,708 | 715 | 6.58x |
| resident sessions | 1 | 561 | -- |
| J/token | 0.7649 | 12.6533 | 16.5x |
...
The GPU's own best machine at **any** area is `a100_sxm_80gb-x224-tensor` at 185,024 mm2 and 784.9 tok/s per user, which is the area-free bound and is quoted so the iso-area row is not the only comparison on the page.

**Headline before and after.**

| rule | design | mm2 | user tok/s | tok/s per 1,000 mm2 | resident sessions | iso-area ratio |
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| before -- smallest within 5% of peak rate | `ROM-N6-native-SRAMKV-wafer-tensor-x1` | 46,225 | 4,707.9 | 101.8 | 1 | 6.58x |
| rank on per-user rate alone | `ROM-N6-native-SRAMKV-wafer-tensor-x1` | 46,225 | 4,707.9 | 101.8 | 1 | 6.58x |
| smallest feasible machine | `ROM-N6-native-HBMKV-array-hybrid-x44` | 35,860 | 1,303.6 | 36.4 | 460 | 1.89x |
| **after -- this report's rule** | `ROM-N6-native-SRAMKV-wafer-tensor-x1` | 46,225 | 4,707.9 | 101.8 | 1 | 6.58x |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | --- | ---: | ---: | --- | ---: |
| `ROM-N6-native-SRAMKV-wafer-tensor-x1` **<-- recommended** | 46,225 | 1 | 4,707.9 | 4,708 | 101.8 | 1 | `link_latency` | 3,601 | 764.9 | `a100_sxm_80gb-x56-tensor` | 6.58x |

**Array or wafer, with the losing class's own best machine on the page.** A frontier can honestly be a single row -- that is what it means for one design to win on both axes at once -- and a single row tells a reader nothing about what it beat. Each class enters at its own optimum, never at its minimum-feasible machine, because comparing against a floor is how a class gets beaten by its own under-provisioning rather than by the other class.

| class | designs | pick | design | mm2 | user tok/s | tok/s per 1,000 mm2 | resident sessions |
| --- | ---: | --- | --- | ---: | ---: | ---: | ---: |
| array | 138 | densest | `ROM-N6-native-SRAMKV-array-hybrid-x46` | 37,490 | 1,431.7 | 38.2 | 1 |
| array | 138 | fastest | `ROM-N6-native-SRAMKV-array-hybrid-x48` | 39,120 | 1,449.0 | 37.0 | 1 |
| array | 138 | smallest | `ROM-N6-native-HBMKV-array-hybrid-x44` | 35,860 | 1,303.6 | 36.4 | 460 |
| wafer | 70 | densest | `ROM-N6-native-SRAMKV-wafer-tensor-x1` | 46,225 | 4,707.9 | 101.8 | 1 |
| wafer | 70 | fastest | `ROM-N6-native-SRAMKV-wafer-tensor-x1` | 46,225 | 4,707.9 | 101.8 | 1 |
| wafer | 70 | smallest | `ROM-N6-native-SRAMKV-wafer-tensor-x1` | 46,225 | 4,707.9 | 101.8 | 1 |

**Three classes at iso-area, batch by batch.** Each ROM class enters at its fastest feasible design for that batch and is read against the GPU comparator at *its own* silicon area (the area ratio is stated). The `array @ wafer area` row is the fastest reticle array within 5% of the wafer's silicon, which is the wafer-versus-array comparison at iso-area. Read resident sessions before the ratio.

| batch | class | design | mm2 | user tok/s | aggregate tok/s | resident sessions | W | mJ/token | binds on | iso-area GPU | GPU user tok/s | GPU sessions | GPU mJ/token | area ratio | speed ratio | J/token ratio |
| ---: | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 1 | array | `ROM-N6-native-SRAMKV-array-hybrid-x48` | 39,120 | 1,449.0 | 1,449 | 1 | 2,528 | 1,744.8 | `link_latency` | `a100_sxm_80gb-x47-tensor` | 699.8 | 467 | 11,041.9 | 1.008 | 2.07x | 6.3x |
| 1 | wafer | `ROM-N6-native-SRAMKV-wafer-tensor-x1` | 46,225 | 4,707.9 | 4,708 | 1 | 3,601 | 764.9 | `link_latency` | `a100_sxm_80gb-x56-tensor` | 715.0 | 561 | 12,653.3 | 0.999 | 6.58x | 16.5x |
| 1 | array @ wafer area | `ROM-N6-native-HBMKV-array-hybrid-x56` | 45,640 | 1,368.4 | 9,579 | 585 | 6,145 | 641.6 | `link_latency` | `a100_sxm_80gb-x55-tensor` | 713.4 | 550 | 12,476.0 | 1.005 | 1.92x | 19.4x |
| 1 | wafer reference | `ROM-N6-native-SRAMKV-wafer-tensor-x1` | 46,225 | 4,707.9 | -- | 1 | -- | 764.9 | -- | -- | -- | -- | -- | 0.987 | 3.44x wafer/array | -- |
| 2 | array | `ROM-N6-native-HBMKV-array-hybrid-x47` | 38,305 | 1,449.0 | 8,694 | 491 | 4,807 | 552.9 | `link_latency` | `a100_sxm_80gb-x46-tensor` | 584.2 | 456 | 6,616.7 | 1.008 | 2.48x | 12.0x |
| 2 | wafer | `ROM-N6-native-HBMKV-wafer-hybrid-x3-romfill` | 138,675 | 3,388.5 | 10,165 | 269 | 14,609 | 1,437.1 | `link_latency` | `a100_sxm_80gb-x168-tensor` | 684.0 | 1,732 | 18,662.7 | 0.999 | 4.95x | 13.0x |
| 2 | array @ wafer area | `ROM-N6-native-HBMKV-array-hybrid-x170-romfill` | 138,550 | 1,190.9 | 26,199 | 1,777 | 19,807 | 756.0 | `link_latency` | `a100_sxm_80gb-x168-tensor` | 684.0 | 1,732 | 18,662.7 | 0.998 | 1.74x | 24.7x |
| 2 | wafer reference | `ROM-N6-native-HBMKV-wafer-hybrid-x3-romfill` | 138,675 | 3,388.5 | -- | 269 | -- | 1,437.1 | -- | -- | -- | -- | -- | 0.999 | 2.85x wafer/array | -- |
| 4 | array | `ROM-N6-native-HBMKV-array-hybrid-x47` | 38,305 | 1,449.0 | 8,694 | 491 | 4,807 | 552.9 | `link_latency` | `a100_sxm_80gb-x46-tensor` | 463.5 | 456 | 4,302.3 | 1.008 | 3.13x | 7.8x |
| 4 | wafer | `ROM-N6-native-HBMKV-wafer-hybrid-x4-romfill` | 184,900 | 3,331.4 | 13,326 | 359 | 19,440 | 1,458.8 | `link_latency` | `a100_sxm_80gb-x224-tensor` | 565.2 | 2,318 | 15,026.2 | 0.999 | 5.89x | 10.3x |
| 4 | array @ wafer area | `ROM-N6-native-HBMKV-array-hybrid-x227-romfill` | 185,005 | 1,173.9 | 34,043 | 2,373 | 26,289 | 772.2 | `link_latency` | `a100_sxm_80gb-x224-tensor` | 565.2 | 2,318 | 15,026.2 | 1.000 | 2.08x | 19.5x |
| 4 | wafer reference | `ROM-N6-native-HBMKV-wafer-hybrid-x4-romfill` | 184,900 | 3,331.4 | -- | 359 | -- | 1,458.8 | -- | -- | -- | -- | -- | 1.001 | 2.84x wafer/array | -- |
| 8 | array | `ROM-N6-native-HBMKV-array-hybrid-x56` | 45,640 | 1,366.3 | 10,930 | 585 | 6,370 | 582.8 | `link_latency` | `a100_sxm_80gb-x55-tensor` | 355.3 | 550 | 3,395.9 | 1.005 | 3.85x | 5.8x |
| 8 | wafer | `ROM-N6-native-HBMKV-wafer-hybrid-x8-romfill` | 369,800 | 3,121.2 | 24,970 | 719 | 38,597 | 1,545.7 | `link_latency` | `a100_sxm_80gb-x448-tensor` | 391.8 | 4,660 | 21,239.0 | 0.999 | 7.97x | 13.7x |
| 16 | array | `ROM-N6-native-HBMKV-array-hybrid-x56` | 45,640 | 1,290.9 | 20,655 | 585 | 7,985 | 386.6 | `link_latency` | `a100_sxm_80gb-x55-tensor` | 245.8 | 550 | 2,541.3 | 1.005 | 5.25x | 6.6x |
| 16 | wafer | `ROM-N6-native-HBMKV-wafer-hybrid-x12-romfill` | 554,700 | 2,656.1 | 42,497 | 1,079 | 58,720 | 1,381.7 | `link_latency` | `a100_sxm_80gb-x672-tensor` | 298.2 | 7,002 | 20,859.7 | 0.999 | 8.91x | 15.1x |
| 32 | array | `ROM-N6-native-HBMKV-array-hybrid-x111-romfill` | 90,465 | 1,211.2 | 38,760 | 1,160 | 16,532 | 426.5 | `link_latency` | `a100_sxm_80gb-x110-tensor` | 182.6 | 1,125 | 3,167.0 | 0.996 | 6.64x | 7.4x |
| 32 | wafer | `ROM-N6-native-HBMKV-wafer-hybrid-x12-romfill` | 554,700 | 1,922.9 | 61,533 | 1,079 | 61,862 | 1,005.4 | `kv_read` | `a100_sxm_80gb-x672-tensor` | 201.6 | 7,002 | 15,491.2 | 0.999 | 9.54x | 15.4x |
| 64 | array | `ROM-N6-native-HBMKV-array-hybrid-x170-romfill` | 138,550 | 1,149.9 | 73,596 | 1,777 | 27,683 | 376.1 | `link_latency` | `a100_sxm_80gb-x168-tensor` | 118.5 | 1,732 | 3,566.4 | 0.998 | 9.71x | 9.5x |
| 64 | wafer | `ROM-N6-native-HBMKV-wafer-hybrid-x12` | 554,700 | 1,238.9 | 79,291 | 1,079 | 92,218 | 1,163.0 | `kv_read` | `a100_sxm_80gb-x672-tensor` | 124.3 | 7,002 | 12,562.6 | 0.999 | 9.97x | 10.8x |
| 256 | array | `ROM-N6-native-HBMKV-array-hybrid-x340-romfill` | 277,100 | 746.5 | 191,107 | 3,555 | 62,614 | 327.6 | `link_latency` | `a100_sxm_80gb-x335-tensor` | 38.4 | 3,478 | 5,151.9 | 1.001 | 19.45x | 15.7x |
| 256 | wafer | `ROM-N6-native-HBMKV-wafer-hybrid-x12` | 554,700 | 395.3 | 101,194 | 1,079 | 95,817 | 946.9 | `kv_read` | `a100_sxm_80gb-x672-tensor` | 39.0 | 7,002 | 9,944.6 | 0.999 | 10.13x | 10.5x |
...
| 2-4 | `ROM-N6-native-HBMKV-array-hybrid-x46` | 37,490 | array | HBM | 481 |
| 8 | `ROM-N6-native-HBMKV-array-hybrid-x47` | 38,305 | array | HBM | 491 |
| 16 | `ROM-N6-native-HBMKV-array-hybrid-x56` | 45,640 | array | HBM | 585 |
| 32-64 | `ROM-N6-native-HBMKV-array-hybrid-x57` | 46,455 | array | HBM | 596 |
| 256 | `ROM-N6-native-HBMKV-array-pipeline-x74` | 60,310 | array | HBM | 773 |
...
| --- | --- | --- | --- |
| DeepSeek-V4-Flash-0731 | HBM | rom | 44, 46, 47, 56, 57, 74, 111, 113, 148, 170, 227, 340 |
| DeepSeek-V4-Flash-0731 | HBM | sram | 44, 46, 47, 56, 57, 74, 111, 113, 148, 170, 227, 340 |
| DeepSeek-V4-Flash-0731 | SRAM | rom | 44, 46, 48, 57, 76, 113, 114, 152, 170, 227, 340 |
| DeepSeek-V4-Flash-0731 | SRAM | sram | 44, 46, 48, 57, 76, 113, 114, 152, 170, 227, 340 |
...
- **0 of 1,846 feasible points (0.0%) are power-limited.**
- 0 points are uncoolable at any speed (static power alone at or above the cooling budget).

Nothing in this study is power-limited. That is a statement about these designs and not an artifact of the energy model: static power is charged per mm2 per second, so a design cannot escape it by moving fewer bytes. The busiest point reaches 66% of its cooling budget, and the busiest wafer-scale ROM design 35%. The companion study at the other node, whose HBM generation delivers more than twice the bandwidth per stack, does have power-limited points.

| Family | Area class | Points | Throttled | Median power / budget | Worst power / budget | Peak W/mm2 | Median static share |
|---|---|---:|---:|---:|---:|---:|---:|
| gpu | large array (5,000-40,000 mm2) | 120 | 0 | 49.0% | 66.4% | 0.322 | 74% |
| gpu | wafer (>=40,000 mm2) | 504 | 0 | 42.1% | 58.3% | 0.282 | 86% |
| rom | large array (5,000-40,000 mm2) | 162 | 0 | 25.1% | 41.4% | 0.207 | 69% |
| rom | wafer (>=40,000 mm2) | 1,060 | 0 | 26.9% | 65.7% | 0.329 | 83% |
...
|---|---:|---:|---|---:|---:|---|---|---:|---:|---|---:|
| DeepSeek-V4-Flash-0731 | 1 | 46,225 | `DSV4-Flash/ROM-N6-native-SRAMKV-wafer-tensor-x1` | 0.764938 | 3,601.3 | link_latency | `DSV4-Flash/a100_sxm_80gb-x56-tensor` | 12.653332 | 9,046.9 | link_latency | 16.54x |
| DeepSeek-V4-Flash-0731 | 2 | 138,675 | `DSV4-Flash/ROM-N6-native-HBMKV-wafer-hybrid-x3-romfill` | 1.437122 | 14,609.0 | link_latency | `DSV4-Flash/a100_sxm_80gb-x168-tensor` | 18.662657 | 25,531.8 | link_latency | 12.99x |
| DeepSeek-V4-Flash-0731 | 4 | 184,900 | `DSV4-Flash/ROM-N6-native-HBMKV-wafer-hybrid-x4-romfill` | 1.458848 | 19,440.2 | link_latency | `DSV4-Flash/a100_sxm_80gb-x224-tensor` | 15.026236 | 33,969.2 | link_latency | 10.30x |
| DeepSeek-V4-Flash-0731 | 8 | 369,800 | `DSV4-Flash/ROM-N6-native-HBMKV-wafer-hybrid-x8-romfill` | 1.545750 | 38,596.8 | link_latency | `DSV4-Flash/a100_sxm_80gb-x448-tensor` | 21.239003 | 66,570.5 | link_latency | 13.74x |
| DeepSeek-V4-Flash-0731 | 16 | 554,700 | `DSV4-Flash/ROM-N6-native-HBMKV-wafer-hybrid-x12-romfill` | 1.381739 | 58,719.9 | link_latency | `DSV4-Flash/a100_sxm_80gb-x672-tensor` | 20.859684 | 99,516.2 | link_latency | 15.10x |
| DeepSeek-V4-Flash-0731 | 32 | 554,700 | `DSV4-Flash/ROM-N6-native-HBMKV-wafer-hybrid-x12-romfill` | 1.005359 | 61,862.3 | kv_read | `DSV4-Flash/a100_sxm_80gb-x672-tensor` | 15.491243 | 99,917.1 | link_latency | 15.41x |
| DeepSeek-V4-Flash-0731 | 64 | 554,700 | `DSV4-Flash/ROM-N6-native-HBMKV-wafer-hybrid-x12` | 1.163041 | 92,218.3 | kv_read | `DSV4-Flash/a100_sxm_80gb-x672-tensor` | 12.562645 | 99,946.7 | link_latency | 10.80x |
| DeepSeek-V4-Flash-0731 | 256 | 277,100 | `DSV4-Flash/ROM-N6-native-HBMKV-array-hybrid-x340-romfill` | 0.327637 | 62,613.8 | link_latency | `DSV4-Flash/a100_sxm_80gb-x335-tensor` | 5.151885 | 50,619.6 | link_latency | 15.72x |
...
|---|---:|---:|---:|---|---:|---|---:|---:|---|---:|---|---:|---:|---:|---:|
| DeepSeek-V4-Flash-0731 | 1 | 46,225 | 19,190.8 | wafer-pipeline | 4,707.9 | wafer-tensor | 4.08x | 1,547.7 | pipeline | 715.0 | tensor | 2.16x | 12.40x | 6.58x | 0.53x |
| DeepSeek-V4-Flash-0731 | 2 | 92,450 | 25,096.8 | wafer-pipeline | 4,397.5 | wafer-hybrid | 5.71x | 1,790.9 | pipeline | 759.4 | tensor | 2.36x | 14.01x | 5.79x | 0.41x |
| DeepSeek-V4-Flash-0731 | 3 | 138,675 | 30,966.6 | wafer-pipeline | 4,310.3 | wafer-hybrid | 7.18x | 1,894.2 | pipeline | 776.1 | tensor | 2.44x | 16.35x | 5.55x | 0.34x |
| DeepSeek-V4-Flash-0731 | 4 | 184,900 | 34,996.6 | wafer-pipeline | 4,222.4 | wafer-hybrid | 8.29x | 1,951.4 | pipeline | 784.9 | tensor | 2.49x | 17.93x | 5.38x | 0.30x |
| DeepSeek-V4-Flash-0731 | 6 | 277,350 | 40,171.3 | wafer-pipeline | 4,053.2 | wafer-hybrid | 9.91x | 2,012.9 | pipeline | 621.7 | tensor | 3.24x | 19.96x | 6.52x | 0.33x |
| DeepSeek-V4-Flash-0731 | 8 | 369,800 | 43,352.1 | wafer-pipeline | 3,895.3 | wafer-hybrid | 11.13x | 2,045.4 | pipeline | 624.6 | tensor | 3.27x | 21.19x | 6.24x | 0.29x |
| DeepSeek-V4-Flash-0731 | 12 | 554,700 | 47,059.3 | wafer-pipeline | 3,612.4 | wafer-hybrid | 13.03x | 2,079.2 | pipeline | 627.5 | tensor | 3.31x | 22.63x | 5.76x | 0.25x |

**Did the error cancel in the ratio?** If it had, `Ratio change` would be 1.00x on every row. It runs from 0.25x to 0.53x across this ladder. It does not cancel, for the reason the two families reach equal area at very different device counts and therefore at very different slot counts, and because the correction changes which topology each side picks -- a change that lands on whichever side was relying on depth.
...
|---|---:|---|---|---:|---:|---:|---|---|---:|---:|---|---:|---:|---:|---|---:|---:|---:|---:|
| DeepSeek-V4-Flash-0731 | 1 | fastest | DSV4-Flash/ROM-N6-native-SRAMKV-wafer-tensor-x1 | 46,225 | 4,707.9 | 4,707.9 | link_latency | DSV4-Flash/a100_sxm_80gb-x56-tensor | 46,256 | 1.00x | tensor | 857.79 | 715.0 | 715.0 | link_latency | 6.58x | 6.58x | 139.88x | 6.58x |
| DeepSeek-V4-Flash-0731 | 1 | smallest silicon | DSV4-Flash/ROM-N6-native-HBMKV-array-hybrid-x44 | 35,860 | 1,303.6 | 7,821.3 | link_latency | DSV4-Flash/a100_sxm_80gb-x43-tensor | 35,518 | 1.01x | tensor | 855.78 | 690.9 | 690.9 | link_latency | 1.89x | 11.32x | 32.62x | 1.89x |
| DeepSeek-V4-Flash-0731 | 2 | fastest | DSV4-Flash/ROM-N6-native-HBMKV-wafer-hybrid-x3-romfill | 138,675 | 3,388.5 | 10,165.5 | link_latency | DSV4-Flash/a100_sxm_80gb-x168-tensor | 138,768 | 1.00x | tensor | 952.52 | 684.0 | 1,368.1 | link_latency | 4.95x | 7.43x | 233.94x | 4.95x |
| DeepSeek-V4-Flash-0731 | 2 | smallest silicon | DSV4-Flash/ROM-N6-native-HBMKV-array-hybrid-x44 | 35,860 | 1,303.6 | 7,821.3 | link_latency | DSV4-Flash/a100_sxm_80gb-x43-tensor | 35,518 | 1.01x | tensor | 932.39 | 577.0 | 1,154.0 | link_latency | 2.26x | 6.78x | 32.62x | 2.26x |
| DeepSeek-V4-Flash-0731 | 4 | fastest | DSV4-Flash/ROM-N6-native-HBMKV-wafer-hybrid-x4-romfill | 184,900 | 3,331.4 | 13,325.7 | link_latency | DSV4-Flash/a100_sxm_80gb-x224-tensor | 185,024 | 1.00x | tensor | 1,129.91 | 565.2 | 2,260.7 | link_latency | 5.89x | 5.89x | 294.98x | 5.89x |
| DeepSeek-V4-Flash-0731 | 4 | smallest silicon | DSV4-Flash/ROM-N6-native-HBMKV-array-hybrid-x44 | 35,860 | 1,303.6 | 7,821.3 | link_latency | DSV4-Flash/a100_sxm_80gb-x43-tensor | 35,518 | 1.01x | tensor | 1,085.62 | 456.7 | 1,826.8 | link_latency | 2.85x | 4.28x | 32.62x | 2.85x |
| DeepSeek-V4-Flash-0731 | 8 | fastest | DSV4-Flash/ROM-N6-native-HBMKV-wafer-hybrid-x8-romfill | 369,800 | 3,121.2 | 24,969.6 | link_latency | DSV4-Flash/a100_sxm_80gb-x448-tensor | 370,048 | 1.00x | tensor | 1,841.89 | 391.8 | 3,134.4 | link_latency | 7.97x | 7.97x | 519.42x | 7.97x |
| DeepSeek-V4-Flash-0731 | 8 | smallest silicon | DSV4-Flash/ROM-N6-native-HBMKV-array-hybrid-x44 | 35,860 | 1,146.8 | 9,174.3 | link_latency | DSV4-Flash/a100_sxm_80gb-x43-tensor | 35,518 | 1.01x | tensor | 1,392.09 | 335.9 | 2,686.9 | link_latency | 3.41x | 3.41x | 28.70x | 3.41x |
| DeepSeek-V4-Flash-0731 | 16 | fastest | DSV4-Flash/ROM-N6-native-HBMKV-wafer-hybrid-x12-romfill | 554,700 | 2,656.1 | 42,497.1 | link_latency | DSV4-Flash/a100_sxm_80gb-x672-tensor | 555,072 | 1.00x | tensor | 2,563.51 | 298.2 | 4,770.7 | link_latency | 8.91x | 8.91x | 648.68x | 8.91x |
| DeepSeek-V4-Flash-0731 | 16 | smallest silicon | DSV4-Flash/ROM-N6-native-HBMKV-array-hybrid-x44 | 35,860 | 774.3 | 12,389.1 | compute | DSV4-Flash/a100_sxm_80gb-x43-tensor | 35,518 | 1.01x | tensor | 2,005.01 | 230.6 | 3,689.3 | link_latency | 3.36x | 3.36x | 19.38x | 3.36x |
| DeepSeek-V4-Flash-0731 | 32 | fastest | DSV4-Flash/ROM-N6-native-HBMKV-wafer-hybrid-x12-romfill | 554,700 | 1,922.9 | 61,532.5 | kv_read | DSV4-Flash/a100_sxm_80gb-x672-tensor | 555,072 | 1.00x | tensor | 3,998.70 | 201.6 | 6,449.9 | link_latency | 9.54x | 9.54x | 469.62x | 9.54x |
| DeepSeek-V4-Flash-0731 | 32 | smallest silicon | DSV4-Flash/ROM-N6-native-HBMKV-array-hybrid-x44 | 35,860 | 469.4 | 15,020.9 | compute | DSV4-Flash/a100_sxm_80gb-x43-tensor | 35,518 | 1.01x | tensor | 3,230.86 | 150.0 | 4,798.9 | link_latency | 3.13x | 3.13x | 11.75x | 3.13x |
| DeepSeek-V4-Flash-0731 | 64 | fastest | DSV4-Flash/ROM-N6-native-HBMKV-wafer-hybrid-x12 | 554,700 | 1,238.9 | 79,290.6 | kv_read | DSV4-Flash/a100_sxm_80gb-x672-tensor | 555,072 | 1.00x | tensor | 6,869.09 | 124.3 | 7,955.9 | link_latency | 9.97x | 9.97x | 302.58x | 9.97x |
| DeepSeek-V4-Flash-0731 | 64 | smallest silicon | DSV4-Flash/ROM-N6-native-HBMKV-array-pipeline-x44 | 35,860 | 294.9 | 18,871.7 | compute | DSV4-Flash/a100_sxm_80gb-x43-tensor | 35,518 | 1.01x | tensor | 5,682.56 | 94.3 | 6,032.2 | link_latency | 3.13x | 3.13x | 8.72x | 3.13x |
| DeepSeek-V4-Flash-0731 | 256 | fastest | DSV4-Flash/ROM-N6-native-HBMKV-array-hybrid-x340-romfill | 277,100 | 746.5 | 191,107.3 | link_latency | DSV4-Flash/a100_sxm_80gb-x335-tensor | 276,710 | 1.00x | tensor | 23,833.74 | 38.4 | 9,825.5 | link_latency | 19.45x | 19.45x | 94.92x | 19.45x |
| DeepSeek-V4-Flash-0731 | 256 | smallest silicon | DSV4-Flash/ROM-N6-native-HBMKV-array-pipeline-x44 | 35,860 | 75.6 | 19,364.4 | compute | DSV4-Flash/a100_sxm_80gb-x43-hybrid | 35,518 | 1.01x | hybrid | 773.07 | 40.4 | 10,332.3 | weight_read | 1.87x | 1.87x | 4.38x | 1.87x |
...
|---|---:|---:|---:|---:|---:|---|---:|---:|---|
| DeepSeek-V4-Flash-0731 | 27 | 22,302 | 52.5 | 639.3 | 306.0 | tensor | 848.73 | 54.3% | link_latency |
| DeepSeek-V4-Flash-0731 | 43 | 35,518 | 40.0 | 690.9 | 254.1 | tensor | 855.78 | 59.1% | link_latency |
| DeepSeek-V4-Flash-0731 | 45 | 37,170 | 38.8 | 695.5 | 257.8 | tensor | 855.78 | 59.5% | link_latency |
| DeepSeek-V4-Flash-0731 | 46 | 37,996 | 38.3 | 697.7 | 259.7 | tensor | 855.78 | 59.7% | link_latency |
| DeepSeek-V4-Flash-0731 | 47 | 38,822 | 37.8 | 699.8 | 261.4 | tensor | 855.78 | 59.9% | link_latency |
| DeepSeek-V4-Flash-0731 | 55 | 45,430 | 34.1 | 713.4 | 239.0 | tensor | 857.79 | 61.2% | link_latency |
| DeepSeek-V4-Flash-0731 | 56 | 46,256 | 33.7 | 715.0 | 240.3 | tensor | 857.79 | 61.3% | link_latency |
| DeepSeek-V4-Flash-0731 | 73 | 60,298 | 28.0 | 734.2 | 186.9 | tensor | 861.41 | 63.2% | link_latency |
| DeepSeek-V4-Flash-0731 | 74 | 61,124 | 27.7 | 735.2 | 187.5 | tensor | 861.41 | 63.3% | link_latency |
| DeepSeek-V4-Flash-0731 | 75 | 61,950 | 27.4 | 736.1 | 188.1 | tensor | 861.41 | 63.4% | link_latency |
| DeepSeek-V4-Flash-0731 | 76 | 62,776 | 27.2 | 737.1 | 188.7 | tensor | 861.41 | 63.5% | link_latency |
| DeepSeek-V4-Flash-0731 | 77 | 63,602 | 26.9 | 738.0 | 189.3 | tensor | 861.41 | 63.6% | link_latency |
| DeepSeek-V4-Flash-0731 | 87 | 71,862 | 24.6 | 745.7 | 178.5 | tensor | 862.18 | 64.3% | link_latency |
| DeepSeek-V4-Flash-0731 | 92 | 75,992 | 23.6 | 748.8 | 166.8 | tensor | 862.82 | 64.6% | link_latency |
| DeepSeek-V4-Flash-0731 | 110 | 90,860 | 20.5 | 758.4 | 149.9 | tensor | 863.83 | 65.5% | link_latency |
| DeepSeek-V4-Flash-0731 | 111 | 91,686 | 20.4 | 758.9 | 150.1 | tensor | 863.83 | 65.6% | link_latency |
| DeepSeek-V4-Flash-0731 | 112 | 92,512 | 20.2 | 759.4 | 150.4 | tensor | 863.83 | 65.6% | link_latency |
| DeepSeek-V4-Flash-0731 | 125 | 103,250 | 18.5 | 764.4 | 135.4 | tensor | 864.58 | 66.1% | link_latency |
| DeepSeek-V4-Flash-0731 | 146 | 120,596 | 16.3 | 770.8 | 118.2 | tensor | 865.42 | 66.7% | link_latency |
| DeepSeek-V4-Flash-0731 | 150 | 123,900 | 15.9 | 772.0 | 118.7 | tensor | 865.42 | 66.8% | link_latency |
| DeepSeek-V4-Flash-0731 | 168 | 138,768 | 14.5 | 776.1 | 109.8 | tensor | 865.84 | 67.2% | link_latency |
| DeepSeek-V4-Flash-0731 | 224 | 185,024 | 11.3 | 784.9 | 86.5 | tensor | 866.85 | 68.0% | link_latency |
| DeepSeek-V4-Flash-0731 | 335 | 276,710 | 7.9 | 621.7 | 60.7 | tensor | 1,217.01 | 75.7% | link_latency |
| DeepSeek-V4-Flash-0731 | 336 | 277,536 | 7.8 | 621.7 | 60.7 | tensor | 1,217.01 | 75.7% | link_latency |
| DeepSeek-V4-Flash-0731 | 448 | 370,048 | 6.0 | 624.6 | 46.9 | tensor | 1,217.52 | 76.0% | link_latency |
| DeepSeek-V4-Flash-0731 | 672 | 555,072 | 4.1 | 627.5 | 32.2 | tensor | 1,218.02 | 76.4% | link_latency |
...
|---|---|---:|---|---|---|---:|---:|---:|---:|---|
| DSV4-Flash/ROM-N6-native-SRAMKV-array-pipeline-x46 | DeepSeek-V4-Flash-0731 | 46 | pipeline | nvlink3 | infiniband_hdr | 42 | 105.30 us | 949.7 tok/s | 9,496.8 tok/s | 37 x point_to_point span 2 on nvlink3 (traversals 1.0) = 93.51 us; 5 x point_to_point span 2 on infiniband_hdr (traversals 1.0) = 11.79 us |
...
| DSV4-Flash/ROM-N6-native-SRAMKV-array-tensor-x44 | DeepSeek-V4-Flash-0731 | 44 | tensor | nvlink3 | infiniband_hdr | 172 | 855.78 us | 116.9 tok/s | 1,168.5 tok/s | 86 x all_reduce span 8 on nvlink3 (traversals 2.0) = 436.16 us; 86 x all_reduce span 6 on infiniband_hdr (traversals 2.0) = 419.61 us |
...
| DSV4-Flash/ROM-N6-native-HBMKV-array-pipeline-x46 | DeepSeek-V4-Flash-0731 | 46 | pipeline | nvlink3 | infiniband_hdr | 42 | 105.30 us | 949.7 tok/s | 9,496.8 tok/s | 37 x point_to_point span 2 on nvlink3 (traversals 1.0) = 93.51 us; 5 x point_to_point span 2 on infiniband_hdr (traversals 1.0) = 11.79 us |
| DSV4-Flash/ROM-N6-native-HBMKV-wafer-pipeline-x3 | DeepSeek-V4-Flash-0731 | 3 | pipeline | on_wafer | inter_wafer | 42 | 5.25 us | 19,047.6 tok/s | 190,475.7 tok/s | 42 x point_to_point span 2 on on_wafer (traversals 1.0) = 5.25 us |
| DSV4-Flash/ROM-N6-native-HBMKV-array-tensor-x44 | DeepSeek-V4-Flash-0731 | 44 | tensor | nvlink3 | infiniband_hdr | 172 | 855.78 us | 116.9 tok/s | 1,168.5 tok/s | 86 x all_reduce span 8 on nvlink3 (traversals 2.0) = 436.16 us; 86 x all_reduce span 6 on infiniband_hdr (traversals 2.0) = 419.61 us |
| DSV4-Flash/ROM-N6-native-HBMKV-wafer-tensor-x3 | DeepSeek-V4-Flash-0731 | 3 | tensor | on_wafer | inter_wafer | 172 | 1,034.94 us | 96.6 tok/s | 966.2 tok/s | 86 x all_reduce span 57 on on_wafer (traversals 15.4) = 165.55 us; 86 x all_reduce span 3 on inter_wafer (traversals 2.0) = 869.39 us |
| DSV4-Flash/ROM-N6-native-HBMKV-array-hybrid-x46 | DeepSeek-V4-Flash-0731 | 46 | hybrid | nvlink3 | infiniband_hdr | 91 | 447.95 us | 223.2 tok/s | 2,232.4 tok/s | 86 x all_reduce span 8 on nvlink3 (traversals 2.0) = 436.16 us; 5 x point_to_point span 2 on infiniband_hdr (traversals 1.0) = 11.79 us |
| DSV4-Flash/ROM-N6-native-HBMKV-wafer-hybrid-x3 | DeepSeek-V4-Flash-0731 | 3 | hybrid | on_wafer | inter_wafer | 88 | 175.66 us | 569.3 tok/s | 5,692.8 tok/s | 86 x all_reduce span 57 on on_wafer (traversals 15.4) = 165.55 us; 2 x point_to_point span 2 on inter_wafer (traversals 1.0) = 10.11 us |
...
| DSV4-Flash/a100_sxm_80gb-x125-pipeline | DeepSeek-V4-Flash-0731 | 125 | pipeline | nvlink3 | infiniband_hdr | 42 | 105.30 us | 949.7 tok/s | 9,496.8 tok/s | 37 x point_to_point span 2 on nvlink3 (traversals 1.0) = 93.51 us; 5 x point_to_point span 2 on infiniband_hdr (traversals 1.0) = 11.79 us |
| DSV4-Flash/a100_sxm_80gb-x125-tensor | DeepSeek-V4-Flash-0731 | 125 | tensor | nvlink3 | infiniband_hdr | 172 | 864.58 us | 115.7 tok/s | 1,156.6 tok/s | 86 x all_reduce span 8 on nvlink3 (traversals 2.0) = 436.16 us; 86 x all_reduce span 16 on infiniband_hdr (traversals 2.0) = 428.42 us |
| DSV4-Flash/a100_sxm_80gb-x125-hybrid | DeepSeek-V4-Flash-0731 | 125 | hybrid | nvlink3 | infiniband_hdr | 101 | 471.53 us | 212.1 tok/s | 2,120.8 tok/s | 86 x all_reduce span 8 on nvlink3 (traversals 2.0) = 436.16 us; 15 x point_to_point span 2 on infiniband_hdr (traversals 1.0) = 35.37 us |
...
|---|---:|---|---|---|---:|---:|---:|---:|---:|---:|---|
| DeepSeek-V4-Flash-0731 | 1 | wafer | wafer | DSV4-Flash/ROM-N6-native-SRAMKV-wafer-tensor-x1 | 46,225 | 4,707.9 | 0.102 | 1,431.7 (37,490) | 4,707.9 (46,225) | 3.29x | link_latency |
| DeepSeek-V4-Flash-0731 | 2 | wafer | array | DSV4-Flash/ROM-N6-native-HBMKV-wafer-hybrid-x3-romfill | 138,675 | 3,388.5 | 0.024 | 1,430.6 (37,490) | 3,388.5 (138,675) | 2.37x | link_latency |
| DeepSeek-V4-Flash-0731 | 4 | wafer | array | DSV4-Flash/ROM-N6-native-HBMKV-wafer-hybrid-x4-romfill | 184,900 | 3,331.4 | 0.018 | 1,430.6 (37,490) | 3,331.4 (184,900) | 2.33x | link_latency |
| DeepSeek-V4-Flash-0731 | 8 | wafer | array | DSV4-Flash/ROM-N6-native-HBMKV-wafer-hybrid-x8-romfill | 369,800 | 3,121.2 | 0.008 | 1,334.5 (38,305) | 3,121.2 (369,800) | 2.34x | link_latency |
| DeepSeek-V4-Flash-0731 | 16 | wafer | array | DSV4-Flash/ROM-N6-native-HBMKV-wafer-hybrid-x12-romfill | 554,700 | 2,656.1 | 0.005 | 1,290.9 (45,640) | 2,656.1 (554,700) | 2.06x | link_latency |
| DeepSeek-V4-Flash-0731 | 32 | wafer | array | DSV4-Flash/ROM-N6-native-HBMKV-wafer-hybrid-x12-romfill | 554,700 | 1,922.9 | 0.003 | 1,211.2 (90,465) | 1,922.9 (554,700) | 1.59x | kv_read |
| DeepSeek-V4-Flash-0731 | 64 | wafer | array | DSV4-Flash/ROM-N6-native-HBMKV-wafer-hybrid-x12 | 554,700 | 1,238.9 | 0.002 | 1,149.9 (138,550) | 1,238.9 (554,700) | 1.08x | kv_read |
| DeepSeek-V4-Flash-0731 | 256 | array | array | DSV4-Flash/ROM-N6-native-HBMKV-array-hybrid-x340-romfill | 277,100 | 746.5 | 0.003 | 746.5 (277,100) | 395.3 (554,700) | 0.53x | link_latency |
...
| DeepSeek-V4-Flash-0731 | 8 | sram | 26,095.5 | 26,043.0 | 26,353.6 | 1.00x | 1.01x | weight_read | weight_read | link_latency |
| DeepSeek-V4-Flash-0731 | 16 | sram | 32,708.1 | 26,043.0 | 26,043.0 | 1.26x | 1.00x | kv_read | weight_read | weight_read |
| DeepSeek-V4-Flash-0731 | 32 | sram | 49,787.6 | 26,043.0 | 31,317.9 | 1.91x | 1.20x | kv_read | weight_read | weight_read |
| DeepSeek-V4-Flash-0731 | 64 | sram | 79,290.6 | 26,043.0 | 50,216.6 | 3.04x | 1.93x | kv_read | weight_read | kv_read |
| DeepSeek-V4-Flash-0731 | 256 | sram | 146,688.5 | 26,066.2 | 68,892.6 | 5.63x | 2.64x | weight_read | weight_read | kv_read |
| DeepSeek-V4-Flash-0731 | 1 | rom | 123,286.6 | 30,099.9 | 30,099.9 | 4.10x | 1.00x | weight_read | weight_read | weight_read |
| DeepSeek-V4-Flash-0731 | 2 | rom | 123,286.6 | 30,099.9 | 30,099.9 | 4.10x | 1.00x | weight_read | weight_read | weight_read |
| DeepSeek-V4-Flash-0731 | 4 | rom | 123,286.6 | 30,099.9 | 30,099.9 | 4.10x | 1.00x | weight_read | weight_read | weight_read |
| DeepSeek-V4-Flash-0731 | 8 | rom | 123,286.6 | 30,099.9 | 30,099.9 | 4.10x | 1.00x | weight_read | weight_read | weight_read |
| DeepSeek-V4-Flash-0731 | 16 | rom | 123,286.6 | 30,099.9 | 30,099.9 | 4.10x | 1.00x | weight_read | weight_read | weight_read |
| DeepSeek-V4-Flash-0731 | 32 | rom | 123,286.6 | 30,099.9 | 32,692.6 | 4.10x | 1.09x | weight_read | weight_read | link_latency |
| DeepSeek-V4-Flash-0731 | 64 | rom | 123,286.6 | 30,099.9 | 52,645.2 | 4.10x | 1.75x | weight_read | weight_read | weight_read |
| DeepSeek-V4-Flash-0731 | 256 | rom | 191,107.3 | 30,830.8 | 79,808.6 | 6.20x | 2.59x | link_latency | weight_read | kv_read |
...
|---|---:|---|---|---|---:|---:|---:|---:|---:|---|---:|
| DeepSeek-V4-Flash-0731 | 1 | batched | sram | DSV4-Flash/ROM-N6-native-HBMKV-wafer-pipeline-x12 | 554,700 | 1.00 | 1.00 | 26,095.5 | 0.047 | weight_read | 1.00x |
| DeepSeek-V4-Flash-0731 | 1 | batched | rom | DSV4-Flash/ROM-N6-native-HBMKV-array-pipeline-x340-romfill | 277,100 | 4.93 | 1.00 | 123,286.6 | 0.445 | weight_read | 4.72x |
| DeepSeek-V4-Flash-0731 | 1 | per_stream | sram | DSV4-Flash/ROM-N6-native-HBMKV-wafer-pipeline-x3-perstream | 138,675 | 1.00 | 1.00 | 26,043.0 | 0.188 | weight_read | 1.00x |
| DeepSeek-V4-Flash-0731 | 1 | per_stream | rom | DSV4-Flash/ROM-N6-native-HBMKV-array-pipeline-x93-perstream-romfill | 75,795 | 1.20 | 1.00 | 30,099.9 | 0.397 | weight_read | 1.15x |
| DeepSeek-V4-Flash-0731 | 1 | per_region | sram | DSV4-Flash/ROM-N6-native-HBMKV-wafer-pipeline-x3-perregion | 138,675 | 1.00 | 1.00 | 26,043.0 | 0.188 | weight_read | 1.00x |
| DeepSeek-V4-Flash-0731 | 1 | per_region | rom | DSV4-Flash/ROM-N6-native-HBMKV-array-pipeline-x93-perregion-romfill | 75,795 | 1.20 | 1.00 | 30,099.9 | 0.397 | weight_read | 1.15x |
| DeepSeek-V4-Flash-0731 | 2 | batched | sram | DSV4-Flash/ROM-N6-native-HBMKV-wafer-pipeline-x12 | 554,700 | 1.00 | 1.00 | 26,095.5 | 0.047 | weight_read | 1.00x |
| DeepSeek-V4-Flash-0731 | 2 | batched | rom | DSV4-Flash/ROM-N6-native-HBMKV-array-pipeline-x340-romfill | 277,100 | 4.93 | 1.00 | 123,286.6 | 0.445 | weight_read | 4.72x |
| DeepSeek-V4-Flash-0731 | 2 | per_stream | sram | DSV4-Flash/ROM-N6-native-HBMKV-wafer-pipeline-x3-perstream | 138,675 | 1.00 | 1.00 | 26,043.0 | 0.188 | weight_read | 1.00x |
| DeepSeek-V4-Flash-0731 | 2 | per_stream | rom | DSV4-Flash/ROM-N6-native-HBMKV-array-pipeline-x93-perstream-romfill | 75,795 | 1.20 | 1.00 | 30,099.9 | 0.397 | weight_read | 1.15x |
| DeepSeek-V4-Flash-0731 | 2 | per_region | sram | DSV4-Flash/ROM-N6-native-HBMKV-wafer-pipeline-x3-perregion | 138,675 | 1.00 | 1.00 | 26,043.0 | 0.188 | weight_read | 1.00x |
| DeepSeek-V4-Flash-0731 | 2 | per_region | rom | DSV4-Flash/ROM-N6-native-HBMKV-array-pipeline-x93-perregion-romfill | 75,795 | 1.20 | 1.00 | 30,099.9 | 0.397 | weight_read | 1.15x |
| DeepSeek-V4-Flash-0731 | 4 | batched | sram | DSV4-Flash/ROM-N6-native-HBMKV-wafer-pipeline-x12 | 554,700 | 1.00 | 1.00 | 26,095.5 | 0.047 | weight_read | 1.00x |
| DeepSeek-V4-Flash-0731 | 4 | batched | rom | DSV4-Flash/ROM-N6-native-HBMKV-array-pipeline-x340-romfill | 277,100 | 4.93 | 1.00 | 123,286.6 | 0.445 | weight_read | 4.72x |
| DeepSeek-V4-Flash-0731 | 4 | per_stream | sram | DSV4-Flash/ROM-N6-native-HBMKV-wafer-pipeline-x3-perstream | 138,675 | 1.00 | 1.00 | 26,043.0 | 0.188 | weight_read | 1.00x |
| DeepSeek-V4-Flash-0731 | 4 | per_stream | rom | DSV4-Flash/ROM-N6-native-HBMKV-array-pipeline-x93-perstream-romfill | 75,795 | 1.20 | 1.00 | 30,099.9 | 0.397 | weight_read | 1.15x |
| DeepSeek-V4-Flash-0731 | 4 | per_region | sram | DSV4-Flash/ROM-N6-native-HBMKV-wafer-pipeline-x3-perregion | 138,675 | 1.00 | 1.00 | 26,043.0 | 0.188 | weight_read | 1.00x |
| DeepSeek-V4-Flash-0731 | 4 | per_region | rom | DSV4-Flash/ROM-N6-native-HBMKV-array-pipeline-x93-perregion-romfill | 75,795 | 1.20 | 1.00 | 30,099.9 | 0.397 | weight_read | 1.15x |
| DeepSeek-V4-Flash-0731 | 8 | batched | sram | DSV4-Flash/ROM-N6-native-HBMKV-wafer-pipeline-x12 | 554,700 | 1.00 | 1.00 | 26,095.5 | 0.047 | weight_read | 1.00x |
| DeepSeek-V4-Flash-0731 | 8 | batched | rom | DSV4-Flash/ROM-N6-native-HBMKV-array-pipeline-x340-romfill | 277,100 | 4.93 | 1.00 | 123,286.6 | 0.445 | weight_read | 4.72x |
| DeepSeek-V4-Flash-0731 | 8 | per_stream | sram | DSV4-Flash/ROM-N6-native-HBMKV-wafer-pipeline-x3-perstream | 138,675 | 1.00 | 1.00 | 26,043.0 | 0.188 | weight_read | 1.00x |
| DeepSeek-V4-Flash-0731 | 8 | per_stream | rom | DSV4-Flash/ROM-N6-native-HBMKV-array-pipeline-x93-perstream-romfill | 75,795 | 1.20 | 1.00 | 30,099.9 | 0.397 | weight_read | 1.15x |
| DeepSeek-V4-Flash-0731 | 8 | per_region | sram | DSV4-Flash/ROM-N6-native-SRAMKV-wafer-hybrid-x2-perregion | 92,450 | 1.00 | 1.57 | 26,353.6 | 0.285 | link_latency | 1.01x |
| DeepSeek-V4-Flash-0731 | 8 | per_region | rom | DSV4-Flash/ROM-N6-native-HBMKV-array-pipeline-x93-perregion-romfill | 75,795 | 1.20 | 1.00 | 30,099.9 | 0.397 | weight_read | 1.15x |
| DeepSeek-V4-Flash-0731 | 16 | batched | sram | DSV4-Flash/ROM-N6-native-HBMKV-wafer-hybrid-x6 | 277,350 | 1.00 | 1.00 | 32,708.1 | 0.118 | kv_read | 1.00x |
| DeepSeek-V4-Flash-0731 | 16 | batched | rom | DSV4-Flash/ROM-N6-native-HBMKV-array-pipeline-x340-romfill | 277,100 | 4.93 | 1.00 | 123,286.6 | 0.445 | weight_read | 3.77x |
| DeepSeek-V4-Flash-0731 | 16 | per_stream | sram | DSV4-Flash/ROM-N6-native-HBMKV-wafer-pipeline-x3-perstream | 138,675 | 1.00 | 1.00 | 26,043.0 | 0.188 | weight_read | 0.80x |
| DeepSeek-V4-Flash-0731 | 16 | per_stream | rom | DSV4-Flash/ROM-N6-native-HBMKV-array-pipeline-x93-perstream-romfill | 75,795 | 1.20 | 1.00 | 30,099.9 | 0.397 | weight_read | 0.92x |
| DeepSeek-V4-Flash-0731 | 16 | per_region | sram | DSV4-Flash/ROM-N6-native-HBMKV-wafer-pipeline-x3-perregion | 138,675 | 1.00 | 1.00 | 26,043.0 | 0.188 | weight_read | 0.80x |
| DeepSeek-V4-Flash-0731 | 16 | per_region | rom | DSV4-Flash/ROM-N6-native-HBMKV-array-pipeline-x93-perregion-romfill | 75,795 | 1.20 | 1.00 | 30,099.9 | 0.397 | weight_read | 0.92x |
| DeepSeek-V4-Flash-0731 | 32 | batched | sram | DSV4-Flash/ROM-N6-native-HBMKV-wafer-hybrid-x8 | 369,800 | 1.00 | 1.00 | 49,787.6 | 0.135 | kv_read | 1.00x |
| DeepSeek-V4-Flash-0731 | 32 | batched | rom | DSV4-Flash/ROM-N6-native-HBMKV-array-pipeline-x340-romfill | 277,100 | 4.93 | 1.00 | 123,286.6 | 0.445 | weight_read | 2.48x |
| DeepSeek-V4-Flash-0731 | 32 | per_stream | sram | DSV4-Flash/ROM-N6-native-HBMKV-wafer-pipeline-x3-perstream | 138,675 | 1.00 | 1.00 | 26,043.0 | 0.188 | weight_read | 0.52x |
| DeepSeek-V4-Flash-0731 | 32 | per_stream | rom | DSV4-Flash/ROM-N6-native-HBMKV-array-pipeline-x93-perstream-romfill | 75,795 | 1.20 | 1.00 | 30,099.9 | 0.397 | weight_read | 0.60x |
| DeepSeek-V4-Flash-0731 | 32 | per_region | sram | DSV4-Flash/ROM-N6-native-HBMKV-array-hybrid-x78-perregion | 63,570 | 1.00 | 1.39 | 31,317.9 | 0.493 | weight_read | 0.63x |
| DeepSeek-V4-Flash-0731 | 32 | per_region | rom | DSV4-Flash/ROM-N6-native-HBMKV-array-hybrid-x93-perregion-romfill | 75,795 | 1.20 | 1.27 | 32,692.6 | 0.431 | link_latency | 0.66x |
| DeepSeek-V4-Flash-0731 | 64 | batched | sram | DSV4-Flash/ROM-N6-native-HBMKV-wafer-hybrid-x12 | 554,700 | 1.00 | 1.00 | 79,290.6 | 0.143 | kv_read | 1.00x |
| DeepSeek-V4-Flash-0731 | 64 | batched | rom | DSV4-Flash/ROM-N6-native-HBMKV-array-pipeline-x340-romfill | 277,100 | 4.93 | 1.00 | 123,286.6 | 0.445 | weight_read | 1.55x |
| DeepSeek-V4-Flash-0731 | 64 | per_stream | sram | DSV4-Flash/ROM-N6-native-HBMKV-wafer-pipeline-x3-perstream | 138,675 | 1.00 | 1.00 | 26,043.0 | 0.188 | weight_read | 0.33x |
| DeepSeek-V4-Flash-0731 | 64 | per_stream | rom | DSV4-Flash/ROM-N6-native-HBMKV-array-pipeline-x93-perstream-romfill | 75,795 | 1.20 | 1.00 | 30,099.9 | 0.397 | weight_read | 0.38x |
| DeepSeek-V4-Flash-0731 | 64 | per_region | sram | DSV4-Flash/ROM-N6-native-HBMKV-array-hybrid-x78-perregion | 63,570 | 1.00 | 1.97 | 50,216.6 | 0.790 | kv_read | 0.63x |
| DeepSeek-V4-Flash-0731 | 64 | per_region | rom | DSV4-Flash/ROM-N6-native-HBMKV-array-hybrid-x93-perregion-romfill | 75,795 | 1.20 | 1.82 | 52,645.2 | 0.695 | weight_read | 0.66x |
| DeepSeek-V4-Flash-0731 | 256 | batched | sram | DSV4-Flash/ROM-N6-native-HBMKV-array-hybrid-x227 | 185,005 | 1.00 | 1.00 | 146,688.5 | 0.793 | weight_read | 1.00x |
| DeepSeek-V4-Flash-0731 | 256 | batched | rom | DSV4-Flash/ROM-N6-native-HBMKV-array-hybrid-x340-romfill | 277,100 | 4.93 | 1.00 | 191,107.3 | 0.690 | link_latency | 1.30x |
| DeepSeek-V4-Flash-0731 | 256 | per_stream | sram | DSV4-Flash/ROM-N6-native-HBMKV-wafer-pipeline-x3-perstream | 138,675 | 1.00 | 1.50 | 26,066.2 | 0.188 | weight_read | 0.18x |
| DeepSeek-V4-Flash-0731 | 256 | per_stream | rom | DSV4-Flash/ROM-N6-native-HBMKV-array-pipeline-x93-perstream-romfill | 75,795 | 1.20 | 2.75 | 30,830.8 | 0.407 | weight_read | 0.21x |
| DeepSeek-V4-Flash-0731 | 256 | per_region | sram | DSV4-Flash/ROM-N6-native-HBMKV-array-hybrid-x78-perregion | 63,570 | 1.00 | 3.59 | 68,892.6 | 1.084 | kv_read | 0.47x |
| DeepSeek-V4-Flash-0731 | 256 | per_region | rom | DSV4-Flash/ROM-N6-native-HBMKV-array-hybrid-x93-perregion-romfill | 75,795 | 1.20 | 3.30 | 79,808.6 | 1.053 | kv_read | 0.54x |
...
|---|---:|---:|---:|---:|---:|---:|
| DeepSeek-V4-Flash-0731 | 1 | 75 | 1.00 | 1.000 | 1.000 | 1.00x |
...
| DeepSeek-V4-Flash-0731 | 16 | 78 | 1.00 | 1.000 | 1.000 | 1.00x |
| DeepSeek-V4-Flash-0731 | 32 | 78 | 1.00 | 1.000 | 1.000 | 1.00x |
...
| DeepSeek-V4-Flash-0731 | 2 | 27 | 5.47 | 4.22 | 1.30x |
| DeepSeek-V4-Flash-0731 | 4 | 27 | 5.47 | 4.22 | 1.30x |
| DeepSeek-V4-Flash-0731 | 8 | 27 | 5.47 | 4.22 | 1.30x |
| DeepSeek-V4-Flash-0731 | 16 | 27 | 5.47 | 4.22 | 1.30x |
| DeepSeek-V4-Flash-0731 | 32 | 27 | 6.34 | 4.56 | 1.39x |
| DeepSeek-V4-Flash-0731 | 64 | 27 | 11.08 | 6.15 | 1.80x |
| DeepSeek-V4-Flash-0731 | 256 | 27 | 23.14 | 10.27 | 2.25x |
...
| DeepSeek-V4-Flash-0731 | 2 | 74 | 5.80 | 5.09 | 1.14x |
| DeepSeek-V4-Flash-0731 | 4 | 74 | 5.80 | 5.09 | 1.14x |
| DeepSeek-V4-Flash-0731 | 8 | 74 | 5.80 | 5.09 | 1.14x |
| DeepSeek-V4-Flash-0731 | 16 | 74 | 5.80 | 5.09 | 1.14x |
| DeepSeek-V4-Flash-0731 | 32 | 74 | 5.80 | 5.09 | 1.14x |
| DeepSeek-V4-Flash-0731 | 64 | 74 | 5.80 | 5.09 | 1.14x |
| DeepSeek-V4-Flash-0731 | 256 | 74 | 17.76 | 9.74 | 1.82x |
| DeepSeek-V4-Flash-0731 | 1 | 75 | 5.80 | 5.10 | 1.14x |
| DeepSeek-V4-Flash-0731 | 2 | 75 | 5.80 | 5.10 | 1.14x |
| DeepSeek-V4-Flash-0731 | 4 | 75 | 5.80 | 5.10 | 1.14x |
| DeepSeek-V4-Flash-0731 | 8 | 75 | 5.80 | 5.10 | 1.14x |
| DeepSeek-V4-Flash-0731 | 16 | 75 | 5.80 | 5.10 | 1.14x |
| DeepSeek-V4-Flash-0731 | 32 | 75 | 5.80 | 5.10 | 1.14x |
| DeepSeek-V4-Flash-0731 | 64 | 75 | 5.80 | 5.10 | 1.14x |
| DeepSeek-V4-Flash-0731 | 256 | 75 | 17.59 | 9.70 | 1.81x |
| DeepSeek-V4-Flash-0731 | 1 | 76 | 5.81 | 5.11 | 1.14x |
| DeepSeek-V4-Flash-0731 | 2 | 76 | 5.81 | 5.11 | 1.14x |
| DeepSeek-V4-Flash-0731 | 4 | 76 | 5.81 | 5.11 | 1.14x |
| DeepSeek-V4-Flash-0731 | 8 | 76 | 5.81 | 5.11 | 1.14x |
| DeepSeek-V4-Flash-0731 | 16 | 76 | 5.81 | 5.11 | 1.14x |
| DeepSeek-V4-Flash-0731 | 32 | 76 | 5.81 | 5.11 | 1.14x |
| DeepSeek-V4-Flash-0731 | 64 | 76 | 5.81 | 5.11 | 1.14x |
| DeepSeek-V4-Flash-0731 | 256 | 76 | 17.42 | 9.67 | 1.80x |
...
| DeepSeek-V4-Flash-0731 | 1 | 125 | 5.88 | 5.41 | 1.09x |
| DeepSeek-V4-Flash-0731 | 2 | 125 | 5.88 | 5.41 | 1.09x |
| DeepSeek-V4-Flash-0731 | 4 | 125 | 5.88 | 5.41 | 1.09x |
| DeepSeek-V4-Flash-0731 | 8 | 125 | 5.88 | 5.41 | 1.09x |
| DeepSeek-V4-Flash-0731 | 16 | 125 | 5.88 | 5.41 | 1.09x |
| DeepSeek-V4-Flash-0731 | 32 | 125 | 5.88 | 5.41 | 1.09x |
| DeepSeek-V4-Flash-0731 | 64 | 125 | 5.88 | 5.41 | 1.09x |
| DeepSeek-V4-Flash-0731 | 256 | 125 | 11.61 | 8.57 | 1.35x |
...
| DeepSeek-V4-Flash-0731 | 2 | 150 | 5.90 | 5.49 | 1.07x |
| DeepSeek-V4-Flash-0731 | 4 | 150 | 5.90 | 5.49 | 1.07x |
| DeepSeek-V4-Flash-0731 | 8 | 150 | 5.90 | 5.49 | 1.07x |
| DeepSeek-V4-Flash-0731 | 16 | 150 | 5.90 | 5.49 | 1.07x |
| DeepSeek-V4-Flash-0731 | 32 | 150 | 5.90 | 5.49 | 1.07x |
| DeepSeek-V4-Flash-0731 | 64 | 150 | 5.90 | 5.49 | 1.07x |
| DeepSeek-V4-Flash-0731 | 256 | 150 | 9.85 | 8.03 | 1.23x |
...
|---|---:|---:|---:|---:|---:|
| DeepSeek-V4-Flash-0731 | 1 | 75 | 90.81% | 139.49 | 2.05 |
| DeepSeek-V4-Flash-0731 | 2 | 2 | 10.14% | 23.67 | 2.05 |
| DeepSeek-V4-Flash-0731 | 4 | 2 | 10.14% | 23.67 | 2.05 |
| DeepSeek-V4-Flash-0731 | 8 | 2 | 10.14% | 23.67 | 2.05 |
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| DeepSeek-V4-Flash-0731 | 1 | 2.34% | 11.2 GB | 6.72% | 325.47 TB/s | 4,841.92 TB/s | 422.4 | 18,585.1 |
| DeepSeek-V4-Flash-0731 | 2 | 2.34% | 11.2 GB | 6.72% | 325.47 TB/s | 4,841.92 TB/s | 422.4 | 18,585.1 |
| DeepSeek-V4-Flash-0731 | 4 | 2.34% | 11.2 GB | 6.72% | 325.47 TB/s | 4,841.92 TB/s | 422.4 | 18,585.1 |
| DeepSeek-V4-Flash-0731 | 8 | 2.34% | 11.2 GB | 6.72% | 325.47 TB/s | 4,841.92 TB/s | 422.4 | 18,585.1 |
| DeepSeek-V4-Flash-0731 | 16 | 2.34% | 11.2 GB | 6.72% | 325.47 TB/s | 4,841.92 TB/s | 422.4 | 18,585.1 |
| DeepSeek-V4-Flash-0731 | 32 | 2.34% | 11.2 GB | 6.72% | 325.47 TB/s | 4,841.92 TB/s | 422.4 | 18,585.1 |
| DeepSeek-V4-Flash-0731 | 64 | 3.39% | 12.8 GB | 7.65% | 370.19 TB/s | 4,841.92 TB/s | 294.9 | 18,871.7 |
| DeepSeek-V4-Flash-0731 | 256 | 12.89% | 26.7 GB | 16.02% | 775.76 TB/s | 4,841.92 TB/s | 75.6 | 19,364.4 |

## Binding constraint census

| Family | Binding constraint | Points |
|---|---|---:|
| gpu | link_latency | 204 |
| gpu | weight_read | 420 |
| rom | compute | 80 |
| rom | infeasible | 1018 |
| rom | kv_read | 144 |
| rom | link_latency | 606 |
| rom | weight_read | 392 |
