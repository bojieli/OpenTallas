...
module ot_a3_fp32_exp_pos_cr_rne #(
    ...
    localparam [3:0] S_RANGE_PRODUCT = 4'd11;
    localparam [3:0] S_RANGE_DIFF = 4'd12;
    reg [3:0] state;
    ...
    reg [WIDE:0] range_product_q, range_difference_q;
    reg range_negative_q;
    // Payload becomes valid only through the range states; no reset fanout.
    always @(posedge clk) begin
        if (state == S_RANGE_PRODUCT) range_product_q <= n_ln2_fixed;
        if (state == S_RANGE_DIFF) begin
            range_difference_q <= x_fixed - range_product_q;
            range_negative_q <= x_fixed < range_product_q;
        end
    end
        ...
        if (!rst_n) begin
            ...
            case (state)
                ...
                S_REDUCE: begin
                    ...
                    state <= S_RANGE_PRODUCT;
                end
                S_RANGE_PRODUCT: state <= S_RANGE_DIFF;
                S_RANGE_DIFF: state <= S_FIXUP;
                ...
                S_FIXUP: begin
                    ...
                    if (range_negative_q) begin
                        n_power <= n_power - 16'sd1;
                        state <= S_RANGE_PRODUCT;
                    end else if (range_difference_q >=
                                 {{(INT_BITS){1'b0}}, LN2}) begin
                        n_power <= n_power + 16'sd1;
                        state <= S_RANGE_PRODUCT;
                    end else begin
                        reduced <= range_difference_q[FRAC_BITS:0];
