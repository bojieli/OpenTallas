...
#(
    ...
    // Capacity is stable until retirement; read it in phase1 instead of
    // duplicating 32 bits in an operand register. apply_slot is also stable.
    reg [31:0] next_cursor_q;
    ...
    always @(posedge clk) begin
        ...
        if (apply_busy && apply_phase == 1) begin
            next_remaining_q <= {1'b0,slot_capacity[apply_slot]} - {1'b0,next_cursor_q};
