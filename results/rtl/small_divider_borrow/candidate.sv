...
module ot_wide_div_small_seq #(
    ...
    reg [DIVISOR_BITS+1:0] trial_difference;
    integer j;
    always @* begin
        walk = rem;
        ...
        for (j = BITS_PER_STEP - 1; j >= 0; j = j - 1) begin
            ...
            // The extra bit is the unsigned subtraction borrow. One carry
            // network supplies both the decision and the candidate remainder.
            trial_difference = {1'b0, walk} - {2'b0, divisor};
            if (!trial_difference[DIVISOR_BITS+1]) begin
                walk = trial_difference[DIVISOR_BITS:0];
                step_digit[j] = 1'b1;
            end
        end
        next_rem = walk;
