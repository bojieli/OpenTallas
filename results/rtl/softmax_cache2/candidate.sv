...
module ot_a3_attention_softmax_block #(
    ...
    // Two exact last-successful-result entries, newest first. The operation and arithmetic configuration
    ...
    reg exp_cache_valid2;
    reg [31:0] exp_cache_argument2, exp_cache_result2;
    wire cache_first_hit = exp_cache_valid && offset_q == exp_cache_argument;
    wire exp_cache_hit = EXP_REUSE && (cache_first_hit ||
                         (exp_cache_valid2 && offset_q == exp_cache_argument2));
    wire [31:0] cache_value = cache_first_hit ? exp_cache_result : exp_cache_result2;
    always @(posedge clk) begin
        if (exp_out_valid && exp_error == 0 &&
            (state == S_RESC_W || state == S_EXP_W)) begin
            exp_cache_argument2 <= exp_cache_argument;
            exp_cache_result2 <= exp_cache_result;
    ...
    wire [31:0] probability_value = state == S_OFF_W ? cache_value : exp_result;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            exp_cache_valid <= 0; exp_cache_valid2 <= 0;
            state <= S_IDLE;
            ...
            if (exp_out_valid && exp_error == 0 &&
                (state == S_RESC_W || state == S_EXP_W)) begin
                exp_cache_valid2 <= exp_cache_valid;
                exp_cache_valid <= 1;
            end
