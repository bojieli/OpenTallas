...
#(
    ...
    // Evaluate bounds beside descriptor matching. Selection carries only a
    // refusal bit, keeping lookup out of the add-and-compare dependency.
    // A missing descriptor always fails target_open before these tests.
    wire [SLOTS-1:0] slot_zero_refusal, slot_capacity_refusal;
    generate for (genvar n=0; n<SLOTS; n=n+1) begin : admission_slot
        wire unstaged = slot_policy[n] == ot_a3_pkg::A3_COMMIT_POLICY_UNSTAGED;
        wire saturating = slot_policy[n] == ot_a3_pkg::A3_COMMIT_POLICY_SATURATING;
        wire [32:0] ending = {1'b0,slot_cursor[n]} +
                            {1'b0,(unstaged ? 32'd0 : commit_span)};
        assign slot_zero_refusal[n] = slot_match[n] && !unstaged && commit_span==0;
        assign slot_capacity_refusal[n] = slot_match[n] && !saturating &&
                                         ending > {1'b0,slot_capacity[n]};
    end endgenerate
