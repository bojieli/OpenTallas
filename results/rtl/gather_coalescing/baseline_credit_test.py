...
@pytest.mark.parametrize('stride', [0, 2, 8])
def test_overlapped_reads_and_fault_drain(tmp_path, simulator, stride):
...
reg [31:0] command_generation=7;wire [31:0] command_object=1,command_lane_stride=STRIDE;
...
initial begin
 ...
 for(integer n=0;n<8;n=n+1)begin
  ...
  for(integer l=0;l<8;l=l+1)
   if(word_data[l*16+:16]!={byte_at(64'(n*256+l*STRIDE*2+1)),byte_at(64'(n*256+l*STRIDE*2))})$fatal(1,"wrong gathered data");
 ...
 if(reads-before_reads!=8*LINES || peak<WINDOW)$fatal(1,"overlap not exercised");
 // Foreign identity must not retire any expected read.
 begin_command();submit(0);wait(count==WINDOW);@(negedge clk);
 ...
 begin_command();submit(0);wait(count==WINDOW);@(negedge clk);fail_response=1;
...
''')
    ...
    for credits in [1,2,4,8]:
        ...
        if simulator == 'iverilog':
            ...
            compile_cmd=[str(verilator),'--binary','--timing','-j','4','-Wno-fatal','--top-module','tb',f'-GCREDITS={credits}',f'-GSTRIDE={stride}','--Mdir',str(obj),*sources]
            run_cmd=[str(obj/'Vtb')]
