...
campaign.RTL_SOURCES = tuple(
    'results/rtl/state_apply_overlap/candidate.sv'
    if p == 'rtl/abi3/ot_a3_state_controller.sv' else p
    for p in campaign.RTL_SOURCES
)
result = campaign.run(ROOT/'build/state_apply_overlap_containing')
result['candidate_scope'] = 'Isolated overlapping apply pipeline; production state controller unchanged.'
result['source_sha256']['results/rtl/state_apply_overlap/run_containing_campaign.py'] = campaign.sha256_file(Path(__file__))
output = ROOT/'results/rtl/state_apply_overlap/containing_campaign.json'
