...
#(
    ...
    // Valid only for used non-saturating slots; bit32 records underflow.
    // Initialized on allocation, refreshed atomically with each cursor write.
    reg [32:0] slot_remaining [0:SLOTS-1];
    reg [32:0] matched_remaining;
    ...
    always @* begin
        matched_remaining=0;matched_cursor=0;matched_capacity=0;matched_policy=0;matched_open=0;
        for(integer n=0;n<SLOTS;n=n+1)begin
            matched_remaining=matched_remaining | (slot_remaining[n] & {33{slot_match[n]}});
    ...
    reg [31:0] next_cursor_q, next_capacity_q;
    reg [32:0] next_remaining_q;
    always @(posedge clk) begin
        if (apply_busy && apply_phase == 0) begin
            next_cursor_q <= apply_end[31:0];
            next_capacity_q <= slot_capacity[apply_slot];
            apply_rows_q <= apply_rows;
            apply_row_bytes_q <= slot_row_bytes[apply_slot];
        end
        if (apply_busy && apply_phase == 1) begin
            next_remaining_q <= {1'b0,next_capacity_q} - {1'b0,next_cursor_q};
    ...
    // Payload validity is slot_used. Allocation initializes every field;
    // retirement is the only later writer. Keep reset off the private bank.
    always @(posedge clk) begin
        if (rst_n && !clear && !discard_all && !commit_all) begin
            if (apply_busy) begin
                if (modulo_state != 1 && !(needs_modulo && modulo_state == 0) &&
                    apply_phase == 2) begin
                slot_cursor[apply_slot] <=
                    apply_saturating ? apply_wrapped : apply_end[31:0];
                if (!apply_saturating) slot_remaining[apply_slot] <= next_remaining_q;
                slot_generation[apply_slot] <= slot_generation[apply_slot] + 32'd1;
                end
            end else if (op_valid && !hit_found && free_found) begin
                        slot_descriptor[target] <= op_descriptor_id;
                        slot_remaining[target] <= {1'b0,payload_capacity} - {1'b0,payload_initial_cur};
        ...
        if (!rst_n) begin
            ...
            if (clear) begin
                ...
                if (!hit_found && !free_found) begin
                    ...
                    if (!hit_found) slot_used[target] <= 1'b1;
                    case (op_sub)
                        ...
                        ot_a3_pkg::A3_STATE_COMMIT: begin
                            if (!target_open) begin
                                ...
                                op_trap_class <= ot_a3_pkg::A3_TRAP_STATE;
                                         ...
                                         (matched_remaining[32] ||
                                          (!target_unstaged && commit_span > matched_remaining[31:0]))) begin
