...
OUT=ROOT/'results/rtl/mul_operand_shift'
BUILD=ROOT/'build/mul_operand_shift_latency'
...
variants={'baseline':'results/rtl/positive_range_pipeline/candidate.sv','pipeline':'results/rtl/positive_range_pipeline/candidate.sv'}
def run(item):
 name,source=item;obj=BUILD/name
 selected=common if name=='baseline' else ['results/rtl/mul_operand_shift/candidate.sv',common[1]]
 cmd=[str(Path.home()/'.local/opentallas-tools/verilator-5.050/bin/verilator'),'--binary','--timing','-j','2','-Wno-fatal','--top-module','tb_a3_exp_pos','--Mdir',str(obj),*[str(ROOT/f) for f in selected],str(ROOT/source),str(BUILD/'tb.sv')]
...
assert c['active_cycles']==b['active_cycles']
files=[*common,*variants.values(),'rtl/test/tb_a3_exp_pos.sv','build/series_divisor_positive_vectors/cases.txt','results/rtl/mul_operand_shift/run_latency.py','results/rtl/mul_operand_shift/candidate.sv']
