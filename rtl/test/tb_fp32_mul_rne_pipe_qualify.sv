`timescale 1ns/1ps
// Qualify ot_fp32_mul_rne_pipe against ot_fp32_rne_pkg::fp32_mul_rne.
//
...
//
// A MULTIPLIER BREAKS IN DIFFERENT PLACES THAN AN ADDER, so the corpus is aimed
// at its own edges: a subnormal operand (which the authority NORMALIZES rather
// than flushing, so the product of two subnormals is not simply zero), a product
// that lands in the subnormal range and is therefore rounded in units of the
// subnormal LSB, a subnormal product that rounds UP into the smallest normal,
// ties-to-even at the round bit of a 48-bit product, and the overflow boundary
// that must FAIL CLOSED rather than encode an infinity. Signed zeros are here
// too because the authority gives canonical +0 for a zero operand whatever the
// signs, which is not what IEEE says.
module tb_fp32_mul_rne_pipe_qualify;
    reg clk = 0, rst_n = 0;
    reg valid_in = 0;
    reg [31:0] a = 0, b = 0;
    wire [31:0] y; wire [1:0] err; wire valid_out;

    ot_fp32_mul_rne_pipe dut (
    ...
    task push;
        ...
        begin
            ...
            q_a[wptr % QUEUE] = ca;
            q_b[wptr % QUEUE] = cb;
            q_e[wptr % QUEUE] = ot_fp32_rne_pkg::fp32_mul_rne(ca, cb);
            wptr = wptr + 1;
            issued = issued + 1;
        end
    endtask
    ...
    initial begin
        ...
        //: A random tail. Every 16th case pairs an operand with a RECIPROCAL-ISH
        //: partner -- its exponent mirrored about the bias -- so products land
        //: near 1.0 and near the subnormal floor far more often than uniform
        //: random exponents would put them there.
        ...
        for (i = 0; i < 880000; i = i + 1) begin
            next_a = pick(0);
            if (i[3:0] == 4'd0)
                push(next_a, {next_a[31], 8'd254 - next_a[30:23], next_a[22:0]});
        ...
        if (errors == 0)
            $display("PASS fp32_mul_rne_pipe: bit-identical to fp32_mul_rne over %0d cases at one result per cycle", checked);
        else
            $display("FAIL fp32_mul_rne_pipe: %0d of %0d cases differ", errors, checked);
        $finish;
    end
endmodule
