`timescale 1ns/1ps
module tb_qwen_ta_q_proj;
    localparam integer OUTPUT_BLOCKS = 64;
    ...
    initial begin
        $readmemh("q_proj_commands.hex", command_mem);
        $readmemh("q_proj_input.hex", input_mem);
        $readmemh("q_proj_accumulators.hex", expected_acc_mem);
        $readmemh("q_proj_output.hex", expected_output_mem);
        payload_file = $fopen("q_proj_weights.bin", "rb");
        if (!payload_file)
            $fatal(1, "cannot open q_proj_weights.bin");

        reset_case();
        submit_command(command_mem[0], 32'd3, 1'b1);
        await_done();
        ...
        $fclose(payload_file);
        $display(
            "PASS: complete Qwen q_proj RTL commands=2048 blocks=64 tiles=1024 inputs=262144 weights=16777216 accumulators=65536 bf16=4096 faults=1 cycles=%0d request_stalls=%0d read_stalls=%0d write_stalls=%0d vector_set=32dbdaa446fc4192f31a0a26394e04094e74c6459858d8a6b0135c11fc9e324b",
