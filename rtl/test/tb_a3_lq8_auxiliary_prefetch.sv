...
module tb_a3_lq8_auxiliary_prefetch;
    ...
    reg rst_n=0,clear=0,run=0,inject_stale=0;
    reg [31:0] generation=7;
    integer sent=0,responded=0,consumed=0,ticks=0,peak=0;
    wire request_ready,service_valid,response_ready,response_mismatch,auxiliary_valid;
    wire request_valid=run && sent<100;
    wire service_ready=ticks%7!=2;
    wire response_valid=inject_stale || (run && responded<sent && ticks>12 && ticks%5!=1);
    ...
    ot_a3_lq8_auxiliary_prefetch #(.DEPTH(DEPTH)) dut(
        .clk(clk),.rst_n(rst_n),.clear(clear),
        .request_valid(request_valid),.request_ready(request_ready),
        .request_generation(generation),.request_a(32'(sent+10)),.request_s(32'(sent+20)),
        .request_ws(32'(sent+30)),.request_w(32'(sent)),
    ...
    always @(posedge clk)begin
        if(rst_n && !clear && run)begin
            ticks<=ticks+1;
            if(32'(occupied)>DEPTH || responded>sent || consumed>responded)$fatal(1,"reservation accounting");
    ...
    initial begin
        tick();rst_n=1;run=1;
        wait(occupied==CW'(DEPTH));@(negedge clk);inject_stale=1;
        ...
        clear=1;tick();clear=0;generation=8;sent=0;responded=0;consumed=0;ticks=0;run=1;
        ...
        tick();inject_stale=0;
        generation=9;sent=0;responded=0;consumed=0;ticks=0;run=1;
        wait(consumed==100);@(negedge clk);run=0;
