...
module tb_stage_top;
    ...
    reg schedule_commit_req=0, schedule_manifest_crc_ok=1; wire schedule_commit_ack, schedule_valid;
    ...
    ot_stage_top #(.SESSION_ENTRIES(4),.CREDIT_SINKS(2),.CREDIT_DEPTH(4),
        ...
        .schedule_wr_slot(schedule_wr_slot),.schedule_wr_data(schedule_wr_data),.schedule_commit_req(schedule_commit_req),
        .schedule_manifest_crc_ok(schedule_manifest_crc_ok),.schedule_commit_ack(schedule_commit_ack),.schedule_valid(schedule_valid));
    ...
    task automatic send_cmd;
        ...
        reg [239:0] b;
        begin
            b=240'b0; b[7:0]=op; b[15:8]=8'h04; b[23:16]=8'h01; b[31:24]=0; b[39:32]=0;
    ...
    integer responses=0; reg service_pending=0;
    ...
    initial begin
        ...
        repeat(8) @(negedge aon_clk); csr_write_control();
        repeat(12) @(negedge aon_clk); send_cmd(8'h01,24'h1,32'hcafe0001);
        repeat(20) @(negedge aon_clk); send_cmd(8'h10,24'h1,32'hcafe0002);
        repeat(150) @(negedge aon_clk);
        if(responses<2) begin $display("FAIL expected responses got %0d",responses); failures=failures+1; end
        if(failures==0) begin $display("PASS: stage top command/session/service integration"); $finish; end
