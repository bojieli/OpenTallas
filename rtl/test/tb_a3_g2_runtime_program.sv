...
module tb_a3_g2_runtime_program;
 ...
 reg [127:0] program_image[0:PROGRAM_WORDS-1],descriptor_image[0:DESCRIPTOR_WORDS-1],weight_image[0:80*ROWS-1];
 reg [31:0] expected[0:8*ROWS-1];
 ...
 always @(posedge clk)begin
  if(!rst_n)begin wactive<=0;auxvalid<=0;end
   ...
   if(weight_request_valid && !wactive)begin wactive<=1;wtag<=weight_request_tag ^ ((phase==3) ? 64'd1 : 64'd0);wi<=0;wn<=weight_request_words;wbase<=weight_request_address;
    if(weight_request_address+weight_request_words>80*ROWS)$fatal(1,"weight address out of bounds");end
   ...
   if(auxiliary_request_valid && !auxvalid)begin auxvalid<=1;agen<=auxiliary_request_generation;aw<=auxiliary_request_w;adata<=activation_image[auxiliary_request_a];
    if(auxiliary_request_a>=80*ROWS || auxiliary_request_w>=80*ROWS)$fatal(1,"auxiliary address out of bounds");end
   ...
   if(kick)seen<=0;
    ...
    for(integer i=0;i<8;i=i+1)if(part_we[i])begin
     if(part_addr[32*i+:32]>=ROWS)$fatal(1,"wrong output address");
 ...
 initial begin
  ...
  if(seen!={8*ROWS{1'b1}} || outputs!=8*ROWS)$fatal(1,"missing first outputs");
  ...
  if(seen!=0 || outputs!=8*ROWS)$fatal(1,"aborted operation wrote output");
  phase=1;launch();
  if(STRESS_OUTPUT)begin
   // Hold the last result until after compute and external acknowledgements.
   wait(seen[8*(ROWS-2)]);@(negedge clk);tail_hold=1;
  end
  drain(0);
  if(seen!={8*ROWS{1'b1}} || outputs!=16*ROWS)$fatal(1,"missing restart outputs");
  ...
  if(seen!=0 || outputs!=16*ROWS)$fatal(1,"transport fault wrote output");
  phase=1;launch();drain(0);
  if(seen!={8*ROWS{1'b1}} || outputs!=24*ROWS || runtime_generation!=5)$fatal(1,"transport fault recovery");
  if(STRESS_OUTPUT)begin
   ...
   if(seen[7:0]!=8'hff || outputs!=24*ROWS+8)$fatal(1,"queued abort result lost");
