...
module tb_a3_g2_descriptor_prefix;
 ...
 ot_a3_g2_descriptor_store #(.AUXILIARY_WORDS(WORDS)) dut(
 .clk(clk),.rst_n(rst_n),.desc_req(req),.desc_id(id),.desc_valid(valid),.desc_fault(fault),.desc_data(data),
 .aux_req(aux_req),.aux_id(aux_id),.aux_valid(aux_valid),.aux_fault(aux_fault),.aux_data(aux_data),
 ...
 task read_one(input bit aux,input integer record_id,input bit bad);integer before_reads;begin
  ...
  id=32'(record_id);aux_id=32'(record_id);req=!aux;aux_req=aux;started=cycles;tick();req=0;aux_req=0;
  if(aux)begin
   wait(aux_valid);@(negedge clk);aux_latency=cycles-started;
   if(aux_fault!=bad || aux_data!=(bad?1536'd0:record_pattern(record_id,WORDS)))$fatal(1,"aux prefix mismatch");
  end else begin
  ...
  if(ram_reads-before_reads!=(bad?0:aux?WORDS:6))$fatal(1,"wrong number of SRAM reads");
  checks=checks+1;tick();tick();
 end endtask
 initial begin
  tick();rst_n=1;tick();load(0);load(1);load(340);
  read_one(0,0,0);read_one(1,1,0);
  if(seq_latency-aux_latency!=6-WORDS)$fatal(1,"prefix latency did not remove unused beats");
  // Simultaneous requests retain sequencer priority and preserve both identities.
  id=0;aux_id=340;req=1;aux_req=1;tick();req=0;aux_req=0;
  ...
  if(aux_fault || aux_data!=record_pattern(340,WORDS))$fatal(1,"queued prefix response");
  ...
  read_one(0,1,0);
  $display("PASS descriptor prefix words=%0d checks=%0d saved_cycles=%0d",WORDS,checks,6-WORDS);$finish;
 end
