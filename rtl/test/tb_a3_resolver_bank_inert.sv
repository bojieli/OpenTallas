`timescale 1ns/1ps
// ---------------------------------------------------------------------------
// Inertness bench: the DISABLED build against the SHIPPED block.
//
// The optimisation's own correctness bench drives its fast build against its
// slow build of the SAME modified source.  That says nothing about whether the
// slow build is the block that shipped, and it was not: with both parameters
// zero the design still ran 2,318 cycles faster than the committed deployment
// count, so that evidence compared two modified builds and never touched
// original behaviour.
//
// This bench closes the hole.  ``ot_a3_resolver_bank_shipped`` is a verbatim
// copy of rtl/abi3/ot_a3_resolver_bank.sv and rtl/abi3/ot_a3_view_resolver.sv
// as they stand at the base commit, with only the module names changed, and it
// is driven from one stimulus beside
// ``ot_a3_resolver_bank #(.FAST_SCAN(0), .FAST_WALK(0))``.  The bar is not
// "the same answers": it is CYCLE-EXACT identity of every output, of the
// descriptor port's request and ID, and of the shared divider's request, on
// every clock edge out of reset, an aborted transaction included.  Latency IS
// compared here, unlike in the fast-versus-slow bench, because a disabled
// optimisation that runs faster is exactly the defect this repairs.
//
// The stimulus is the fast-versus-slow bench's, unchanged, so both benches
// cover the same paths: all 64 slot-occupancy patterns, every fail-closed path
// at slot zero and away from it, the A18 divider, the A26 edge phase, the
// 64-bit symbol second pass, ranks zero through six, 600 randomised operators
// and 24 mid-walk aborts.
// ---------------------------------------------------------------------------
module tb_a3_resolver_bank_inert;
    ...
    ot_a3_resolver_bank_shipped dut_shipped (
        .clk(clk), .rst_n(rst_n), .clear(clear),
    ...
    // -- cycle-exact inertness watch ---------------------------------------
    // The disabled build is not merely required to publish the same results as
    // the shipped block; it is required to BE the shipped block.  Every output
    // and every descriptor-port request is compared on every clock edge the
    // design is out of reset, an aborted transaction included, so a difference
    // of one cycle anywhere is a failure rather than a faster run.
    integer edges = 0, edge_bad = 0;
    task watch_eq;
        input [255:0]  name;
        input [1023:0] got;
        input [1023:0] want;
        begin
            if (got !== want) begin
                if (edge_bad < 20)
                    $display("FAIL cycle=%0d %0s: shipped=%0h disabled=%0h",
                             cycle, name, got, want);
                edge_bad = edge_bad + 1;
            end
        end
    ...
    always @(posedge clk) if (rst_n) begin
        edges = edges + 1;
        watch_eq("desc_req",      {1023'd0, req_f},   {1023'd0, req_s});
        if (req_f || req_s)
            watch_eq("desc_id",   {992'd0,  id_f},    {992'd0,  id_s});
        watch_eq("busy",          {1023'd0, busy_f},  {1023'd0, busy_s});
        watch_eq("done",          {1023'd0, done_f},  {1023'd0, done_s});
        watch_eq("fault",         {1023'd0, fault_f}, {1023'd0, fault_s});
        watch_eq("trap_class",    {1008'd0, trap_f},  {1008'd0, trap_s});
        watch_eq("slot_valid",    {1018'd0, sv_f},    {1018'd0, sv_s});
        watch_eq("slot_write",    {1018'd0, swr_f},   {1018'd0, swr_s});
        watch_eq("scale_valid",   {1018'd0, ssv_f},   {1018'd0, ssv_s});
        watch_eq("descriptor_id", {832'd0,  sdid_f},  {832'd0,  sdid_s});
        watch_eq("extent",        {832'd0,  sext_f},  {832'd0,  sext_s});
        watch_eq("scale_object",  {832'd0,  sso_f},   {832'd0,  sso_s});
        watch_eq("extent_axis",   {976'd0,  sax_f},   {976'd0,  sax_s});
        watch_eq("rank",          {976'd0,  srk_f},   {976'd0,  srk_s});
        watch_eq("offset",        {640'd0,  soff_f},  {640'd0,  soff_s});
        watch_eq("object",        {928'd0,  sobj_f},  {928'd0,  sobj_s});
        watch_eq("lo",            {784'd0,  slo_f},   {784'd0,  slo_s});
        watch_eq("hi",            {784'd0,  shi_f},   {784'd0,  shi_s});
        // the divider is a shared resource: the request vector it is driven
        // with is part of this block's observable behaviour too
        watch_eq("div_req",       {1018'd0, dreq_f},  {1018'd0, dreq_s});
        if ((dreq_f != 6'd0) || (dreq_s != 6'd0)) begin
            watch_eq("div_num",   {640'd0,  dnum_f},  {640'd0,  dnum_s});
            watch_eq("div_den",   {832'd0,  dden_f},  {832'd0,  dden_s});
        end
    end
    ...
    task expect_eq;
        ...
        begin
            ...
            if (got !== want) begin
                mismatch = mismatch + 1;
                $display("FAIL case=%0d %0s: shipped=%0h disabled=%0h",
                         cases, name, got, want);
            end
        end
    endtask
    ...
    task run_case;
        begin
            ...
            if (t_f < 0 || t_s < 0) begin
                mismatch = mismatch + 1;
                $display("FAIL case=%0d a build never asserted done (shipped=%0d disabled=%0d)",
                         cases, t_f, t_s);
            end else begin
                expect_eq("latency",     {352'd0, t_f[31:0]}, {352'd0, t_s[31:0]});
    ...
    initial begin
        ...
        for (i = 1; i <= 24; i = i + 1) begin
            ...
            if (busy_f || busy_s) begin
                mismatch = mismatch + 1;
                $display("FAIL: a build is still busy after clear (shipped=%0d disabled=%0d) at offset %0d",
                         busy_f, busy_s, i);
            end
        ...
        $display("cycle-exact edges compared=%0d differences=%0d", edges, edge_bad);
        mismatch = mismatch + edge_bad;
        $display("mean latency: shipped=%0d disabled=%0d over %0d case(s); worst shipped=%0d disabled=%0d",
                 fast_lat_total / timed, slow_lat_total / timed, timed,
                 fast_worst, slow_worst);
        if (mismatch == 0)
            $display("PASS: FAST_SCAN=0 FAST_WALK=0 is cycle-exactly the shipped block cases=%0d checks=%0d edges=%0d",
                     cases, checks, edges);
        else
