...
module tb_a3_shipped_prefix;
    ...
    localparam integer RESULT_WORDS = 2048;
    localparam [31:0] UNWRITTEN = 32'hdead_beef;

    reg [31:0] case_mem [0:CASES*CASE_STRIDE-1];
    reg [31:0] issue_mem [0:39];
    reg [31:0] expect_mem [0:1023];
    ...
    initial begin
        ...
        for (case_index = 0; case_index < CASES;
            ...
            check_equal("wait events", count_wait_events, 0);
            ...
            check_equal("last gather result count", engine_result_count,
                        record[27]);
            check_equal("last gather checked indices", engine_work_count, 1);
        ...
        $display("PASS: ABI3 shipped-prefix engine integration cases=4 launches=6 words=1024 capability_faults=4 checks=%0d", checks);
        $finish;
    end
endmodule
