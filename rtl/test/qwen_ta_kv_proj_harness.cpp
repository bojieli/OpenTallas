...
constexpr uint32_t kOutputBlocks = 32U;
...
constexpr uint32_t kOutputsPerProjection = 1024U;
constexpr uint64_t kHbmBase = 1278246912ULL;
...
constexpr uint64_t kKAuxiliaryBase = 7340032ULL;
constexpr uint64_t kVAuxiliaryBase = 8388608ULL;
    ...
    void cycle(Vot_ta_dma_matmul_sequencer& dut) {
        ...
        if (write_fire) {
            if (write_address >= kWeightBase &&
                ...
                ++accumulator_writes;
            } else if ((write_address >= kKAuxiliaryBase &&
                        write_address < kKAuxiliaryBase +
                                            kOutputsPerProjection * 2ULL) ||
                       (write_address >= kVAuxiliaryBase &&
                        write_address < kVAuxiliaryBase +
                                            kOutputsPerProjection * 2ULL)) {
                const uint32_t index = write_address >= kVAuxiliaryBase
                    ? kOutputsPerProjection + static_cast<uint32_t>(
                          (write_address - kVAuxiliaryBase) >> 1U)
                    : static_cast<uint32_t>(
                          (write_address - kKAuxiliaryBase) >> 1U);
...
void prove_early_terminal(Vot_ta_dma_matmul_sequencer& dut, Model& model,
    ...
    submit(dut, model, records[0], 2051U, true);
    await_done(dut, model);
    require(dut.program_done_error == 12U &&
                dut.program_done_failing_command_index == 2051U &&
...
void prove_success(Vot_ta_dma_matmul_sequencer& dut, Model& model,
    ...
    for (uint32_t offset = 0; offset < kCommands; ++offset)
        submit(dut, model, records[offset], 2051U + offset,
               offset == kCommands - 1U);
    await_done(dut, model);
    require(dut.program_done_error == 0U &&
                dut.program_done_failing_command_index == 0xffffffffU &&
                dut.program_done_last_command_index == 3074U &&
            ...
            "complete K/V projection accounting differs");
    ...
    require(model.outputs == model.expected_outputs,
            "complete K/V projection BF16 payload differs");
    acknowledge(dut, model);
}
...
int main(int argc, char** argv) {
    ...
    const auto records = load_records("kv_proj_commands.hex", kCommands);
    const auto payload = load_binary("kv_proj_weights.bin", kDmaBytes);
    const auto inputs = load_codes16("kv_proj_input.hex", kInputs);
    const auto accumulators =
        load_codes32("kv_proj_accumulators.hex", kAccumulatorWrites);
    const auto outputs = load_codes16("kv_proj_output.hex", kOutputs);
