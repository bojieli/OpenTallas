...
// behavioural memories (tools/hdc_program_v41.py images from +DIR).  The core
// and memory wiring is tb_hdc_core_v41.sv's (generated from it; the bench's
// checking arrays and control are dropped).
//
...
`include "tb_host_ports.svh"
    ...
    localparam integer PLB = 8, SB = $clog2(NSLOT);
...
`include "tb_host_conn.svh"
    ...
    always @(posedge clk) begin
        ...
        // clear handshake: reset the core for a cycle, zero its user state, acknowledge
        clr_ack <= 1'b0;
