...
module ot_hdc_v41_matvec #(
    ...
    always @(posedge clk) begin
        for (gi = 0; gi < G; gi = gi + 1)
            x_addr[gi*AW +: AW] <= xc + ((gi & ((1 << split_r) - 1)) + (gi >> (LG - hg_r))) * xcs_r;
    end
    ...
    localparam integer TW = 1 + 1 + 1 + 1 + 1 + 2 + AW + AW + 3 * (NW + 1) + 2 + AW;
    ...
    assign {r_last, r_oen, r_amax, r_wsrc, r_mmode, r_split, r_oa, r_ots, r_nb, r_lb, r_nout, r_hg, r_ogs} = a_tag;
    ...
    always @(posedge clk) begin
        for (q = 0; q < G; q = q + 1)
            o_addr1[q*AW +: AW] <= r_oa + (q & r_tq) * r_ots + (q >> (LG - r_hg)) * r_ogs;
