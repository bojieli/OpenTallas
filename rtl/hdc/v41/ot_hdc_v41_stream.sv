...
//
// A 2-D loop (outer o, inner i) issues one element per cycle and never stalls
// (in SEQ-reduction ops, one element every 8 cycles).  Each element reads four
// operands and passes one fixed pipeline of qualified binary32 units:
//
...
// ot_hdc_fdiv, exactly as tools/hdc_golden_v41.py does.  The depth of an
// element depends on its op's CLASS (M1 divides or not, and the SFU function);
// a new op of another class waits for the unit to drain, so writes stay in
// order.  Everything travelling beside the data (B, C', D, immediates, control,
// addresses) rides tapped delay lines whose tap is the class depth.
//
// The reducer (ot_hdc_reduce) takes out, or out*out, per outer segment or over
// the whole op; SEQ is its SUM with one partial (elements 8 cycles apart).  A
// result may be rounded to BF16 (the address's top bit carries the flag).
// ---------------------------------------------------------------------------
...
module ot_hdc_v41_stream #(
    ...
    input  wire              i_redsq, i_redwhole, i_redrnd,
    input  wire [AW-1:0]     i_rbase, i_rso,
    input  wire [31:0]       i_imm1, i_imm2, i_imm3,
    // gather-index read (vector memory)
    output reg               vi_re,
    output reg  [AW-1:0]     vi_addr,
    input  wire [31:0]       vi_q,
    // operand reads: vector memory and constant ROM per stream, weight ROM for A
    ...
    assign ready = !active && ((i_cls == cls) || (inflight == 0));
    ...
    reg [1:0]        asrc, bsrc, csrc, dsrc, aind, dst, red;
    reg              bhalf, cpair, arnd, arelu, amin, cclip, rnd, redsq, redwhole, redrnd;
    reg [2:0]        m1, qm, ad, e1;
    reg [1:0]        m2, e2;
    ...
    always @(posedge clk) begin
        if (accept) begin
            ...
            rnd <= i_rnd; redsq <= i_redsq; redwhole <= i_redwhole; redrnd <= i_redrnd;
            ...
            if (!i_last_r) begin
                ...
                arow <= arow + aso; acol <= 0; brow <= brow + bso; bcol <= 0; crow <= crow + cso; ccol <= 0;
                drow <= drow + dso; dcol <= 0; orow_a <= orow_a + oso; ocol <= 0; rrow <= rrow + rso;
                krow <= krow + 1'b1;
            end
        end
    end

    // -- F0: address parts, gather index -------------------------------------------------------
    // per-element control that travels to the end (the "tail")
    localparam integer TT = 2 + 3 + 3 + 2 + 1 + 2 + AW + 2 + 1 + 1 + 1 + 1 + 1 + 3 + (AW + 1) + 32 + 32 + 1;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin f0_v <= 0; f1_v <= 0; f2_v <= 0; f3_v <= 0; f4_v <= 0; vi_re <= 0; end
            ...
            f0_v <= emit; f1_v <= f0_v; f2_v <= f1_v; f3_v <= f2_v; f4_v <= f3_v;
            vi_re <= emit && (aind != 2'd0);
        end
    end
    wire [AW-1:0] kv_t = (krow >> 4) << KVT_SH;
    always @(posedge clk) begin
        vi_addr <= aibase + ((aind == 2'd1) ? {{(AW-NW){1'b0}}, i} : {{(AW-NW){1'b0}}, o});
        f0_arow <= arow; f0_acol <= acol;
        f0_b <= brow + bcol; f0_c <= crow + ccol; f0_d <= drow + dcol;
        f0_o <= (dst == 2'd3) ? obase + kv_t + ({{(AW-NW){1'b0}}, i} << 4) + {{(AW-4){1'b0}}, krow[3:0]}
                              : orow_a + ocol;
        f0_par <= i[0];
        if (redwhole) begin
            ...
            f0_ifirst <= (i == 0); f0_first8 <= (i < 8);
            f0_final <= ($signed({1'b0, i}) >= $signed(fin_th)); f0_last <= i_last_r; f0_p <= i[2:0];
        end
        f0_r <= {redrnd, rrow};
    end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin vm_re <= 0; cr_re <= 0; wrom_re <= 0; end
            ...
            vm_re <= {4{f1_v}} & {dsrc == 2'd0, csrc == 2'd0, bsrc == 2'd0, asrc == 2'd0};
            cr_re <= {4{f1_v}} & {dsrc != 2'd0, csrc != 2'd0, bsrc != 2'd0, asrc == 2'd1 || asrc == 2'd2};
            wrom_re <= f1_v && asrc == 2'd3;
        end
    end
    always @(posedge clk) begin
    ...
    // F4: capture and select sources.  Op constants are read here: an op's
    // elements are at most 4 cycles behind its issue registers, and a following
    // op is accepted only after this op's last element left F0, so the fields
    // must be sampled per element: they are carried from F0 as a snapshot.
    ...
    // op snapshot (fields that the datapath needs), taken at emit, delayed to F4
    ...
    wire        clrv;                                   // class change: clear stale valid bits
    wire v_m1;
    ...
    wire [31:0] y_exp, y_rsq, y_sqrt, y_sp, y_spr;
    wire vo_e, vo_r, vo_s, vo_sp, f_e, f_r, f_s, f_sp;
    ot_hdc_rsqrt u_rsq (.clk(clk), .rst_n(rst_n), .v(v_r && sfu == SFU_RSQRT), .x(R), .y(y_rsq), .vo(vo_r), .fault(f_r));
    ot_hdc_softplus u_sp (.clk(clk), .rst_n(rst_n), .v(v_r && sfu == SFU_SPSQRT), .x(R), .sp(y_sp), .r(y_spr),
                          .vo(vo_sp), .fault(f_sp));
    ...
    ot_hdc_fsqrt u_sq (.clk(clk), .rst_n(rst_n), .v(sq_in_v), .a((sfu == SFU_EGATE) ? eg_m : R), .y(y_sqrt),
                       .vo(vo_s), .fault(f_s));
    reg  [31:0] eg_y;
    reg         eg_yv;
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) eg_yv <= 1'b0; else eg_yv <= vo_s && sfu == SFU_EGATE;
    end
    always @(posedge clk) eg_y <= {y_sqrt[31] ^ eg_sgn[D_SQRT], y_sqrt[30:0]};
    ...
    wire [31:0] den, num_d, y_div;
    wire f_den, f_div, vo_div;
    ...
    wire [AW:0] u_r = u_tail[TT-24-AW -: AW+1];
    reg  [31:0] out;
    ...
    wire [31:0] red_data0;
    wire f_red, red_busy;
    ot_hdc_reduce #(.AW(AW + 1)) u_red (.clk(clk), .rst_n(rst_n), .v_in(ov && o_red != RED_NONE),
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) red_we <= 1'b0; else red_we <= red_we0;
    end
    always @(posedge clk) begin
        red_addr <= red_addr0[AW-1:0];
        red_data <= red_addr0[AW] ? bf16(red_data0) : red_data0;
    end
    assign clrv = accept && (i_cls != cls);
    // -- status --------------------------------------------------------------------------------------------------------
    wire idle_c = !active && inflight == 0 && !vm_we && !kv_we && !red_we && !red_busy && !red_we0 &&
