`timescale 1ns/1ps
// ---------------------------------------------------------------------------
// Tensor-core-style pipelined MAC -- PROTOTYPE, timing evidence only.
//
// Measured on ASAP7: 725 MHz, 1,859 cells, 258 um2, critical path 1.380 ns.
// Against ot_fp32_rne_pkg::fp32_add_rne at 174 MHz and 2,458 cells: 4.2x the
// frequency in FEWER cells.  See docs/DATAPATH_PIPELINE_REDESIGN.md.
//
// NOT numerically qualified.  It exists to establish that GHz-class operation is
// reachable in this PDK with this toolchain; it has no campaign against a scalar
// reference and must not be wired into any consumer until it does.
//
