...
| ABI 3.0 microsequencer and control plane | `abi3/ot_a3_pkg.sv`, `abi3/ot_a3_program_header.sv`, `abi3/ot_a3_instruction_decoder.sv`, `abi3/ot_a3_shared_divider.sv`, `abi3/ot_a3_symbol_file.sv`, `abi3/ot_a3_loop_stack.sv`, `abi3/ot_a3_view_resolver.sv`, `abi3/ot_a3_resolver_bank.sv`, `abi3/ot_a3_event_scoreboard.sv`, `abi3/ot_a3_issue_record_store.sv`, `abi3/ot_a3_dependence_table.sv`, `abi3/ot_a3_state_controller.sv`, `abi3/ot_a3_microsequencer.sv`, `abi3/ot_a3_device_top.sv` | Program-header admission, instruction fetch/decode, loop nesting, predication, the three-bit event scoreboard (2,048 events, AM-C1/AM-C10), trap/completion, and operand tensor-view resolution on six lanes over one shared divider (A4 dynamic index terms, A13 partial final extent, A18 extent axis); the asynchronous front end of docs/CHIP_ARCHITECTURE_DESIGN.md section 3.2 -- issue with a serial and an issue-record slot, completion in any order through the completion port, retirement at completion, the dependence table ordering overlapped operations, counters frozen at the faulting serial -- behind the design's own host load path for the program store, the descriptor store and the 64-bit symbol file. `STATE_COMPAT=1` retains the historical ABI 3.0 state controller for compatibility tests. Every shipped comparison deployment uses `STATE_COMPAT=0`: the controller is not elaborated, compatibility counters are zero, nonzero state resources are refused before fetch, completion follows the program's terminal fence, and faults are fail-stop. `A3_EVENT_COUNT`, `A3_LOOP_DEPTH`, and `A3_WAIT_PRODUCERS` remain production bounds; `A3_STATE_SLOTS` applies only to the compatibility profile | A3-SEQ-001 |
...
| ABI 3.0 Qwen SwiGLU gate | `abi3/ot_a3_vector_silu_mul.sv`, `abi3/ot_a3_fp32_transcendental_cr_rne.sv`, `abi3/ot_a3_fp32_div_rne.sv`, `ot_fp32_rne_pkg.sv` | `qwen3_silu_mul_bf16_v1`: the stable sign-selected sigmoid as one correctly rounded binary32 exponential, one binary32 RNE add and one correctly rounded division; the SiLU activation *materialised* to BF16 and read back for the gating multiply, then one more RNE BF16 conversion. Activation and output saturations counted separately; nonfinite operands, uncertified exponentials, refused divisions and out-of-range products all fail closed | A3-OP-ADMIT-001 |
| ABI 3.0 on-device token append | `abi3/ot_a3_selection_token_append.sv` | `exact_token_append_eos_v1`: GENERATION_POLICY admission (greedy mode, request bound inside the immutable policy ceiling, EOS set inside the frozen ABI bound), vocabulary-bounded token, optional token-ring write, and the EOS-over-length precedence. A policy this device cannot honour causes no architectural read and no architectural write | A3-OP-ADMIT-001 |
...
| ABI 3.0 shipped-prefix issue bridge | `abi3/ot_a3_engine_issue_bridge.sv` | Descriptor- and view-checked connection from the real microsequencer to the exact shipped dense gather, BF16 embedding, Qwen RMSNorm, and DeepSeek stride-zero transfer profiles; completion only after datapath finish and precise descriptor/capability/engine traps for every refusal. Six further governed families -- `DMA.SCATTER`, `ATTENTION.GQA`, `VECTOR.ADD`, `VECTOR.SILU_MUL`, `SELECTION.ARGMAX` and `SELECTION.TOKEN_APPEND` -- share one admission walk that fetches every bound tensor view in slot order and evaluates the family's shape predicate over the captured slots. Those six use *mapped* result placement: the base comes from the descriptor's own primary object through the `cfg_map_object_N`/`cfg_map_base_N` table and the resolved element offset selects the plane, so a scatter's read-modify-write into a pre-existing KV plane, and the second residual add's write back over the trunk object it read, neither move nor are moved by the append cursor the original seven share | A3-SHIP-PREFIX-001, A3-OP-ADMIT-001 |
...
| ABI 3.0 Qwen runtime-context GQA continuation | `abi3/ot_a3_qwen_gqa.sv`, `abi3/ot_a3_qwen_gqa_adapter.sv`, `abi3/ot_a3_fp32_div_rne.sv`, `abi3/ot_a3_fp32_transcendental_cr_rne.sv` | Exact shipped ROM/HBM PC-38 descriptor admission and `[1,32,128] x [C,8,128]` grouped-query attention at a runtime context length inside the `MAX_CONTEXT` parameter's bound, with a context below the eight the frozen eight-lane softmax reduction is defined for refused as ERR_CONFIG: ordered binary32 products and reductions, BF16 score/probability/output boundaries, stable eight-lane softmax, complete-result buffering, ready/valid backpressure, and zero-write late-fault behavior. Only the current Q/K/V row is authentic; prior KV rows are synthetic, so this is not a layer, token, EOS, tick, or TPOT result | A3-QW-GQA-001 |
