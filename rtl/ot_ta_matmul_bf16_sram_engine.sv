`timescale 1ns/1ps
// Bounded data-bearing execution of the first production Qwen
// MATMUL_BF16_TILE profile: [1,256] @ [64,256]^T into 64 raw binary32
// accumulators.  BF16 operands widen exactly, every product and every
// increasing-K accumulation rounds once to binary32 RNE, and exact zero is
// canonicalized positive by the arithmetic package.
//
// This slice admits MATMUL_INIT without MATMUL_FINAL.  It therefore writes
// only the 64 FP32 accumulator codes at destination and deliberately performs
// no BF16 auxiliary write.  Inputs are retained while weights are streamed in
// strict output-major/K-minor order;
// complete accumulators remain buffered until every operand and arithmetic
// event succeeds.  An operand or arithmetic fault therefore cannot expose a
// partial accumulator tile.  SRAM macros, ECC, arbitration, and response-error
// signaling remain external.
module ot_ta_matmul_bf16_sram_engine #(
    ...
    localparam [2:0] STATE_INPUT_REQ    = 3'd1;
    localparam [2:0] STATE_INPUT_WAIT   = 3'd2;
    localparam [2:0] STATE_WEIGHT_REQ   = 3'd3;
    localparam [2:0] STATE_WEIGHT_WAIT  = 3'd4;
    ...
    wire [63:0] output_byte_offset = {56'b0, output_index, 2'b00};
    ...
    assign sram_write_address = destination_base + output_byte_offset;
    assign sram_write_data = accumulator_buffer[output_index];
    assign sram_write_byte_enable = 4'hf;
    ...
    wire [31:0] compute_accumulator_code = reduction_index == 0
                                             ? 32'b0
