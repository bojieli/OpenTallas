`timescale 1ns/1ps
// Strictly ordered execution boundary for the currently implemented production
// DMA_HBM_TO_SRAM and ADD_BF16 engines.  At most one command is admitted to an
// engine at a time.  A command error or a non-monotonic expected index ends the
// current program before another command can be accepted.  The external SRAM
// model or macro sees one shared write channel, so DMA population and ADD
// writeback cannot bypass the ordering point.
//
// This block deliberately does not claim to be the complete production command
// processor: it has no command-program header/body CRC, queue, timeout, ECC,
// bank arbiter, or COMPLETE-command implementation.  cmd_last delimits one
// bounded submitted program.
module ot_ta_dma_add_sequencer #(
    ...
    output reg  [31:0]  program_done_saturation_count
);
    ...
    localparam [2:0] STATE_DISPATCH_ADD = 3'd3;
    localparam [2:0] STATE_WAIT_ADD     = 3'd4;
    ...
    wire add_cmd_valid = state == STATE_DISPATCH_ADD;
    wire add_cmd_ready;
    wire add_done_valid;
    wire add_done_ready = state == STATE_WAIT_ADD;
    wire [7:0] add_done_error;
    wire [31:0] add_done_command_index;
    wire [31:0] add_done_element_count;
    wire [31:0] add_done_saturation_count;
    wire [31:0] add_done_sram_read_count;
    wire [31:0] add_done_sram_write_count;
    wire add_sram_read_valid;
    wire add_sram_read_ready;
    wire [63:0] add_sram_read_address;
    wire add_sram_response_ready;
    wire add_sram_write_valid;
    wire add_sram_write_ready;
    wire [63:0] add_sram_write_address;
    wire [15:0] add_sram_write_data;
    wire [1:0] add_sram_write_byte_enable;

    assign dma_sram_write_ready = (state == STATE_WAIT_DMA) &&
                                  sram_write_ready;
    assign add_sram_write_ready = (state == STATE_WAIT_ADD) &&
                                  sram_write_ready;
    assign sram_write_valid = state == STATE_WAIT_DMA
                              ? dma_sram_write_valid
                              : state == STATE_WAIT_ADD
                                ? add_sram_write_valid : 1'b0;
    assign sram_write_address = state == STATE_WAIT_DMA
                                ? dma_sram_write_address
                                : add_sram_write_address;
    assign sram_write_data = state == STATE_WAIT_DMA
                             ? dma_sram_write_data
                             : {112'b0, add_sram_write_data};
    assign sram_write_byte_enable = state == STATE_WAIT_DMA
                                    ? dma_sram_write_byte_enable
                                    : {14'b0, add_sram_write_byte_enable};

    assign sram_read_valid = (state == STATE_WAIT_ADD) &&
                             add_sram_read_valid;
    assign add_sram_read_ready = (state == STATE_WAIT_ADD) &&
                                 sram_read_ready;
    assign sram_read_address = add_sram_read_address;
    assign sram_response_ready = (state == STATE_WAIT_ADD) &&
                                 add_sram_response_ready;
    ...
    ot_ta_add_bf16_sram_engine add_engine (
        .clk(clk),
        .rst_n(rst_n),
        .cmd_valid(add_cmd_valid),
        .cmd_ready(add_cmd_ready),
        .abi_major(buffered_abi_major),
        .abi_minor(buffered_abi_minor),
        .expected_command_index(buffered_expected_index),
        .command_record(buffered_record),
        .sram_read_valid(add_sram_read_valid),
        .sram_read_ready(add_sram_read_ready),
        .sram_read_address(add_sram_read_address),
        .sram_response_valid(sram_response_valid),
        .sram_response_ready(add_sram_response_ready),
        .sram_response_data(sram_response_data),
        .sram_write_valid(add_sram_write_valid),
        .sram_write_ready(add_sram_write_ready),
        .sram_write_address(add_sram_write_address),
        .sram_write_data(add_sram_write_data),
        .sram_write_byte_enable(add_sram_write_byte_enable),
        .done_valid(add_done_valid),
        .done_ready(add_done_ready),
        .done_error(add_done_error),
        .done_command_index(add_done_command_index),
        .done_element_count(add_done_element_count),
        .done_saturation_count(add_done_saturation_count),
        .done_sram_read_count(add_done_sram_read_count),
        .done_sram_write_count(add_done_sram_write_count)
    );
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            program_done_saturation_count <= 0;
        end else begin
            ...
            if (state == STATE_DISPATCH_ADD && add_cmd_ready)
                state <= STATE_WAIT_ADD;
            ...
            if (state == STATE_WAIT_ADD && add_done_valid) begin
                program_done_last_command_index <= add_done_command_index;
                program_done_sram_read_count <=
                    program_done_sram_read_count +
                    add_done_sram_read_count;
                program_done_sram_bytes_read <=
                    program_done_sram_bytes_read +
                    (add_done_sram_read_count << 1);
                program_done_sram_write_count <=
                    program_done_sram_write_count +
                    add_done_sram_write_count;
                program_done_sram_bytes_written <=
                    program_done_sram_bytes_written +
                    (add_done_sram_write_count << 1);
                program_done_element_count <=
                    program_done_element_count + add_done_element_count;
                program_done_saturation_count <=
                    program_done_saturation_count +
                    add_done_saturation_count;
                if (add_done_error != ERR_NONE) begin
                    program_done_valid <= 1'b1;
                    program_done_error <= add_done_error;
                    program_done_failing_command_index <=
                        add_done_command_index;
                    state <= STATE_DONE;
                end else begin
                    program_done_commands_completed <=
                        program_done_commands_completed + 1'b1;
