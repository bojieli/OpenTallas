...
//
//     TX_STAGES + CHANNEL_CYCLES + RX_STAGES + (flits - 1)
//
...
module ot_rom_pkg_link #(
    ...
    // Receiver buffer (FIFO) sized by CREDITS.
    reg [W-1:0] fifo_data [0:CREDITS-1];
    reg         fifo_last [0:CREDITS-1];
    reg [CW-1:0] head, tail, fill;
    assign out_valid = (fill != 0);
    assign out_data  = fifo_data[head];
    assign out_last  = fifo_last[head];
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (drain) head <= (head + 1 == CREDITS) ? 0 : head + 1;
