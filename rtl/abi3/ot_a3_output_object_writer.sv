...
module ot_a3_output_object_writer #(
 ...
 reg [63:0] row_offset,cursor_offset,row_step,col_step,object_bytes;
 ...
 always @(posedge clk)begin
  if(command_valid && command_ready)begin
   ...
   row_offset<={32'b0,command_element_base}<<(command_fp32?2:1);
   cursor_offset<={32'b0,command_element_base}<<(command_fp32?2:1);
   row_step<={32'b0,command_row_stride}<<(command_fp32?2:1);
   col_step<={32'b0,command_col_stride}<<(command_fp32?2:1);
  ...
  if(accept_part && !protocol_error)begin
   ...
   for(lane=0;lane<LANES;lane=lane+1)
    write_offset[64*lane+:64]<=cursor_offset+col_step*64'(lane);
   ...
   if(cols_left==1)begin
    ...
    cols_left<=cols_left-1'b1;cursor_offset<=cursor_offset+col_step*64'(LANES);
   end
  end
 end
