`timescale 1ns/1ps
// One-token DeepSeek HC_PRE arithmetic behind explicit memory handshakes.
//
// The semantic configuration is admitted by the existing exact ROM-PC15 /
// HBM-PC14 tile scheduler.  The first real projection tile starts the
// handshake-native RMS/projection engine, whose result feeds the atomic
// coefficient tail.  Scheduler completion and every arithmetic stage must
// succeed before the packed 8 weight and 16 combination words become visible.
// A memory response error has its own result code and never publishes data.
//
// This remains one HC_PRE operator, not a transformer layer, token, EOS event,
// architectural latency measurement, or TPOT result.
module ot_a3_hc_pre_t1_handshake_rne #(
    ...
    localparam [7:0] ERR_MEMORY = 8'd34;
    ...
    wire projection_in_valid =
        state == S_WAIT_FIRST && scheduler_tile_valid && first_tile_expected;
    ...
    ot_a3_vector_mhc_pre_tile_scheduler scheduler (
        .clk(clk), .rst_n(rst_n), .start(scheduler_start),
        .config_words(config_q), .tile_valid(scheduler_tile_valid),
        .tile_ready(scheduler_tile_ready), .tile_kind(scheduler_tile_kind),
        ...
        .tile_last(scheduler_tile_last), .busy(scheduler_busy),
        .done(scheduler_done), .error_code(scheduler_error),
    ...
    ot_a3_hc_projection_rms_handshake_rne projection_rms (
        .clk(clk), .rst_n(rst_n), .in_valid(projection_in_valid),
        .in_ready(projection_in_ready),
        .hidden_req_valid(hidden_req_valid),
        .hidden_req_ready(hidden_req_ready), .hidden_req_k(hidden_req_k),
        .hidden_rsp_valid(hidden_rsp_valid),
        .hidden_rsp_ready(hidden_rsp_ready), .hidden_rsp_data(hidden_rsp_data),
        .hidden_rsp_error(hidden_rsp_error),
        .weight_req_valid(weight_req_valid),
        .weight_req_ready(weight_req_ready),
        .weight_req_field_base(weight_req_field_base),
        .weight_req_k(weight_req_k), .weight_rsp_valid(weight_rsp_valid),
        .weight_rsp_ready(weight_rsp_ready), .weight_rsp_data(weight_rsp_data),
        .weight_rsp_error(weight_rsp_error),
        .out_valid(projection_out_valid), .out_ready(projection_out_ready),
        ...
        .result_error(projection_error), .square_count(arithmetic_square_count),
    ...
    assign projection_out_ready = state == S_RUN &&
        ((projection_error != 0) || coefficient_in_ready);

    ot_a3_hc_coefficients_rne coefficients (
        .clk(clk), .rst_n(rst_n), .in_valid(coefficient_in_valid),
        .in_ready(coefficient_in_ready),
        .normalized_projection_codes(projection_normalized),
        .base_codes(base_q), .scale_codes(scale_q),
        .out_valid(coefficient_out_valid), .out_ready(coefficient_out_ready),
    ...
    wire success_out_valid = state == S_RUN && scheduler_finished &&
        coefficient_out_valid && coefficient_error == 0;
    assign coefficient_out_ready = state == S_RUN && coefficient_out_valid &&
        ((coefficient_error != 0) || (scheduler_finished && out_ready));
    assign out_valid = success_out_valid || state == S_ERROR_OUT;
    assign result_error = state == S_ERROR_OUT ? error_q : 0;
    assign result_weight_codes = success_out_valid && coefficient_error == 0
    ...
    function automatic [7:0] projection_error_code;
        input [1:0] error_value;
        begin
            case (error_value)
                2'd1: projection_error_code = ERR_ARITHMETIC_ARGUMENT;
                2'd2: projection_error_code = ERR_ARITHMETIC_RANGE;
                2'd3: projection_error_code = ERR_MEMORY;
                default: projection_error_code = ERR_INTEGRATION;
            endcase
        end
    endfunction
            ...
            case (state)
                ...
                S_RUN: begin
                    if (scheduler_done && scheduler_error != 0) begin
                        ...
                        if (scheduler_done)
                            scheduler_finished <= 1'b1;
                        if (projection_out_valid && projection_error != 0) begin
                            error_q <= projection_error_code(projection_error);
