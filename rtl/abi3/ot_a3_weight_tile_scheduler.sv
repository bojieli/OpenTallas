`timescale 1ns/1ps
// Two-bank sequential stream scheduler. Admission supplies the complete word
...
module ot_a3_weight_tile_scheduler #(
    ...
    assign reserve_words=fill_left<32'(TILE_WORDS)?fill_left[9:0]:10'(TILE_WORDS);
    assign tile_words=tile_left<32'(TILE_WORDS)?tile_left[9:0]:10'(TILE_WORDS);
        ...
        if(!rst_n)begin
            ...
            if(command_valid && command_ready)begin
                if(command_words==0 || range_end>33'h100000000)command_error<=1;
                    ...
                    fill_left<=command_words;tile_left<=command_words;
                    index<=0;reserve_bank<=0;tile_bank<=0;
                end
                ...
                if(tile_valid && tile_ready)begin
                    tile_base<=tile_base+{22'b0,tile_words};
                    tile_left<=tile_left-{22'b0,tile_words};tile_bank<=!tile_bank;
                end
