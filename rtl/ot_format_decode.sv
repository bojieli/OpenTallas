    ...
    always @* begin
        ...
        case (mode)
            4'd0: begin // signed integer DV value in low byte
                finite = 1'b1;
                negative = code[7];
                value_q16 = $signed(code[7:0]) <<< 16;
                mantissa = $signed(code[7:0]);
                exponent = 0;
                zero = (code[7:0] == 8'h00);
            end
            4'd1: begin // E2M1 nibble, represented exactly in Q2
                ...
                if (negative) begin
                    value_q16 = -($signed({1'b0,mag_q2}) <<< 14);
                    mantissa = -$signed({1'b0,mag_q2});
                end else begin
                    value_q16 = $signed({1'b0,mag_q2}) <<< 14;
                    mantissa = $signed({1'b0,mag_q2});
                end
            ...
            4'd2: begin // FP8 E4M3FN (finite-only)
                ...
                if (exp4 == 0) begin
                    if (frac3 == 0) begin
                        ...
                        mantissa = negative ? -$signed({1'b0,frac3}) : $signed({1'b0,frac3});
                        exponent = -9;
                    end
                    ...
                    finite = 1'b1;
                    unbiased = exp4 - 7;
                    mantissa = negative ? -$signed({1'b1,frac3}) : $signed({1'b1,frac3});
            ...
            4'd3: begin // BF16 classification; code[15:0] is BF16
                ...
                if (code[14:7] == 8'hff) begin
                    ...
                    mantissa = $signed({1'b0,code[6:0]});
                    exponent = $signed({1'b0,code[14:7]}) - 127;
                end
            end
            default: begin
