...
def activity(*, work: Path, bench: Path, bench_top: str, dut_module: str, rtl: list[Path],
    ...
    * window: [begin, end) in clock cycles, or None for the whole run.
    ...
    sources += [p for p in rtl if Path(p).stem not in netlists]
    ...
    cmd = ["verilator", "--cc", "--exe", "--build", "-O1", "-Wno-fatal", "-Wno-WIDTH", "-Wno-UNUSED",
           ...
           "--trace", "--top-module", bench_top, "-Mdir", str(obj),
    ...
    if fifo.exists():
        fifo.unlink()
    os.mkfifo(fifo)
    begin_t = (window[0] * 2 * half_period_ps) if window else 0
    end_t = (window[1] * 2 * half_period_ps) if window else None
    ...
    for name, scope in scopes.items():
        ...
        os.mkfifo(f)
        tee_fifos.append(f)
        saif = work / f"{name}.saif"
        args = [str(conv), str(f), str(saif), scope, str(begin_t)] + ([str(end_t)] if end_t else [])
    ...
    for name, saif, p in readers:
        _, err = p.communicate()
        meta["saif"][name] = {"path": str(saif), "scope": scopes[name], "converter": err.strip()[-400:],
                              "bytes": saif.stat().st_size if saif.exists() else 0}
