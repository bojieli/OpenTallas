...
def main(argv: list[str] | None = None) -> int:
    ...
    rp = sub.add_parser("report", help="splice Markdown tables into docs/POWER_CLOCK_SIGNOFF.md")
    rp.add_argument("--doc", default="docs/POWER_CLOCK_SIGNOFF.md")
    rp.add_argument("--energy", default="results/physical_abi3/asap7/signoff/energy_per_token.json")
    rp.add_argument("signoff", nargs="*", default=[
        "results/physical_abi3/asap7/signoff/qwen_core_signoff.json",
        "results/physical_abi3/asap7/signoff/hbm_kv_stream_signoff.json",
        "results/physical_abi3/asap7/signoff/v41_core_signoff.json",
        "results/physical_abi3/asap7/signoff/rom_fabric_signoff.json"])
    ...
    if args.cmd == "cell-models":
        ...
        splice_report(Path(args.doc), report_tables([Path(x) for x in args.signoff], Path(args.energy)))
...
# ---------------------------------------------------------------------------
# report: Markdown tables for docs/POWER_CLOCK_SIGNOFF.md
# ---------------------------------------------------------------------------
REPORT_BEGIN = "<!-- signoff-tables:begin (generated by tools/signoff_analysis.py report; do not edit) -->"
REPORT_END = "<!-- signoff-tables:end -->"
...
def _f(x: Any, fmt: str = "{:,.0f}") -> str:
    return "--" if x is None else fmt.format(x)


def report_tables(signoffs: list[Path], energy: Path | None) -> str:
    ...
    for arch, key, b in blocks:
        ...
        out.append(f"| {arch} | {key} | {b['routed'].get('stage', 'final')} | {act} | {_mw(p['total'])} | "
                   f"{_mw(p['internal'])} | {_mw(p['switching'])} | {_mw(p['leakage'])} | "
                   f"{_mw(tt['power_w']['clock']['total'])} | {_f(share and share * 100, '{:.0f}%')} |")
        for g, hp in sorted(tt.get("power_by_hierarchy_w", {}).items()):
            if g == "__other__" or not hp.get("instances"):
                continue
            out.append(f"| | &nbsp;&nbsp;`{g}*` ({hp['instances']:,} instances) | | | {_mw(hp['total'])} | "
                       f"{_mw(hp['internal'])} | {_mw(hp['switching'])} | {_mw(hp['leakage'])} | | |")
    out.append("\n#### Clock tree (TT)\n")
    ...
    for arch, key, b in blocks:
        ...
        if not ct:
            continue
        lat = ct.get("latency_rise_ns") or {}
        out.append(f"| {key} | {_f(ct.get('register_clock_pins'))} | {_f(ct.get('tree_cells'))} | "
                   f"{_f(ct.get('tree_cell_area_um2'), '{:,.1f}')} | "
                   f"{_f(lat.get('min_ns'), '{:.3f}')}-{_f(lat.get('max_ns'), '{:.3f}')} | "
                   f"{_f(ct.get('setup_skew_ns'), '{:.3f}')} | {_f(ct.get('hold_skew_ns'), '{:.3f}')} |")
    ...
    for arch, key, b in blocks:
        ...
        def fm(c: str, ocv: bool = False) -> str:
            t = cs.get(c, {}).get("timing_ocv" if ocv else "timing") or {}
            mp = t.get("min_period") or []
            return _f(mp[0]["fmax_mhz"], "{:,.0f}") if mp else "--"

        holds = " / ".join(_f((cs.get(c, {}).get("timing") or {}).get("hold_wns_ns"), "{:+.3f}") for c in
                           ("SS", "TT", "FF"))
        out.append(f"| {key} | {fm('SS')} | {fm('SS', True)} | {fm('TT')} | {fm('TT', True)} | {fm('FF')} | "
                   f"{fm('FF', True)} | {holds} |")
    ...
    for arch, key, b in blocks:
        ...
        for src in ("PINS", "BUMPS"):
            vdd = next((i for i in irs if i.get("source_type") == src and i.get("net") == "VDD"), None)
            vss = next((i for i in irs if i.get("source_type") == src and i.get("net") == "VSS"), None)
            if not vdd:
                continue
    ...
    if energy and Path(energy).exists():
        ...
        out.append("| Architecture | Step cycles | Logic uJ | Memory and links uJ | Static uJ | Token uJ | "
                   "pJ per weight MAC (matrix engine / whole step) | Not included |")
        out.append("|---|---:|---:|---:|---:|---:|---|---|")
        for name, a in e["architectures"].items():
            t = a["totals_j"]
            pm = a.get("pj_per_mac") or {}
            mac = (f"{pm['matrix_engine_logic_pj_per_mac']:.2f} / {pm['token_pj_per_mac']:.2f}" if pm else "--")
            miss = "; ".join(a.get("missing", [])) or "--"
            out.append(f"| {name} | {a['token_cycles']:,} | {t['logic_TT'] * 1e6:,.2f} | "
                       f"{t['memory_and_links'] * 1e6:,.2f} | {t['static'] * 1e6:,.2f} | {t['token_TT'] * 1e6:,.2f} | "
                       f"{mac} | {miss} |")
    return "\n".join(out) + "\n"


def splice_report(doc: Path, tables: str) -> None:
    text = doc.read_text()
    a, b = text.index(REPORT_BEGIN), text.index(REPORT_END)
    doc.write_text(text[:a] + REPORT_BEGIN + "\n\n" + tables + "\n" + text[b:])
