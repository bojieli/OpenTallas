...
def params_for(sv):
    # the variants are pipelined-step what-ifs, so they price the pipelined fadd/fdiv datapath (the model's
    # default now picks the faster of it and the routed ot_hdc_sinkhorn unit)
    return replace(D.Params(), fdiv_cycles=sv["fdiv_cycles"], sinkhorn_step_cycles=sv["sinkhorn_step_cycles"],
                   sinkhorn=sv["sinkhorn"], sinkhorn_impl="pipelined")
