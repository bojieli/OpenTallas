#!/usr/bin/env python3
"""Serial-latency attribution behind the roofline studies' headline points.

The operator dependency graph that ``roofline.evaluate`` prices every study
point with (``roofline.serial_step``) lives in ``src/opentallas/critical_path.py``.
It is the roofline's generalisation of ``tools/decode_critical_path.py``, the
standalone bottom-up study (and its topology sweep and the wafer-versus-array
study built on it), which remains the authority for the V4.1 decode core's
own design questions.  This tool only reports the attribution behind the
roofline's headline points:
...
results/roofline/critical_path/serial_latency_report.json.  Rerun it after the
...
SCHEMA = "opentallas.serial-latency-report.v1"
OUT = ROOT / "results/roofline/critical_path/serial_latency_report.json"
