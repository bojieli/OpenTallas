...
def build(scratch: Path) -> Path:
    ...
    subprocess.run(["verilator", "--cc", "--exe", "--build", "-O2", "-Wno-fatal", "-Wno-WIDTH", "-Wno-UNUSED",
                    ...
                    f"-I{SVH.parent}", *map(str, RTL), str(TB), str(HARNESS), "-CFLAGS", "-O1", "-j", "8"],
                   check=True, capture_output=True)
    return obj / "Vtb_hdc_core_v41"


def images(out: Path, *extra):
...
def run(ngen: int, context: int) -> dict:
    ...
    status = "pass" if one["pass"] and multi_rec["pass"] and lint.returncode == 0 and \
        (longc is None or longc["pass"]) else "fail"
    return {
        ...
        "claim_boundary": "functional token-level RTL simulation (Verilator, cycle-accurate at the core boundary) "
                          "with behavioural synchronous-read memories (many-ported vector memory); the Sinkhorn "
        ...
        "parameters": {"me_lanes_per_group": I.W_LANES, "me_groups": I.GROUPS, "interleave": I.INTERLEAVE,
...
def main() -> int:
    ...
    result = run(args.ngen, args.context)
