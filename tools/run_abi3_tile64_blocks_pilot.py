...
ELAPSED_RE = re.compile(r"Elapsed time: (\d+):(\d+):(\d+)\[h:\]min:sec")
...
def main(argv: list[str] | None = None) -> int:
    ...
    configs = write_configs(work, args.clock_period_ns, args.core_utilization, args.place_density, args.macro_halo)
    ...
    if args.leg == "block":
        goal = "build_macros"
        make = make_in_container(work, goal, work / "orfs_block.log", args.timeout_seconds)
        ...
        make = make_in_container(work, goal, work / "orfs_parent.log", args.timeout_seconds)
